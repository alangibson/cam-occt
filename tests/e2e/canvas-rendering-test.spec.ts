...
test.describe('Canvas Rendering Test', () => {
    test('should actually render DXF content on canvas immediately after loading', async ({
        page,
    }) => {
        ...
        const renderingInfo = await page.evaluate(() => {
            ...
            return {
                ...
                samplePixels,
                hasNonWhitePixels: samplePixels.some(
                    (p) => p.a > 0 && (p.r < 255 || p.g < 255 || p.b < 255)
                ),
            };
        });
        ...
        expect(
            renderingInfo.hasNonWhitePixels,
            'Canvas should have visible content after DXF load'
        ).toBe(true);
    });
});
