...
test.describe('CIRCLE Entity Rendering Tests', () => {
    test('should parse and render CIRCLE entities from DXF files', async ({
        page,
    }) => {
        await page.goto('/');
        ...
        console.log(
            'DXF file contains CIRCLE entities:',
            dxfContent.includes('CIRCLE')
        );
        ...
        expect(
            hasContent,
            'DXF with CIRCLE should render visible content on canvas'
        ).toBe(true);
        console.log(
            '✅ CIRCLE entities rendered successfully with visible content'
        );
    });
