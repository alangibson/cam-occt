...
test.describe('Optimize Starts Feature', () => {
  test('should show Optimize Starts button and optimize chain start points', async ({ page }) => {
    // Navigate to the app
    await page.goto('/');
    ...
    const optimizeButton = page.locator('button:has-text("Optimize Starts")');
    ...
    const chainsPanel = page.locator('text=/Chains.*chains with.*connected shapes/');
    await expect(chainsPanel).toBeVisible();
  });
  
  test('Optimize Starts button should be disabled when no chains detected', async ({ page }) => {
    // Navigate to the app
    await page.goto('/');
