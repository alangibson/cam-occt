...
test.describe('Basic CAM Workflow', () => {
  ...
  test('should have disabled tools when no drawing is loaded', async ({ page }) => {
    ...
    await expect(page.locator('button:has-text("Generate G-Code")')).toBeDisabled();
  });
  
  test('should show cutting parameters', async ({ page }) => {
    await expect(page.locator('label:has-text("Feed Rate")')).toBeVisible();
    await expect(page.locator('label:has-text("Pierce Height")')).toBeVisible();
    await expect(page.locator('label:has-text("Cut Height")')).toBeVisible();
    await expect(page.locator('label:has-text("Kerf Width")')).toBeVisible();
  });
});