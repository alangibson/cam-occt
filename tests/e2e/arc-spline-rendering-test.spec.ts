...
test.describe('ARC and SPLINE Entity Rendering Tests', () => {
    test('should parse and render ARC entities from DXF files', async ({
        page,
    }) => {
        ...
        for (const dxfFile of arcFiles) {
            ...
            if (!hasArcEntities) {
                console.log(
                    `⚠️  ${dxfFile} does not contain ARC entities, skipping`
                );
                continue;
            }
            ...
            expect(
                hasContent,
                `${dxfFile} should render visible ARC content on canvas`
            ).toBe(true);
            console.log(`✅ ${dxfFile} rendered visible content with ARCs`);
        }
    });

    test('should parse and render SPLINE entities from DXF files', async ({
        page,
    }) => {
        // Find DXF files that contain SPLINE entities
        const splineFiles = ['tests/dxf/polygons/nested-splines.dxf'];
        ...
        for (const dxfFile of splineFiles) {
            ...
            if (!hasSplineEntities) {
                console.log(
                    `⚠️  ${dxfFile} does not contain SPLINE entities, skipping`
                );
                continue;
            }
            ...
            expect(
                hasContent,
                `${dxfFile} should render visible SPLINE content on canvas`
            ).toBe(true);
            console.log(`✅ ${dxfFile} rendered visible content with SPLINEs`);
        }
    });

    test('should provide detailed debugging info for ARC/SPLINE parsing', async ({
        page,
    }) => {
