...
test.describe('Drawing Size Bug Tests', () => {
    test('should never show Infinity x Infinity in footer', async ({
        page,
    }) => {
        await page.goto('/');
        ...
        const sizeMatch = footerText?.match(
            /Size: (\d+\.\d+) mm × (\d+\.\d+) mm/
        );
        if (sizeMatch) {
    ...
    test('should handle multiple DXF files without Infinity', async ({
        page,
    }) => {
        const testFiles = [
            'tests/dxf/1.dxf',
