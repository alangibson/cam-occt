...
test.describe('Drawing Position Stability', () => {
    test('drawing should not drift vertically over time', async ({ page }) => {
        ...
        const screenshot1 = await canvas.screenshot({
            path: 'test-results/drawing-initial.png',
        });
        ...
        const screenshot2 = await canvas.screenshot({
            path: 'test-results/drawing-after-5s.png',
        });
    ...
    test('drawing position should be stable on Prepare stage', async ({
        page,
    }) => {
        // Navigate to the app
        await page.goto('/');
    ...
    test('resizing columns should not affect drawing position', async ({
        page,
    }) => {
        await page.goto('/');
        ...
        console.log(
            'Screenshot comparison after resize:',
            Buffer.compare(screenshotBefore, screenshotAfter) !== 0
                ? 'Different'
                : 'Same'
        );
    });
