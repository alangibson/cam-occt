...
test.describe('Simulation with offset paths', () => {
  ...
  async function createOperationWithOffset(page: Page) {
    ...
    const createButton = page.locator('button:has-text("Create"), button:has-text("Add Operation")').first();
    ...
    const kerfDropdown = page.locator('select').filter({ hasText: /kerf|offset|compensation/i });
  ...
  test('should detect and use offset paths when available', async ({ page }) => {
  ...
  test('should show both offset and original paths in canvas', async ({ page }) => {
    ...
    await page.screenshot({ path: 'test-output/simulation-offset-paths.png' });
  ...
  test('should fall back to original paths when no offset exists', async ({ page }) => {
  ...
  test('should handle mixed operations with and without offsets', async ({ page }) => {
  ...
  test('should update timing calculations with offset path lengths', async ({ page }) => {
  ...
  test('should maintain tool head position along offset paths', async ({ page }) => {
    // Start simulation
    const playButton = page.locator('button:has-text("Play")');
    await playButton.click();
    ...
    await page.waitForFunction(
      () => {
        const el = document.querySelector('*:has-text("Current Operation:")');
  ...
  test('should reset properly with offset paths', async ({ page }) => {
    ...
    // Check time is reset
    const timeText = await page.locator('text=/Time:.*00:00/').textContent();
