...
test.describe('DXF File Loading Tests', () => {
  for (const dxfFile of dxfFiles) {
    test(`should load ${dxfFile} without console errors`, async ({ page }) => {
      ...
      if (criticalErrors.length > 0) {
        console.log(`❌ ${dxfFile} produced console errors:`, criticalErrors);
      } else {
  ...
  test('summary: should load all DXF files without critical errors', async ({ page }) => {
    ...
    for (const dxfFile of dxfFiles.slice(0, 5)) { // Test first 5 files for summary
      const errors: string[] = [];
      ...
      try {
        ...
        allErrors.push({ file: dxfFile, errors: [`Failed to process: ${error}`] });
      }
