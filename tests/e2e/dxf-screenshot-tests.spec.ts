  ...
  for (const dxfFile of testFiles) {
    test(`should render ${dxfFile} visibly on screen`, async ({ page }) => {
      ...
      try {
        ...
        expect(errors, `${dxfFile} should load without errors`).toHaveLength(0);
        ...
        const fileName = dxfFile.split('/').pop()?.replace('.dxf', '') || 'unknown';
        
        // Take a screenshot of the canvas area
        await expect(canvas).toHaveScreenshot(`${fileName}-loaded.png`, {
        ...
        expect(hasContent, `${dxfFile} should render visible content on canvas`).toBe(true);
        
        console.log(`✅ ${dxfFile} rendered successfully with visible content`);
