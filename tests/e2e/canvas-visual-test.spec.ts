...
test.describe('Canvas Visual Test', () => {
    test('should render DXF content visually on canvas', async ({ page }) => {
        ...
        await expect(page.locator('.drawing-canvas')).toHaveScreenshot(
            'empty-canvas.png'
        );
        ...
        const hasContent = await page.evaluate(() => {
            ...
            console.log(
                `Canvas analysis: ${nonWhitePixels} non-white pixels out of ${pixelCount} total pixels`
            );
        ...
        expect(
            hasContent,
            'Canvas should have visible content after loading DXF'
        ).toBe(true);
    });
});
