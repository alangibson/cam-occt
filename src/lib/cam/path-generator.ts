import type { Shape, Point2D, Drawing, ToolPath, CuttingParameters } from '$lib/types';
...
function generateShapeToolPath(
  ...
  const compensatedPoints: Point2D[] = applyKerfCompensation(points, parameters.kerf);
  
  // Generate lead-in and lead-out
  const leadIn: Point2D[] = generateLeadIn(compensatedPoints[0], parameters.leadInLength);
