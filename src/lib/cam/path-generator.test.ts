...
import type { Drawing, CuttingParameters, Shape, Point2D, Line, Circle } from '../../lib/types';
...
describe('generateToolPaths', () => {
  ...
  const createMockCircle = (id: string, center: Point2D, radius: number): Shape => ({
    id,
    type: 'circle',
    layer: 'test',
  ...
  it('should generate tool paths for a simple drawing', () => {
    ...
    mockGetShapePoints
      .mockReturnValueOnce([{ x: 0, y: 0 }, { x: 100, y: 0 }])
      .mockReturnValueOnce([{ x: 75, y: 50 }, { x: 50, y: 75 }, { x: 25, y: 50 }, { x: 50, y: 25 }]);
  ...
  it('should apply kerf compensation', () => {
    const drawing: Drawing = {
      shapes: [createMockLine('shape1', { x: 0, y: 0 }, { x: 100, y: 0 })],
  ...
  it('should generate lead-in and lead-out paths', () => {
    ...
    mockGetShapePoints.mockReturnValueOnce([{ x: 0, y: 0 }, { x: 100, y: 0 }]);
    ...
    expect(paths[0].leadOut![1].x).toBeGreaterThan(paths[0].points[paths[0].points.length - 1].x);
  });
  ...
  describe('toolpath optimization algorithms', () => {
    ...
    it('should optimize cut sequence using nearest neighbor algorithm', () => {
      const drawing: Drawing = {
        shapes: [
          ...
          createMockLine('shape2', { x: 100, y: 100 }, { x: 110, y: 100 }),
      ...
      mockGetShapePoints
        ...
        .mockReturnValueOnce([{ x: 100, y: 100 }, { x: 110, y: 100 }])
        .mockReturnValueOnce([{ x: 15, y: 0 }, { x: 25, y: 0 }]);
    ...
    it('should skip shapes with insufficient points', () => {
      ...
      mockGetShapePoints
        ...
        .mockReturnValueOnce([{ x: 20, y: 0 }, { x: 30, y: 0 }]);
    ...
    it('should calculate travel distances accurately', () => {
      ...
      mockGetShapePoints
        .mockReturnValueOnce([{ x: 0, y: 0 }, { x: 10, y: 0 }])
        .mockReturnValueOnce([{ x: 50, y: 50 }, { x: 60, y: 50 }])
        .mockReturnValueOnce([{ x: 15, y: 5 }, { x: 25, y: 5 }]);
  ...
  describe('kerf compensation algorithm', () => {
    ...
    it('should handle closed paths with kerf compensation', () => {
      ...
      paths[0].points.forEach((point, i) => {
        if (i < 4) { // Skip the duplicate closing point
    ...
    it('should handle single point gracefully', () => {
      const drawing: Drawing = {
        shapes: [createMockLine('point', { x: 50, y: 50 }, { x: 50, y: 50 })],
  ...
  describe('lead generation algorithm', () => {
    it('should generate leads with correct lengths', () => {
      const drawing: Drawing = {
        shapes: [createMockLine('shape1', { x: 50, y: 50 }, { x: 100, y: 50 })],
        bounds: { min: { x: 50, y: 50 }, max: { x: 100, y: 50 } },
        units: 'mm'
      };
      
      mockGetShapePoints.mockReturnValueOnce([{ x: 50, y: 50 }, { x: 100, y: 50 }]);
      
      const customParams = { ...mockParameters, leadInLength: 10, leadOutLength: 15 };
    ...
    it('should handle zero-length leads', () => {
      ...
      const zeroLeadParams = { ...mockParameters, leadInLength: 0, leadOutLength: 0 };
