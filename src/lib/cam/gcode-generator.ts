...
function generateHeader(options: GCodeOptions): GCodeCommand[] {
  ...
  commands.push({
    ...
    parameters: {},
    comment: `Set units to ${
      options.units === "mm" ? "millimeters" : "inches"
    }`,
  });
  ...
  if (options.cutterCompensation !== null) {
    const compensationCode = options.cutterCompensation === "left_outer"
      ? "G41"
      : options.cutterCompensation === "right_inner"
      ? "G42"
      : "G40";
    const compensationComment = options.cutterCompensation === "left_outer"
...
function generateNativeSplineCommands(
  ...
  switch (shape.type) {
    case "spline":
      const spline: import("$lib/types/geometry").Spline = shape
        .geometry as Spline;
      ...
      if (spline.controlPoints && spline.controlPoints.length >= 2) {
        ...
        spline.controlPoints.forEach((point: Point2D, i: number) => {
          if (i > 0) { // Skip first point (already at start position)
            const weight: number = spline.weights && spline.weights[i]
              ? spline.weights[i]
              : 1;
            commands.push({
              code: "",
    ...
    case "arc":
      ...
      const startX: number = arc.center.x +
        arc.radius * Math.cos(arc.startAngle);
      const startY: number = arc.center.y +
    ...
    case "circle":
      ...
      const circle: import("$lib/types/geometry").Circle = shape
        .geometry as Circle;
      if (options.includeComments) {
        commands.push({
          code: "",
      ...
      Object.entries(cmd.parameters).forEach(([key, value]) => {
        if (key.startsWith("$")) {
          ...
          if (typeof value === "number") {
            ...
            const isCoordinate: boolean = ["X", "Y", "Z", "I", "J", "K"].includes(
              key,
            );
