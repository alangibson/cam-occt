...
export function getBoundingBoxForLine(line: Line): BoundingBox {
  if (!line.start || !line.end || 
    ...
    throw new Error('Invalid line: start and end points must be finite numbers');
  }
...
export function getBoundingBoxForCircle(circle: Circle): BoundingBox {
  if (!circle.center || !isFinite(circle.center.x) || !isFinite(circle.center.y) || 
    ...
    throw new Error('Invalid circle: center must be finite and radius must be positive');
  }
...
export function getBoundingBoxForArc(arc: Arc): BoundingBox {
  if (!arc.center || !isFinite(arc.center.x) || !isFinite(arc.center.y) || 
    ...
    throw new Error('Invalid arc: center, radius, and angles must be finite numbers');
  }
  ...
  const isAngleInArc: (testAngle: number) => boolean = (testAngle: number): boolean => {
  ...
  const extremeAngles: number[] = [0, Math.PI / 2, Math.PI, 3 * Math.PI / 2];
...
export function getBoundingBoxForEllipse(ellipse: Ellipse): BoundingBox {
  if (!ellipse.center || !isFinite(ellipse.center.x) || !isFinite(ellipse.center.y) ||
    ...
    throw new Error('Invalid ellipse: center, major axis endpoint, and ratio must be finite numbers');
  }
  ...
  const angle: number = Math.atan2(ellipse.majorAxisEndpoint.y, ellipse.majorAxisEndpoint.x);
...
export function combineBoundingBoxes(boxes: BoundingBox[]): BoundingBox {
  ...
  for (const box of boxes) {
    if (!box || !box.min || !box.max || 
        !isFinite(box.min.x) || !isFinite(box.min.y) ||
        !isFinite(box.max.x) || !isFinite(box.max.y)) {
      throw new Error('Invalid bounding box: min and max must be finite numbers');
    }
...
export function getBoundingBoxForShapes(shapes: Shape[]): BoundingBox {
  if (shapes.length === 0) {
    throw new Error('Cannot calculate bounding box for empty array of shapes');
  }
  
  const boundingBoxes: BoundingBox[] = shapes.map(shape => getBoundingBoxForShape(shape));
  return combineBoundingBoxes(boundingBoxes);
}

export function calculateDynamicTolerance(shapes: Shape[], fallbackTolerance: number = 0.1): number {
  ...
  try {
    ...
    const dynamicTolerance: number = Math.max(0.001, Math.min(1.0, diagonal * 0.001));
