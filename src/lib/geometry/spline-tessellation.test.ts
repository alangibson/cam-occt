...
describe('Spline Tessellation', () => {
  ...
  describe('tessellateSpline', () => {
    it('should tessellate linear spline correctly', () => {
      ...
      expect(['verb-nurbs', 'uniform-sampling', 'adaptive-sampling', 'fallback']).toContain(result.methodUsed);
      
      // First and last points should match control points
      expect(result.points[0]).toEqual({ x: 0, y: 0 });
      expect(result.points[result.points.length - 1]).toEqual({ x: 1, y: 0 });
    ...
    it('should tessellate quadratic bezier curve', () => {
      ...
      expect(result.points[0]).toEqual({ x: 0, y: 0 });
      expect(result.points[result.points.length - 1]).toEqual({ x: 2, y: 0 });
    });
    ...
    it('should handle closed splines correctly', () => {
      ...
      expect(result.points.length).toBeGreaterThanOrEqual(closedSpline.controlPoints.length);
    });
    ...
    it('should try multiple methods until one succeeds', () => {
      ...
      expect(['verb-nurbs', 'adaptive-sampling', 'uniform-sampling', 'fallback']).toContain(result.methodUsed);
    });
    ...
    it('should handle adaptive sampling configuration', () => {
      ...
      if (result.success) {
        expect(['verb-nurbs', 'adaptive-sampling']).toContain(result.methodUsed);
  ...
  describe('validateSplineGeometry', () => {
    ...
    it('should detect insufficient control points', () => {
      ...
      expect(errors).toContain('Spline must have at least 2 control points');
    });
    ...
    it('should detect degree too high for control points', () => {
      ...
      const errors = validateSplineGeometry(invalidSpline);
      expect(errors).toContain('Spline degree (3) must be less than number of control points (2)');
    });
    ...
    it('should detect weight count mismatch', () => {
      ...
      const errors = validateSplineGeometry(invalidSpline);
      expect(errors).toContain('Number of weights must match number of control points');
    });
  });
  ...
  describe('createAdaptiveTessellationConfig', () => {
    ...
    it('should create appropriate config for complex splines', () => {
      ...
      const config = createAdaptiveTessellationConfig(complexSpline, 0.001);
      
      expect(['verb-nurbs', 'adaptive-sampling']).toContain(config.method);
    ...
    it('should handle weighted splines appropriately', () => {
      ...
      const config = createAdaptiveTessellationConfig(weightedSpline, 0.01);
  ...
  describe('simplifyTessellatedSpline', () => {
    ...
    it('should preserve significant points', () => {
      ...
      const simplified = simplifyTessellatedSpline(significantPoints, 0.01);
    ...
    it('should handle edge cases', () => {
      ...
      expect(simplifyTessellatedSpline(singlePoint, 0.01)).toEqual(singlePoint);
      
      // Two points
      const twoPoints = [{ x: 0, y: 0 }, { x: 1, y: 0 }];
      expect(simplifyTessellatedSpline(twoPoints, 0.01)).toEqual(twoPoints);
    });
  ...
  describe('Real-world CAD scenarios', () => {
    it('should handle typical CAD spline curves', () => {
      ...
      expect(result.points[0]).toEqual({ x: 0, y: 0 });
      expect(result.points[result.points.length - 1]).toEqual({ x: 300, y: 0 });
    });
    ...
    it('should handle manufacturing tolerance requirements', () => {
      ...
      const config = createAdaptiveTessellationConfig(precisionSpline, 0.001);
