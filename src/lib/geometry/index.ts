import type { Shape, Point2D, Line, Circle, Arc, Polyline, Spline, Ellipse } from '$lib/types/geometry';
import { getLineStartPoint, getLineEndPoint, reverseLine, getLinePointAt } from './line';
import { getCircleStartPoint, getCircleEndPoint, reverseCircle, getCirclePointAt } from './circle';
import { getArcStartPoint, getArcEndPoint, reverseArc, getArcPointAt } from './arc';
...
export function scaleShape(shape: Shape, scaleFactor: number, origin: Point2D): Shape {
...
export function rotateShape(shape: Shape, angle: number, origin: Point2D): Shape {
        ...
        shapes: polyline.shapes.map((shape) => {
          ...
          if ('start' in segment && 'end' in segment) {
            ...
            return {
              ...shape,
              geometry: {
                start: { x: segment.start.x + delta.x, y: segment.start.y + delta.y },
                end: { x: segment.end.x + delta.x, y: segment.end.y + delta.y }
              }
            };
          } else if ('center' in segment && 'radius' in segment) {
            // Arc segment
            return {
              ...
              geometry: {
                ...segment,
                center: { x: segment.center.x + delta.x, y: segment.center.y + delta.y }
              }
            };
          }
          return shape;
