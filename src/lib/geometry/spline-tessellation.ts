 ...
 */
function createVerbCurveForSampling(spline: Spline): { curve: VerbCurve; domain: { min: number; max: number } } {
  const controlPoints3D: number[][] = spline.controlPoints.map(p => [p.x, p.y, 0]);
  const curve: VerbCurve = verb.geom.NurbsCurve.byControlPoints(controlPoints3D, spline.degree);
...
export interface SplineTessellationConfig {
  ...
  method: 'verb-nurbs' | 'adaptive-sampling' | 'uniform-sampling' | 'fallback';
...
export function tessellateSpline(
  ...
  const fullConfig: Required<SplineTessellationConfig> = { ...DEFAULT_CONFIG, ...config };
  ...
  const uniqueMethods: SplineTessellationConfig['method'][] = [...new Set(methods)];
  
  for (const method of uniqueMethods) {
    try {
      const result: SplineTessellationResult = tessellateWithMethod(spline, fullConfig, method, startTime);
      if (result.success) {
        return result;
      }
    } catch {
 ...
 */
function convertToClampedKnotVector(knots: number[], degree: number, numControlPoints: number): number[] {
...
function tessellateWithVerbNurbs(
  ...
  try {
    ...
    try {
      ...
      try {
        const clampedKnots = convertToClampedKnotVector(spline.knots, spline.degree, spline.controlPoints.length);
        curve = verb.geom.NurbsCurve.byKnotsControlPointsWeights(
          spline.degree,
          clampedKnots,
          controlPoints3D,
        ...
        curve = verb.geom.NurbsCurve.byControlPoints(controlPoints3D, spline.degree);
        warnings.push('Fell back to simplified verb NURBS construction');
      }
    }
    
    // Check timeout
    if (Date.now() - startTime > config.timeoutMs) {
      throw new Error('Verb NURBS tessellation timed out during curve creation');
    }
    ...
    const points2D: Point2D[] = tessellatedPoints.map(p => ({ x: p[0], y: p[1] }));
  ...
  try {
    ...
    while (sampleQueue.length > 0 && iterations < maxIterations) {
      ...
      const deviation: number = Math.sqrt(
        Math.pow(pMid[0] - chordMid[0], 2) + Math.pow(pMid[1] - chordMid[1], 2)
      );
      
      // Subdivide if deviation exceeds tolerance
      if (deviation > config.tolerance && sampleQueue.length < config.maxSamples) {
        sampleQueue.splice(maxGapIndex + 1, 0, { t: tMid, level: Math.max(sampleQueue[maxGapIndex].level, sampleQueue[maxGapIndex + 1].level) + 1 });
      } else {
...
function tessellateWithUniformSampling(
  ...
  try {
    ...
    for (let i: number = 0; i <= config.numSamples; i++) {
      ...
      const t: number = domain.min + (i / config.numSamples) * (domain.max - domain.min);
...
export function validateSplineGeometry(spline: Spline): string[] {
  ...
  if (spline.controlPoints && spline.degree >= spline.controlPoints.length) {
    errors.push(`Spline degree (${spline.degree}) must be less than number of control points (${spline.controlPoints.length})`);
  }
  ...
  if (spline.knots) {
    ...
    if (spline.knots.length !== expectedKnots) {
      errors.push(`Expected ${expectedKnots} knots but got ${spline.knots.length}`);
    }
  ...
  // Check weights
  if (spline.weights && spline.weights.length !== spline.controlPoints.length) {
...
export function createAdaptiveTessellationConfig(
  ...
  const hasWeights: boolean = spline.weights && spline.weights.some(w => Math.abs(w - 1) > 1e-6);
...
export function estimateSplineArcLength(
  ...
  const result: SplineTessellationResult = tessellateSpline(spline, { ...config, method: 'uniform-sampling', numSamples: 100 });
 ...
 */
export function simplifyTessellatedSpline(points: Point2D[], tolerance: number = 0.01): Point2D[] {
  if (points.length <= 2) {
    return [...points];
  }
  ...
  for (let i: number = 1; i < points.length - 1; i++) {
    ...
    const distance: number = Math.abs(A * current.x + B * current.y + C) / Math.sqrt(A * A + B * B);
