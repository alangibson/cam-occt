import type { Polyline, Point2D, PolylineVertex, Shape, Line, Arc, Circle, Ellipse, Spline } from '$lib/types/geometry';
import { generateId } from '$lib/utils/id';
import { calculateArcPoint, convertBulgeToArc } from '../utils/arc-utils';


function createBulgeOrLineShape(start: PolylineVertex, end: Point2D, bulge?: number): Shape {
...
export function createPolylineFromVertices(
  ...
  // Validate vertices
  const validVertices: PolylineVertex[] = vertices.filter(v => 
  ...
  if (closed && polylineVertices.length > 1) {
    ...
    const lastVertex: PolylineVertex = polylineVertices[polylineVertices.length - 1];
    
    // If not already coincident, duplicate the first vertex at the end
    if (firstVertex.x !== lastVertex.x || firstVertex.y !== lastVertex.y) {
      polylineVertices.push({ x: firstVertex.x, y: firstVertex.y, bulge: 0 });
    }
  }
 ...
 */
export function generateSegments(vertices: PolylineVertex[], closed: boolean = false): Shape[] {
  ...
  for (let i: number = 0; i < sourcePoints.length - 1; i++) {
    ...
    const bulge: number | undefined = 'bulge' in start && typeof start.bulge === 'number' ? start.bulge : undefined;
  ...
  if (closed && sourcePoints.length > 2) {
    ...
    const isAlreadyClosed: boolean = Math.abs(start.x - end.x) < EPSILON && Math.abs(start.y - end.y) < EPSILON;
...
export function polylineToVertices(polyline: Polyline): PolylineVertex[] {
  ...
  if (polyline.shapes) {
    ...
    for (let i: number = 0; i < polyline.shapes.length; i++) {
      const segment: Line | Arc | Circle | Polyline | Ellipse | Spline = polyline.shapes[i].geometry;
      ...
      if ('start' in segment && 'end' in segment) {
        ...
        const startPoint: Point2D = calculateArcPoint(arc.center, arc.radius, arc.startAngle);
        ...
        const bulge: number = Math.tan(includedAngle / 4) * (arc.clockwise ? -1 : 1);
    ...
    if (!polyline.closed && polyline.shapes.length > 0) {
      const lastSegment: Line | Arc | Circle | Polyline | Ellipse | import("$lib/types/geometry").Spline = polyline.shapes[polyline.shapes.length - 1].geometry;
      if ('start' in lastSegment && 'end' in lastSegment) {
        ...
        const arc: import("$lib/types/geometry").Arc = lastSegment as Arc;
        const endPoint: Point2D = calculateArcPoint(arc.center, arc.radius, arc.endAngle);
  ...
  if ('points' in polyline && Array.isArray((polyline as { points: Point2D[] }).points)) {
    return (polyline as { points: Point2D[] }).points.map((point: Point2D) => ({
      x: point.x,
      y: point.y,
      bulge: 0
    }));
...
export function polylineToPoints(polyline: Polyline): Point2D[] {
  ...
  if (polyline.shapes) {
    ...
    for (let i: number = 0; i < polyline.shapes.length; i++) {
      const segment: Line | Arc | Circle | Polyline | Ellipse | import("$lib/types/geometry").Spline = polyline.shapes[i].geometry;
...
export function reversePolyline(polyline: Polyline): Polyline {
  ...
  const reversedShapes: Shape[] = [...polyline.shapes].reverse().map(shape => {
    const segment: Line | Arc | Circle | Polyline | Ellipse | Spline = shape.geometry;
    if ('start' in segment && 'end' in segment) {
...
export function getPolylinePointAt(polyline: Polyline, t: number): Point2D {
  ...
  const totalLength: number = points.reduce((sum: number, pt: Point2D, i: number) => {
    if (i === 0) return 0;
  ...
  for (let i: number = 1; i < points.length; i++) {
    ...
    if (currentLength + segmentLength >= targetLength) {
      const segmentT: number = (targetLength - currentLength) / segmentLength;
      return {
        x: p1.x + segmentT * dx,
        y: p1.y + segmentT * dy
      };
