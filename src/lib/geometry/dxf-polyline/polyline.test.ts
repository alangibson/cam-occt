...
describe('createPolylineFromVertices', () => {
    it('should create a basic open polyline', () => {
        ...
        expect(polylineToPoints(polyline.geometry as DxfPolyline)).toEqual(
            points
        );
        expect((polyline.geometry as DxfPolyline).closed).toBe(false);
    ...
    it('should create a closed polyline with coincident endpoints', () => {
        ...
        expect((polyline.geometry as DxfPolyline).closed).toBe(true);
        const resultPoints = polylineToPoints(polyline.geometry as DxfPolyline);
    ...
    it('should handle optional parameters', () => {
        ...
        expect(polylineToVertices(polyline.geometry as DxfPolyline)).toEqual(
            vertices
        );
    ...
    it('should validate and filter invalid points', () => {
        const points: (DxfPolylineVertex | null)[] = [
            { x: 0, y: 0, bulge: 0 },
        ...
        const polyline = createPolylineFromVertices(
            points.filter(Boolean) as DxfPolylineVertex[],
            false
        );
...
describe('polyline utility functions', () => {
    it('should get start and end points correctly', () => {
        ...
        expect(
            getPolylineStartPoint(polylineShape.geometry as DxfPolyline)
        ).toEqual({ x: 0, y: 0 });
        expect(
            getPolylineEndPoint(polylineShape.geometry as DxfPolyline)
        ).toEqual({ x: 10, y: 10 });
    });
});
...
describe('reversePolyline', () => {
    it('should reverse an open polyline with line segments only', () => {
        ...
        const reversed = reversePolyline(polylineShape.geometry as DxfPolyline);
    ...
    it('should handle empty polyline segments', () => {
        const emptyPolyline: DxfPolyline = { closed: false, shapes: [] };
    ...
    it('should preserve start and end points when reversed twice', () => {
        ...
        const original = polylineShape.geometry as DxfPolyline;
