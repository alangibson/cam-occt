...
export function findRayArcIntersections(
  ...
  for (const intersection of circleIntersections) {
    ...
    if (isAngleInArcRange(angle, startAngle, endAngle, clockwise)) {
      ...
      if (approxEqual(angle, startAngle, epsilon) || 
      ...
      const dotProduct = Math.abs(direction.x * radiusVec.x + direction.y * radiusVec.y);
      ...
      arcIntersections.push({
        ...intersection,
        type,
        shapeParameter: normalizeArcParameter(angle, startAngle, endAngle, clockwise)
      });
    }
  }
...
export function countHorizontalRayArcCrossings(
  ...
  const intersection = setupHorizontalRayIntersection(rayOrigin, center, radius, config);
