...
import type { Polyline, Shape, Line, Arc, Point2D } from '../../types/geometry';
...
describe('Ray-Polyline Intersection', () => {
  ...
  function createArcShape(id: string, center: Point2D, radius: number, startAngle: number, endAngle: number, clockwise: boolean = false): Shape {
    return {
      id,
      type: 'arc',
  ...
  describe('countRayPolylineCrossings', () => {
    it('should count crossings for simple rectangular polyline', () => {
      ...
      expect(countRayPolylineCrossings(horizontalRay, rectPolyline)).toBe(2);
    });
    ...
    it('should handle polyline with mixed line and arc segments', () => {
      ...
      const mixedPolyline: Polyline = {
        ...
        shapes: [
          ...
          createArcShape('2', { x: 0, y: 1 }, 1, Math.PI * 1.5, Math.PI * 0.5, false), // right semicircle
  ...
  describe('findRayPolylineIntersections', () => {
    it('should find all intersection points for rectangular polyline', () => {
      ...
      const intersections = findRayPolylineIntersections(ray, rectPolyline);
    ...
    it('should return empty array for no intersections', () => {
      const polyline: Polyline = {
        closed: false,
        shapes: [
          createLineShape('1', { x: 5, y: 5 }, { x: 10, y: 5 })
        ]
      };
  ...
  describe('countHorizontalRayPolylineCrossings', () => {
    it('should count crossings for horizontal ray from point', () => {
      ...
      expect(countHorizontalRayPolylineCrossings(insidePoint, triangle)).toBe(1);

      // Point outside triangle
      const outsidePoint = { x: -1, y: 0.5 };
      expect(countHorizontalRayPolylineCrossings(outsidePoint, triangle)).toBe(2);

      // Point far outside
      const farPoint = { x: -10, y: 10 };
      expect(countHorizontalRayPolylineCrossings(farPoint, triangle)).toBe(0);
    });
  });
  ...
  describe('Edge Cases', () => {
    ...
    it('should handle polyline with collinear segments', () => {
      ...
      const intersections = findRayPolylineIntersections(ray, collinearPolyline);
    ...
    it('should warn about unsupported shape types in polylines', () => {
      ...
      expect(consoleSpy).toHaveBeenCalledWith('Unsupported shape type in polyline: circle');
