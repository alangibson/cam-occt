...
export function countRayEllipseCrossings(
  ...
  const intersections: RayIntersection[] = findRayEllipseIntersections(ray, ellipse, config);
  return intersections.filter(intersection => intersection.t >= -config.epsilon).length;
}
...
export function findRayEllipseIntersections(
  ...
  const rotation: number = Math.atan2(ellipse.majorAxisEndpoint.y, ellipse.majorAxisEndpoint.x);
  
  // Transform ray to ellipse's local coordinate system
  const localRay: Ray = transformRayToEllipseSpace(ray, ellipse.center, rotation);
  
  // Solve intersection in standard ellipse form: x²/a² + y²/b² = 1
  const a: number = majorAxisLength;
  const b: number = minorAxisLength;
  
  const intersections: RayIntersection[] = solveRayEllipseIntersection(localRay, a, b, epsilon);
  
  // Transform intersection points back to world coordinates
  const worldIntersections: RayIntersection[] = intersections.map(intersection => {
  ...
  if (ellipse.startParam !== undefined && ellipse.endParam !== undefined) {
    return filterEllipseArcIntersections(worldIntersections, ellipse, epsilon);
  }
...
function filterEllipseArcIntersections(
  ...
  return intersections.filter(intersection => {
    ...
    const relativeAngle: number = normalizeAngle(pointAngle - majorAxisAngle);
    ...
    if (startParam <= endParam) {
      return relativeAngle >= startParam - epsilon && relativeAngle <= endParam + epsilon;
    } else {
      return relativeAngle >= startParam - epsilon || relativeAngle <= endParam + epsilon;
    }
  });
