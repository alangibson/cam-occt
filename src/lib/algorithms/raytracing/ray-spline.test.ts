...
describe('Ray-Spline Intersection', () => {
  ...
  function createQuadraticSpline(p0: Point2D, p1: Point2D, p2: Point2D): Spline {
    return {
  ...
  function createCubicSpline(p0: Point2D, p1: Point2D, p2: Point2D, p3: Point2D): Spline {
    return {
  ...
  describe('countRaySplineCrossings', () => {
    it('should count crossings for linear spline (equivalent to line)', () => {
      const linearSpline = createLinearSpline({ x: 0, y: -1 }, { x: 0, y: 1 });
    ...
    it('should count crossings for horizontal linear spline', () => {
      const horizontalSpline = createLinearSpline({ x: -1, y: 0 }, { x: 1, y: 0 });
    ...
    it('should count 0 crossings for ray missing spline', () => {
      const spline: import("$lib/types/geometry").Spline = createLinearSpline({ x: 5, y: 5 }, { x: 10, y: 10 });
  ...
  describe('findRaySplineIntersections', () => {
    it('should find intersection for linear spline', () => {
      const linearSpline = createLinearSpline({ x: 0, y: -2 }, { x: 0, y: 2 });
    ...
    it('should return empty array for no intersections', () => {
      const spline: import("$lib/types/geometry").Spline = createLinearSpline({ x: 10, y: 10 }, { x: 15, y: 15 });
    ...
    it('should handle spline with fit points', () => {
      const splineWithFitPoints: Spline = {
        controlPoints: [{ x: 0, y: 0 }, { x: 1, y: 1 }], // These may be ignored
        knots: [0, 1],
        weights: [1, 1],
        degree: 1,
        fitPoints: [{ x: -1, y: 0 }, { x: 1, y: 0 }], // Horizontal line through origin
        closed: false
      };
      ...
      const intersections = findRaySplineIntersections(ray, splineWithFitPoints);
      expect(intersections.length).toBeGreaterThan(0);
    });
  });
  ...
  describe('countHorizontalRaySplineCrossings', () => {
    it('should count crossings for horizontal ray', () => {
      const verticalSpline = createLinearSpline({ x: 2, y: -1 }, { x: 2, y: 1 });
      
      // Point to left of spline
      const leftPoint = { x: 0, y: 0 };
      expect(countHorizontalRaySplineCrossings(leftPoint, verticalSpline)).toBe(1);

      // Point to right of spline
      const rightPoint = { x: 4, y: 0 };
      expect(countHorizontalRaySplineCrossings(rightPoint, verticalSpline)).toBe(0);

      // Point above spline
      const abovePoint = { x: 0, y: 2 };
      expect(countHorizontalRaySplineCrossings(abovePoint, verticalSpline)).toBe(0);
    });
  });
  ...
  describe('Edge Cases', () => {
    it('should handle very small splines', () => {
      const tinySpline = createLinearSpline({ x: 0, y: 0 }, { x: 0.001, y: 0.001 });
    ...
    it('should handle splines with coincident control points', () => {
      const coincidentSpline: Spline = {
        controlPoints: [{ x: 0, y: 0 }, { x: 0, y: 0 }, { x: 1, y: 1 }],
        knots: [0, 0, 0, 1, 1, 1],
        weights: [1, 1, 1],
        degree: 2,
        fitPoints: [],
    ...
    it('should handle degenerate ray direction', () => {
      const spline: import("$lib/types/geometry").Spline = createLinearSpline({ x: 0, y: 0 }, { x: 1, y: 1 });
