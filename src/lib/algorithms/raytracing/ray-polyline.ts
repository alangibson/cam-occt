...
export function countRayPolylineCrossings(
  ...
  for (const shape of polyline.shapes) {
    switch (shape.type) {
      case 'line':
        totalCrossings += countRayLineCrossings(ray, shape.geometry as Line, config);
        break;
      case 'arc':
        totalCrossings += countRayArcCrossings(ray, shape.geometry as Arc, config);
        break;
      default:
        console.warn(`Unsupported shape type in polyline: ${shape.type}`);
    }
  }
...
export function findRayPolylineIntersections(
  ...
  for (const shape of polyline.shapes) {
    switch (shape.type) {
      case 'line':
        intersections.push(...findRayLineIntersections(ray, shape.geometry as Line, config));
        break;
      case 'arc':
        intersections.push(...findRayArcIntersections(ray, shape.geometry as Arc, config));
        break;
      default:
        console.warn(`Unsupported shape type in polyline: ${shape.type}`);
    }
