...
function countRaySplineCrossingsApproximate(
  ...
  const samples: Point2D[] = sampleSplinePoints(spline, config.splineSampleCount || 50);
  ...
  for (let i: number = 0; i < samples.length - 1; i++) {
    ...
    const segmentCrossing: number = countRayLineSegmentCrossing(ray, start, end, config);
...
function findRaySplineIntersectionsApproximate(
  ...
  for (let i: number = 0; i < samples.length - 1; i++) {
    ...
    const segmentIntersection: RayIntersection | null = findRayLineSegmentIntersection(ray, start, end, config);
...
function evaluateSplineAtParameter(spline: Spline, t: number): Point2D | null {
  const { controlPoints, degree }: { controlPoints: Point2D[], degree: number } = spline;
...
function countRayLineSegmentCrossing(
  ...
  const intersection: RayIntersection | null = findRayLineSegmentIntersection(ray, start, end, config);
...
function findRayLineSegmentIntersection(
  ...
  const denominator: number = direction.x * lineVec.y - direction.y * lineVec.x;
  ...
  const t: number = (toStart.x * lineVec.y - toStart.y * lineVec.x) / denominator;
  const s: number = (toStart.x * direction.y - toStart.y * direction.x) / denominator;
