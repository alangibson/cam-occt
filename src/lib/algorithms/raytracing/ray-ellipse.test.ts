...
describe('Ray-Ellipse Intersection', () => {
  ...
  function createAxisAlignedEllipse(center: Point2D, majorRadius: number, minorRadius: number): Ellipse {
    return {
      center,
  ...
  function createRotatedEllipse(center: Point2D, majorRadius: number, minorRadius: number, rotation: number): Ellipse {
    return {
      center,
      majorAxisEndpoint: { 
  ...
  describe('findRayEllipseIntersections', () => {
    ...
    it('should handle tangent case with single intersection', () => {
      ...
      const intersections = findRayEllipseIntersections(tangentRay, ellipse);
  ...
  describe('countHorizontalRayEllipseCrossings', () => {
    it('should count crossings for horizontal ray from point', () => {
      ...
      expect(countHorizontalRayEllipseCrossings(leftPoint, ellipse)).toBe(2);

      // Point inside ellipse
      const insidePoint = { x: 5, y: 3 };
      expect(countHorizontalRayEllipseCrossings(insidePoint, ellipse)).toBe(1);

      // Point above ellipse
      const abovePoint = { x: 0, y: 10 };
      expect(countHorizontalRayEllipseCrossings(abovePoint, ellipse)).toBe(0);
    });
  });
