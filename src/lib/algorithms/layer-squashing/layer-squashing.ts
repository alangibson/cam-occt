 ...
 */
export function squashLayers(
    drawing: Drawing,
    options: LayerSquashingOptions = {}
): Drawing {
    const {
        preserveLayerInfo = false,
        tolerance = DUPLICATE_FILTERING_TOLERANCE_MM,
    } = options;
    ...
    const deduplicatedShapes: Shape[] = removeDuplicateShapes(
        allShapes,
        tolerance
    );
 ...
 */
function removeDuplicateShapes(
    shapes: Array<Shape & { sourceLayer?: string }>,
    tolerance: number
): Shape[] {
 ...
 */
function areShapesGeometricallyEqual(
    shape1: Shape,
    shape2: Shape,
    tolerance: number
): boolean {
...
function areLinesEqual(line1: Line, line2: Line, tolerance: number): boolean {
    ...
    return (
        (startDistance1 < tolerance && endDistance1 < tolerance) ||
...
function arePolylinesEqual(
    ...
    if (vertices1.length > 0 && vertices2.length > 0) {
        ...
        for (let i: number = 0; i < vertices1.length; i++) {
            ...
            const bulgeDiff: number = Math.abs(
                (v1.bulge || 0) - (v2.bulge || 0)
            );
 ...
 */
export function validateSquashing(
    originalDrawing: Drawing,
    squashedDrawing: Drawing
): {
    success: boolean;
    ...
    const originalStats: {
        ...
        layerCounts: Record<string, number>;
        layerNames: string[];
    } = getLayerStatistics(originalDrawing);
