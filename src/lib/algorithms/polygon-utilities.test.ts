...
describe('Polygon Utilities', () => {
  ...
  describe('analyzePolygon', () => {
    ...
    it('should throw error for insufficient points', () => {
      expect(() => analyzePolygon([{ x: 0, y: 0 }, { x: 1, y: 0 }], defaultConfig))
  ...
  describe('classifyPolygonsAsShellsAndHoles', () => {
    it('should classify simple shell with hole correctly', () => {
      ...
      const result = classifyPolygonsAsShellsAndHoles([outerSquare, innerSquare], defaultConfig);
    ...
    it('should handle multiple shells with holes', () => {
      ...
      const shell2: Point2D[] = [
        { x: 3, y: 0 }, { x: 3, y: 2 }, { x: 5, y: 2 }, { x: 5, y: 0 }
      ]; // CW shell
      ...
      const hole2: Point2D[] = [
        { x: 3.5, y: 0.5 }, { x: 4.5, y: 0.5 }, { x: 4.5, y: 1.5 }, { x: 3.5, y: 1.5 }
      ]; // CCW hole in shell2

      const result = classifyPolygonsAsShellsAndHoles([shell1, shell2, hole1, hole2], defaultConfig);
    ...
    it('should identify orphaned holes', () => {
      ...
      const result = classifyPolygonsAsShellsAndHoles([shell, orphanedHole], defaultConfig);
  ...
  describe('isPointInsidePolygon', () => {
    ...
    it('should detect point outside square', () => {
      ...
      expect(isPointInsidePolygon(outsidePoint, unitSquareCW)).toBe(false);
    });
  ...
  describe('isPolygonInsidePolygon', () => {
    it('should detect inner polygon completely inside outer polygon', () => {
      ...
      expect(isPolygonInsidePolygon(innerPolygon, outerPolygon, defaultConfig.tolerance)).toBe(true);
    });
    ...
    it('should detect overlapping polygons', () => {
      ...
      expect(isPolygonInsidePolygon(polygon2, polygon1, defaultConfig.tolerance)).toBe(false);
    });
  ...
  describe('doLineSegmentsIntersect', () => {
    it('should detect intersecting segments', () => {
      ...
      expect(doLineSegmentsIntersect(p1, p2, p3, p4, defaultConfig.tolerance)).toBe(true);
    });
    ...
    it('should detect non-intersecting segments', () => {
      ...
      expect(doLineSegmentsIntersect(p1, p2, p3, p4, defaultConfig.tolerance)).toBe(false);
    });
  ...
  describe('createRegularPolygon', () => {
    it('should create equilateral triangle', () => {
      ...
      for (const point of triangle) {
        const distance: number = Math.sqrt(point.x * point.x + point.y * point.y);
    ...
    it('should throw error for invalid parameters', () => {
      expect(() => createRegularPolygon({ x: 0, y: 0 }, 1, 2))
        .toThrow('Regular polygon must have at least 3 sides');
      
      expect(() => createRegularPolygon({ x: 0, y: 0 }, 0, 3))
        .toThrow('Regular polygon radius must be positive');
    });
  });
  ...
  describe('calculateConvexHull', () => {
    it('should calculate convex hull of square with interior point', () => {
      ...
      expect(hull.find(p => p.x === 0.5 && p.y === 0.5)).toBeUndefined();
    });
  ...
  describe('Real-world CAD scenarios', () => {
    ...
    it('should classify nested parts with holes correctly', () => {
      ...
      const bolt1: Point2D[] = [
        { x: 25, y: 25 }, { x: 35, y: 25 }, { x: 35, y: 35 }, { x: 25, y: 35 }
      ]; // CCW bolt hole

      const bolt2: Point2D[] = [
        { x: 65, y: 65 }, { x: 75, y: 65 }, { x: 75, y: 75 }, { x: 65, y: 75 }
      ]; // CCW bolt hole
    ...
    it('should handle complex nesting scenarios', () => {
      ...
      const part1: Point2D[] = [
        { x: 10, y: 10 }, { x: 10, y: 80 }, { x: 60, y: 80 }, { x: 60, y: 10 }
      ]; // CCW part 1 (hole in sheet)

      const part2: Point2D[] = [
        { x: 100, y: 50 }, { x: 100, y: 150 }, { x: 200, y: 150 }, { x: 200, y: 50 }
      ]; // CCW part 2 (hole in sheet)

      const result = classifyPolygonsAsShellsAndHoles([sheet, part1, part2], { tolerance: 1.0 });
  ...
  describe('Edge cases and error conditions', () => {
    ...
    it('should handle duplicate points gracefully', () => {
      ...
      expect(() => analyzePolygon(withDuplicates, { tolerance: 0.001 })).not.toThrow();
    });
