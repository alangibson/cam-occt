 ...
 */
export function calculateLineParameterForPoint(point: Point2D, line: Line): number {
  ...
  const dotProduct: number = (point.x - line.start.x) * dx + (point.y - line.start.y) * dy;
 ...
 */
export function isParameterValidForSegment(param: number, position: SegmentPosition): boolean {
 ...
 */
export function calculateLineIntersection(line1: Line, line2: Line): IntersectionResult[] {
  const x1: number = line1.start.x, y1: number = line1.start.y;
  const x2: number = line1.end.x, y2: number = line1.end.y;
  const x3: number = line2.start.x, y3: number = line2.start.y;
  const x4: number = line2.end.x, y4: number = line2.end.y;

  const dx1: number = x2 - x1, dy1: number = y2 - y1;
  const dx2: number = x4 - x3, dy2: number = y4 - y3;

  // Handle degenerate cases (point-line intersections)
  const line1IsPoint: boolean = Math.abs(dx1) < EPSILON && Math.abs(dy1) < EPSILON;
  const line2IsPoint: boolean = Math.abs(dx2) < EPSILON && Math.abs(dy2) < EPSILON;
  ...
  if (line1IsPoint) {
    ...
    const t2: number = Math.abs(dx2) > Math.abs(dy2) ? 
      (x1 - x3) / dx2 : 
      (y1 - y3) / dy2;
    ...
    if (Math.abs(projectedX - x1) < EPSILON && Math.abs(projectedY - y1) < EPSILON) {
      return [{
        point: { x: x1, y: y1 },
        param1: 0,
  ...
  if (line2IsPoint) {
    ...
    const t1: number = Math.abs(dx1) > Math.abs(dy1) ? 
      (x3 - x1) / dx1 : 
      (y3 - y1) / dy1;
    ...
    if (Math.abs(projectedX - x3) < EPSILON && Math.abs(projectedY - y3) < EPSILON) {
      return [{
        point: { x: x3, y: y3 },
  ...
  const t1: number = ((x1 - x3) * (y3 - y4) - (y1 - y3) * (x3 - x4)) / denominator;
  const t2: number = ((x1 - x3) * (y1 - y2) - (y1 - y3) * (x1 - x2)) / denominator;
