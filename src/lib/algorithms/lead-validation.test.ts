...
describe('Lead Validation Pipeline', () => {
  ...
  function createLineChain(start: { x: number; y: number }, end: { x: number; y: number }, chainId = 'chain1'): ShapeChain {
    const shape: Shape = {
      id: 'line1',
      type: 'line',
      geometry: { start, end },
  ...
  function createCircleChain(center: { x: number; y: number }, radius: number, chainId = 'chain1'): ShapeChain {
    const shape: Shape = {
      id: 'circle1',
      type: 'circle',
  ...
  describe('basic configuration validation', () => {
    ...
    it('should reject negative lead lengths', () => {
      ...
      expect(result.suggestions).toContain('Set lead-in length to 0 or a positive value');
    });
    ...
    it('should warn about type/length mismatch', () => {
      ...
      expect(result.warnings).toContain('Lead-in type is "none" but length is greater than 0');
    });
    ...
    it('should reject invalid angles', () => {
      ...
      expect(result.isValid).toBe(false);
      expect(result.severity).toBe('error');
      expect(result.warnings).toContain('Lead-in angle must be between 0 and 359 degrees');
    });
  });
  ...
  describe('chain geometry validation', () => {
    ...
    it('should warn about very long leads on small chains', () => {
      ...
      expect(result.isValid).toBe(true);
      expect(result.severity).toBe('warning');
      expect(result.warnings.some(w => w.includes('Lead-in length is very large compared to chain size'))).toBe(true);
    });
    ...
    it('should warn about long leads on very small geometry', () => {
      ...
      expect(result.warnings.some(w => w.includes('Chain is very small but leads are long'))).toBe(true);
    });
  });

  describe('part context validation', () => {
    it('should provide info for hole chains', () => {
      const holeChain = createCircleChain({ x: 5, y: 5 }, 2, 'hole-chain');
      const shellChain = createCircleChain({ x: 5, y: 5 }, 10, 'shell-chain');
      ...
      expect(result.warnings.some(w => w.includes('Generating leads for hole'))).toBe(true);
    });

    it('should warn about chain not belonging to part', () => {
      const orphanChain = createCircleChain({ x: 100, y: 100 }, 5, 'orphan-chain'); // Different chain ID
      const shellChain = createCircleChain({ x: 5, y: 5 }, 10, 'shell-chain');
      ...
      expect(result.warnings).toContain('Chain is not recognized as part of the specified part');
    });
  });
  ...
  describe('lead length validation', () => {
    it('should warn about very long leads', () => {
      ...
      expect(result.warnings.some(w => w.includes('Lead-out length (75) is very long'))).toBe(true);
    });
    ...
    it('should provide info about very short leads', () => {
      ...
      expect(result.warnings.some(w => w.includes('Lead-in length (0.1) is very short'))).toBe(true);
      expect(result.warnings.some(w => w.includes('Lead-out length (0.2) is very short'))).toBe(true);
    });
  });
  ...
  describe('cut direction compatibility', () => {
    it('should suggest cut direction for closed chains', () => {
      ...
      expect(result.warnings.some(w => w.includes('Closed chain detected but cut direction is "none"'))).toBe(true);
      expect(result.suggestions?.some(s => s.includes('Consider specifying "clockwise" or "counterclockwise"'))).toBe(true);
    });
    ...
    it('should note unnecessary cut direction for open chains', () => {
      ...
      expect(result.warnings.some(w => w.includes('Cut direction specified for open chain (not necessary)'))).toBe(true);
    });
    ...
    it('should warn about manual angles overriding cut direction', () => {
      ...
      expect(result.warnings.some(w => w.includes('Manual lead-in angle specified with cut direction'))).toBe(true);
      expect(result.warnings.some(w => w.includes('Manual lead-out angle specified with cut direction'))).toBe(true);
    });
  });
