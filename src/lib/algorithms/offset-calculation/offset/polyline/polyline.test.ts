...
describe('offsetPolyline', () => {
  ...
  it('should offset open polyline', () => {
    ...
    if (result.shapes[0].type === 'polyline') {
      ...
      expect(result.shapes.every(s => s.type === 'line' || s.type === 'arc')).toBe(true);
    }
  });
  ...
  describe('Polyline offset accuracy', () => {
    ...
    it('should produce consistent offset behavior for square polygon', () => {
      ...
      const outsetResult = offsetPolyline(square, offsetDistance, 'outset');
      
      expect(insetResult.success).toBe(true);
      expect(outsetResult.success).toBe(true);
      
      const insetBounds = calculateBounds(extractPointsFromShapes(insetResult.shapes));
      const outsetBounds = calculateBounds(extractPointsFromShapes(outsetResult.shapes));
  ...
  describe('Real DXF polylines with bulges', () => {
    it('should successfully offset polylines_with_bulge.dxf polylines for both inner and outer', async () => {
      ...
      for (let i: number = 0; i < polylineShapes.length; i++) {
        ...
        console.log(`Polyline ${i}: outset produced ${outsetResult.shapes.length} shapes, inset produced ${insetResult.shapes.length} shapes`);
      }
    });
  });
});
  ...
  for (const shape of shapes) {
    switch (shape.type) {
      ...
      case 'circle':
        ...
        break;
      default:
        console.warn(`Unknown shape type for point extraction: ${shape.type}`);
    }
  }
