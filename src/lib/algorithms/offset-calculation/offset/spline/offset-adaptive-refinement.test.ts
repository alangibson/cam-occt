...
describe('Spline offset adaptive refinement', () => {
  ...
  it('should refine when tolerance is not met', () => {
    ...
    const hasRefinementWarning = result.warnings.some(w => 
      w.includes('refined with') && w.includes('samples after')
    );
  ...
  it('should report when tolerance cannot be achieved', () => {
    ...
    const result = offsetSpline(highCurvatureSpline, 10, 'outset', 0.000001, 2);
  ...
  it('should validate offset accuracy', () => {
    ...
    const result = offsetSpline(simpleSpline, offsetDistance, 'outset', 0.5);
  ...
  it('should increase sample count during refinement', () => {
    ...
    if (result.warnings.length > 0) {
      const refinementWarning = result.warnings.find(w => w.includes('refined with'));
      if (refinementWarning) {
        // Extract the number of samples from the warning
        const samplesMatch = refinementWarning.match(/refined with (\d+) samples/);
        if (samplesMatch) {
