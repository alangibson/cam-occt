...
describe('offsetSpline', () => {
  ...
  it('should handle adaptive refinement with tolerance', () => {
    ...
    const refinementWarnings = result.warnings.filter(w => 
      w.includes('refined with') || w.includes('tolerance not achieved')
    );
...
describe('splitVerbCurve', () => {
  ...
  function createTestVerbCurve(degree: number, controlPoints: [number, number, number][], knots: number[], weights?: number[]) {
  ...
  describe('segment validation', () => {
    it('should preserve curve continuity at segment boundaries', () => {
      ...
      for (let i: number = 0; i < segments.length - 1; i++) {
        ...
        const lastPoint = currentSegment.controlPoints[currentSegment.controlPoints.length - 1];
  ...
  describe('edge cases', () => {
    ...
    it('should handle curves with existing maximum knot multiplicity', () => {
      ...
      const controlPoints: [number, number, number][] = [
        [0, 0, 0], [1, 2, 0], [2, 2, 0], [3, 0, 0], [4, -1, 0], [5, 1, 0], [6, 0, 0]
      ];
  ...
  describe('error handling', () => {
    it('should reject unsupported degrees', () => {
      const controlPoints: [number, number, number][] = [
        [0, 0, 0], [1, 1, 0], [2, 1, 0], [3, 1, 0], [4, 0, 0], [5, 0, 0]
      ];
    ...
    it('should validate sufficient control points', () => {
      ...
      const controlPoints: [number, number, number][] = [
        [0, 0, 0], [1, 1, 0], [2, 1, 0], [3, 0, 0] // Valid for degree 3
      ];
