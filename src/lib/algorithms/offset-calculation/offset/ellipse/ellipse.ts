import type { Point2D, Ellipse, Spline, Shape } from '../../../../types/geometry';
 ...
 */
export function offsetEllipse(ellipse: Ellipse, distance: number, direction: OffsetDirection): OffsetResult {
  ...
  try {
    ...
    const { majorAxisLength, minorAxisLength, majorAxisAngle: rotationAngle } = getEllipseParameters(ellipse);
    ...
    const isFullEllipse: boolean = Math.abs(paramRange - 2 * Math.PI) < EPSILON;
    ...
    for (let i: number = 0; i < numPoints; i++) {
      ...
      const magnitude: number = Math.sqrt(normalX * normalX + normalY * normalY);
      ...
      // Apply rotation
      const rotatedX: number = offsetX * Math.cos(rotationAngle) - offsetY * Math.sin(rotationAngle);
      const rotatedY: number = offsetX * Math.sin(rotationAngle) + offsetY * Math.cos(rotationAngle);
    ...
    const offsetPoints3D: [number, number, number][] = points.map(p => [p.x, p.y, 0]);
    ...
    try {
      ...
      return {
        ...
        shapes: [],
        warnings: [],
        errors: [`Failed to fit NURBS curve to ellipse offset points: ${error instanceof Error ? (error as Error).message : String(error)}`]
      };
    }
    
    // Convert verb curve back to our Spline format
    const controlPoints2D: Point2D[] = offsetCurve.controlPoints().map(p => ({
      x: p[0],
      y: p[1]
    }));
    ...
    if (knots.length !== expectedKnots) {
      console.warn(`Ellipse offset: Knot vector mismatch: got ${knots.length}, expected ${expectedKnots}`);
      console.warn(`Control points: ${controlPoints2D.length}, degree: ${curveDegree}`);
      
      // Generate a valid uniform knot vector
      const validKnots = generateUniformKnotVector(controlPoints2D.length, curveDegree);
      
      console.warn(`Using generated uniform knot vector of length ${validKnots.length}`);
      ...
      if (!needsRepair) {
        for (let i: number = knots.length - curveDegree - 1; i < knots.length; i++) {
      ...
      if (needsRepair) {
        console.warn('Ellipse offset: Knot vector structure invalid, generating uniform knot vector');
        
        // Generate a valid uniform knot vector
        validatedKnots = generateUniformKnotVector(controlPoints2D.length, curveDegree);
      }
    }
    ...
    return {
      success: true,
      shapes: [offsetShape],
      warnings: ['Ellipse offset calculated using true mathematical offset and fitted to NURBS curve'],
      errors: []
    };
  } catch (error) {
    return {
      ...
      shapes: [],
      warnings: [],
      errors: [`Failed to offset ellipse: ${error instanceof Error ? (error as Error).message : String(error)}`]
    };
  }
}