...
describe('Ellipse Offset Edge Cases and Error Handling', () => {
  ...
  describe('Edge Cases', () => {
    it('should handle ellipse with zero major axis length', async () => {
      const { getEllipseParameters } = await import('../../../../utils/ellipse-utils');
    ...
    it('should handle degenerate ellipse (both axes zero)', async () => {
      ...
      expect(result.success).toBe(false);
      expect(result.errors.length).toBeGreaterThan(0);
      expect(result.errors[0]).toContain('Insufficient points generated for ellipse offset');
    });
    ...
    it('should handle ellipse arc with reversed parameter range', async () => {
      ...
      const mockCurve = {
        controlPoints: vi.fn(() => [[1, 2, 0], [3, 4, 0], [5, 6, 0]]),
        degree: vi.fn(() => 2),
        knots: vi.fn(() => [0, 0, 0, 1, 1, 1]),
        weights: vi.fn(() => [1, 1, 1]),
        asNurbs: vi.fn(() => ({ degree: 2, knots: [0, 0, 0, 1, 1, 1], controlPoints: [[1, 2, 0], [3, 4, 0], [5, 6, 0]] })),
        clone: vi.fn(),
        ...
        derivatives: vi.fn(() => [[0, 0, 0], [1, 0, 0]]),
        derivativesAsync: vi.fn(() => Promise.resolve([[0, 0, 0], [1, 0, 0]])),
        closestPoint: vi.fn(() => ({ u: 0, pt: [0, 0, 0], d: 0 })),
        closestPointAsync: vi.fn(() => Promise.resolve({ u: 0, pt: [0, 0, 0], d: 0 })),
        ...
        divideByEqualArcLengthAsync: vi.fn(() => Promise.resolve([[0, 0, 0]])),
        divideByArcLength: vi.fn(() => [[0, 0, 0]]),
        divideByArcLengthAsync: vi.fn(() => Promise.resolve([[0, 0, 0]])),
        split: vi.fn(() => []),
    ...
    it('should handle extremely large offset distance', async () => {
      ...
      const mockCurve = {
        controlPoints: vi.fn(() => [[1000, 2000, 0], [3000, 4000, 0]]),
        degree: vi.fn(() => 1),
        knots: vi.fn(() => [0, 0, 1, 1]),
        weights: vi.fn(() => [1, 1]),
        asNurbs: vi.fn(() => ({ degree: 1, knots: [0, 0, 1, 1], controlPoints: [[1000, 2000, 0], [3000, 4000, 0]] })),
        clone: vi.fn(),
        domain: vi.fn(() => ({ min: 0, max: 1 })),
        transform: vi.fn(),
        transformAsync: vi.fn(),
      ...
      expect(result.warnings).toContain('Ellipse offset calculated using true mathematical offset and fitted to NURBS curve');
    });
  ...
  describe('Error Handling', () => {
    it('should handle verb-nurbs curve fitting failure', async () => {
      ...
      expect(result.success).toBe(false);
      expect(result.errors).toContain('Failed to fit NURBS curve to ellipse offset points: Curve fitting failed');
    });
    ...
    it('should handle verb-nurbs curve fitting failure with non-Error exception', async () => {
      ...
      expect(result.success).toBe(false);
      expect(result.errors).toContain('Failed to fit NURBS curve to ellipse offset points: String error in curve fitting');
    });
    ...
    it('should handle getEllipseParameters throwing an error', async () => {
      ...
      expect(result.success).toBe(false);
      expect(result.errors).toContain('Failed to offset ellipse: Invalid ellipse parameters');
    });
    ...
    it('should handle getEllipseParameters throwing non-Error exception', async () => {
      ...
      expect(result.success).toBe(false);
      expect(result.errors).toContain('Failed to offset ellipse: Parameter extraction failed');
    });

    it('should handle invalid knot vector and repair it', async () => {
      const { getEllipseParameters } = await import('../../../../utils/ellipse-utils');
      const { generateUniformKnotVector } = await import('../../../../utils/nurbs-utils');
      const { default: verb } = await import('verb-nurbs');
      ...
      const mockCurve = {
        controlPoints: vi.fn(() => [[1, 2, 0], [3, 4, 0], [5, 6, 0], [7, 8, 0]]),
        degree: vi.fn(() => 3),
        knots: vi.fn(() => [0, 0.5, 1]), // Wrong length - should be 8 for 4 control points + degree 3
        weights: vi.fn(() => [1, 1, 1, 1]),
        asNurbs: vi.fn(() => ({ degree: 3, knots: [0, 0.5, 1], controlPoints: [[1, 2, 0], [3, 4, 0], [5, 6, 0], [7, 8, 0]] })),
        clone: vi.fn(),
        domain: vi.fn(() => ({ min: 0, max: 1 })),
        transform: vi.fn(),
        transformAsync: vi.fn(),
    ...
    it('should handle malformed knot vector structure and repair it', async () => {
      ...
      const mockCurve = {
        ...
        weights: vi.fn(() => [1, 1, 1, 1]),
        asNurbs: vi.fn(() => ({ degree: 3, knots: [0, 0.1, 0.2, 0.3, 0.7, 0.8, 0.9, 1], controlPoints: [[1, 2, 0], [3, 4, 0], [5, 6, 0], [7, 8, 0]] })),
        clone: vi.fn(),
        domain: vi.fn(() => ({ min: 0, max: 1 })),
        transform: vi.fn(),
        transformAsync: vi.fn(),
    ...
    it('should preserve valid knot vector structure', async () => {
      const { getEllipseParameters } = await import('../../../../utils/ellipse-utils');
      const { generateUniformKnotVector: _generateUniformKnotVector } = await import('../../../../utils/nurbs-utils');
      const { default: verb } = await import('verb-nurbs');
      ...
      const mockCurve = {
        ...
        weights: vi.fn(() => [1, 1, 1, 1]),
        asNurbs: vi.fn(() => ({ degree: 3, knots: validKnots, controlPoints: [[1, 2, 0], [3, 4, 0], [5, 6, 0], [7, 8, 0]] })),
        clone: vi.fn(),
        domain: vi.fn(() => ({ min: 0, max: 1 })),
        transform: vi.fn(),
        transformAsync: vi.fn(),
  ...
  describe('Spline Properties', () => {
    ...
    it('should create spline with proper properties', async () => {
      ...
      const controlPoints = [[1, 2, 0], [3, 4, 0], [5, 6, 0], [7, 8, 0]];
      const degree = 3;
    ...
    it('should generate reasonable shape id', async () => {
      ...
      const mockCurve = {
        controlPoints: vi.fn(() => [[1, 2, 0], [3, 4, 0]]),
        degree: vi.fn(() => 1),
        knots: vi.fn(() => [0, 0, 1, 1]),
        weights: vi.fn(() => [1, 1]),
        asNurbs: vi.fn(() => ({ degree: 1, knots: [0, 0, 1, 1], controlPoints: [[1, 2, 0], [3, 4, 0]] })),
        clone: vi.fn(),
        domain: vi.fn(() => ({ min: 0, max: 1 })),
        transform: vi.fn(),
        transformAsync: vi.fn(),
