...
describe('offsetArc', () => {
  ...
  it('should maintain consistent perpendicular distance for all arc types', () => {
    ...
    const testConfigs = [
      { 
        name: '90-degree arc',
        arc: { center: { x: 100, y: 100 }, radius: 50, startAngle: Math.PI / 4, endAngle: 3 * Math.PI / 4, clockwise: false }
      },
      { 
        name: '180-degree semicircle',
        arc: { center: { x: 200, y: 100 }, radius: 60, startAngle: 0, endAngle: Math.PI, clockwise: false }
      },
      { 
        name: '270-degree arc',
        arc: { center: { x: 300, y: 200 }, radius: 40, startAngle: 0, endAngle: 3 * Math.PI / 2, clockwise: false }
      },
      {
        name: '120-degree symmetric arc',
        arc: { center: { x: 150, y: 150 }, radius: 80, startAngle: -Math.PI / 3, endAngle: Math.PI / 3, clockwise: false }
      }
    ];
    ...
    testConfigs.forEach(config => {
      ...
      // Test outset
      const outsetResult = offsetArc(config.arc, offsetDistance, 'outset');
      ...
      const testAngles = [config.arc.startAngle, config.arc.endAngle, (config.arc.startAngle + config.arc.endAngle) / 2];
      
      testAngles.forEach(angle => {
        // Original point
        const origX = config.arc.center.x + config.arc.radius * Math.cos(angle);
        const origY = config.arc.center.y + config.arc.radius * Math.sin(angle);
        
        // Outset point
        const outsetX = config.arc.center.x + outsetArc.radius * Math.cos(angle);
        const outsetY = config.arc.center.y + outsetArc.radius * Math.sin(angle);
        
        // Calculate distance
        const outsetDist = Math.sqrt((outsetX - origX) ** 2 + (outsetY - origY) ** 2);
        expect(outsetDist).toBeCloseTo(offsetDistance, 10);
        
        // Inset point
        const insetX = config.arc.center.x + insetArc.radius * Math.cos(angle);
        const insetY = config.arc.center.y + insetArc.radius * Math.sin(angle);
        
        // Calculate distance
        const insetDist = Math.sqrt((insetX - origX) ** 2 + (insetY - origY) ** 2);
  ...
  it('should maintain arc properties with radius offset', () => {
    ...
    expect(offsetGeometry.startAngle).toBeCloseTo(originalArc.startAngle, 10);
