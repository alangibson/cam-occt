 ...
 */
export function offsetArc(arc: Arc, distance: number, direction: OffsetDirection): OffsetResult {
  ...
  try {
    ...
    if (newRadius <= 0) {
      return {
        ...
        shapes: [],
        warnings: [],
        errors: [`Arc offset would result in negative radius: ${newRadius}`]
      };
    }
    ...
    return {
      ...
      shapes: [],
      warnings: [],
      errors: [`Failed to offset arc: ${error instanceof Error ? (error as Error).message : String(error)}`]
    };
  }
}