...
describe('offsetCircle', () => {
  ...
  it('should create concentric circles with exact radius difference', () => {
    ...
    const outsetResult = offsetCircle(originalCircle, offsetDistance, 'outset');
    const insetResult = offsetCircle(originalCircle, offsetDistance, 'inset');
    ...
    expect(outsetGeometry.center.x).toBeCloseTo(originalCircle.center.x, 10);
    expect(outsetGeometry.center.y).toBeCloseTo(originalCircle.center.y, 10);
