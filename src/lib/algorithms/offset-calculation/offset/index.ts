...
import type { Shape, Line, Arc, Circle, Polyline, Spline, Ellipse } from '../../../types/geometry';
 ...
 */
export function offsetShape(shape: Shape, distance: number, direction: OffsetDirection): OffsetResult {
  switch (shape.type) {
    case 'line':
    ...
    case 'polyline':
      return offsetPolyline(shape.geometry as Polyline, distance, direction);
    case 'spline':
      return offsetSpline(shape.geometry as Spline, distance, direction);
    case 'ellipse':
      return offsetEllipse(shape.geometry as Ellipse, distance, direction);
    default:
      return {
        success: false,
        shapes: [],
