 ...
 */
function createIntersectionParams(
    param1: number,
    param2: number,
    swapParams: boolean
...
export function findLineLineIntersections(
    ...
    const originalResults: IntersectionResult[] = findLineLineIntersectionsCore(
        line1,
        line2,
        swapParams
    );
    ...
    if (intersectionType === 'true') {
        ...
        filteredResults = originalResults.filter((result) => {
            const isWithinLine1: boolean =
                result.param1 >= 0 && result.param1 <= 1;
            const isWithinLine2: boolean =
                result.param2 >= 0 && result.param2 <= 1;
    ...
    const validOriginalResults: IntersectionResult[] = originalResults.filter(
        (result) => {
            const isWithinLine1: boolean =
                result.param1 >= 0 && result.param1 <= 1;
            const isWithinLine2: boolean =
    ...
    const extensionResults: IntersectionResult[] = originalResults
        .filter((result) => {
            const isWithinLine1: boolean =
                result.param1 >= 0 && result.param1 <= 1;
            const isWithinLine2: boolean =
            ...
            const withinExtensionLimit: boolean =
                line1Extension <= extensionLength &&
                line2Extension <= extensionLength;
...
function findLineLineIntersectionsCore(
    ...
    return results.map((result) => {
        const params = createIntersectionParams(
            result.param1,
            result.param2,
            swapParams
        );
...
function findCollinearOverlap(
    ...
    const proj2Start: number =
        ((line2.start.x - line1.start.x) * dx +
            (line2.start.y - line1.start.y) * dy) /
        lineLength;
    const proj2End: number =
        ((line2.end.x - line1.start.x) * dx +
            (line2.end.y - line1.start.y) * dy) /
        lineLength;

    // Find overlap region
    const overlapStart: number = Math.max(
        proj1Start,
        Math.min(proj2Start, proj2End)
    );
    const overlapEnd: number = Math.min(
        proj1End,
        Math.max(proj2Start, proj2End)
    );
    ...
    if (overlapStart <= overlapEnd + EPSILON) {
        ...
        if (overlapStart >= -EPSILON && overlapStart <= lineLength + EPSILON) {
            ...
            const params = createIntersectionParams(
                overlapStart / lineLength,
                calculateLineParameterForPoint(point, line2),
                swapParams
            );
            results.push({
                point,
...
export function findLineLineIntersectionsSegmentAware(
    ...
    return results.filter((result) => {
        ...
        const t1Valid = isParameterValidForSegment(
            result.param1,
            segment1Position
        );
        const t2Valid = isParameterValidForSegment(
            result.param2,
            segment2Position
        );
