...
describe('Line-Spline Gap Intersection Tests', () => {
  ...
  it('should find no intersections without extensions', () => {
    const result = findSplineLineIntersectionsVerb(testSpline, testLine, false, false);
    expect(result).toHaveLength(0);
  });

  it('should find gap intersections with extensions enabled', () => {
    const result = findSplineLineIntersectionsVerb(testSpline, testLine, false, true);
  ...
  it('should find normal intersections for intersecting line and spline', () => {
    ...
    const resultWithoutExtensions = findSplineLineIntersectionsVerb(testSpline, intersectingLine, false, false);
    const resultWithExtensions = findSplineLineIntersectionsVerb(testSpline, intersectingLine, false, true);
    ...
    const normalIntersections = resultWithExtensions.filter(int => !int.onExtension);
  ...
  it('should handle various spline configurations', () => {
    ...
    const result = findSplineLineIntersectionsVerb(cubicSpline, testLineForCubic, false, true);
  ...
  it('should handle parameter swapping correctly', () => {
    const result1 = findSplineLineIntersectionsVerb(testSpline, testLine, false, true);
    const result2 = findSplineLineIntersectionsVerb(testSpline, testLine, true, true);
  ...
  it('should handle edge cases gracefully', () => {
    ...
    // Should not crash
    const result = findSplineLineIntersectionsVerb(smallSpline, testLine, false, true);
