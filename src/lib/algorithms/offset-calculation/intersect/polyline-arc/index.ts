...
export function findPolylineArcIntersections(
  ...
  const originalResults: IntersectionResult[] = findPolylineArcIntersectionsCore(polyline, arc, swapParams);
  ...
  try {
    ...
    const extendedPolylineResults: IntersectionResult[] = findPolylineArcIntersectionsCore(
      extendedPolyline, 
      arc, 
      swapParams, 
      true, 
  ...
  try {
    ...
    const extendedArcResults: IntersectionResult[] = findPolylineArcIntersectionsCore(
      polyline, 
      extendedArc, 
      swapParams, 
      true, 
  ...
  try {
    const extendedPolyline: Polyline = createExtendedPolyline(polyline, true, true, extensionLength);
    const extendedArc: Arc = createExtendedArc(arc, extensionLength);
    const bothExtendedResults: IntersectionResult[] = findPolylineArcIntersectionsCore(
      extendedPolyline, 
      extendedArc, 
      swapParams, 
      true, 
...
function findPolylineArcIntersectionsCore(
  ...
  const segments: (Line | Arc)[] = polyline.shapes?.map(shape => shape.geometry as Line | Arc) || [];
  ...
  segments.forEach((segment, segmentIndex) => {
    ...
    if (isLine(segment)) {
      if (checkExtensions) {
        ...
        segmentIntersections = findLineArcIntersections(
          ...
          return intersection.param1 >= -EPSILON && intersection.param1 <= 1 + EPSILON;
        });
      }
    ...
    segmentIntersections.forEach(intersection => {
      ...
      const polylineParam: number = calculatePolylineParameter(segmentIndex, intersection.param1, segments.length);
 ...
 */
function calculatePolylineParameter(segmentIndex: number, segmentParam: number, totalSegments: number): number {
...
function isIntersectionOnExtension(
  ...
  if (originalPolyline && currentPolyline && currentPolyline !== originalPolyline) {
    ...
    const originalSegments: (Line | Arc)[] = originalPolyline.shapes.map(shape => shape.geometry as Line | Arc);
    const extendedSegments: (Line | Arc)[] = currentPolyline.shapes.map(shape => shape.geometry as Line | Arc);
    
    // If we added segments at start/end, the original polyline is in the middle
    const addedAtStart: number = extendedSegments.length > originalSegments.length ? 1 : 0;
    const originalStart: number = addedAtStart / extendedSegments.length;
    const originalEnd: number = (addedAtStart + originalSegments.length) / extendedSegments.length;
    
    const onPolylineExtension: boolean = polylineParam < (originalStart - tolerance) || polylineParam > (originalEnd + tolerance);
