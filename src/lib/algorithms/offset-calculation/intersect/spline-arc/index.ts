...
export function findSplineArcIntersectionsVerb(
  ...
  const originalResults: IntersectionResult[] = findSplineArcIntersectionsCore(spline, arc, swapParams);
  ...
  try {
    ...
    const intersections = verb.geom.Intersect.curves(extendedSplineCurve, originalArcCurve, INTERSECTION_TOLERANCE);
    const results = processVerbIntersectionResults(intersections, swapParams, true);
  ...
  try {
    ...
    const intersections = verb.geom.Intersect.curves(originalSplineCurve, extendedArcCurve, INTERSECTION_TOLERANCE);
    const results = processVerbIntersectionResults(intersections, swapParams, true);
    allExtensionResults.push(...results);
  } catch {
  ...
  try {
    ...
    const extendedArc = createExtendedArc(arc, extensionLength);
    const extendedArcCurve = createVerbCurveFromArc(extendedArc);
    const intersections = verb.geom.Intersect.curves(extendedSplineCurve, extendedArcCurve, INTERSECTION_TOLERANCE);
    const results = processVerbIntersectionResults(intersections, swapParams, true);
    allExtensionResults.push(...results);
  } catch {
...
function findSplineArcIntersectionsCore(
  ...
  return processSplineWithCurveIntersection(spline, arcCurve, swapParams, false);
}