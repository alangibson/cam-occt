...
import { 
  findLineLineIntersectionsSegmentAware
} from '../line-line/index';
...
export function findPolylinePolylineIntersections(
  ...
  const segments1: (Line | Arc)[] = polyline1.shapes.map(shape => shape.geometry as Line | Arc);
  const segments2: (Line | Arc)[] = polyline2.shapes.map(shape => shape.geometry as Line | Arc);
  ...
  segments1.forEach((segment1, index1) => {
    segments2.forEach((segment2, index2) => {
      ...
      const segment1Position: SegmentPosition = getSegmentPosition(index1, segments1.length);
      const segment2Position: SegmentPosition = getSegmentPosition(index2, segments2.length);
      ...
      if (isLine(segment1) && isLine(segment2)) {
        ...
        segmentIntersections = findIntersectionsByType(shape1, shape2, false, 1000, intersectionType);
      }
      
      segmentIntersections.forEach(intersection => {
        const param1: number = calculatePolylineParameter(index1, intersection.param1, segments1.length);
        const param2: number = calculatePolylineParameter(index2, intersection.param2, segments2.length);
 ...
 */
function getSegmentPosition(index: number, totalSegments: number): SegmentPosition {
  if (totalSegments === 1) {
    return 'only';
  } else if (index === 0) {
    return 'first';
