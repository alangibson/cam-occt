...
import { isPointOnArc, calculateArcParameter } from '../../../intersection-arc-utils';
import { createExtendedArc, isIntersectionOnArcExtension } from '../../extend/arc';
...
export function findArcArcIntersections(
  ...
  const originalResults: IntersectionResult[] = findArcArcIntersectionsCore(arc1, arc2, swapParams);
  ...
  try {
    ...
    const extendedArc1Results: IntersectionResult[] = findArcArcIntersectionsCore(extendedArc1, arc2, swapParams, true, arc1, undefined);
  ...
  try {
    ...
    const extendedArc2Results: IntersectionResult[] = findArcArcIntersectionsCore(arc1, extendedArc2, swapParams, true, undefined, arc2);
  ...
  try {
    const extendedArc1: Arc = createExtendedArc(arc1, extensionLength);
    const extendedArc2: Arc = createExtendedArc(arc2, extensionLength);
    const bothExtendedResults: IntersectionResult[] = findArcArcIntersectionsCore(extendedArc1, extendedArc2, swapParams, true, arc1, arc2);
...
function findArcArcIntersectionsCore(
  ...
  if (Math.abs(d - (r1 + r2)) < EPSILON || Math.abs(d - Math.abs(r1 - r2)) < EPSILON) {
    ...
    if (isPointOnArc(tangentPoint, arc1) && isPointOnArc(tangentPoint, arc2)) {
      const param1 = calculateArcParameter(tangentPoint, arc1);
      const param2 = calculateArcParameter(tangentPoint, arc2);
      
      return [createIntersectionResult(
        tangentPoint, param1, param2, swapParams, 'tangent', 
        checkExtensions, originalArc1, originalArc2
      )];
    }
    return [];
  }
  ...
  const offset1: { x: number; y: number } = { x: -h * dy / d, y: h * dx / d };
  const offset2: { x: number; y: number } = { x: h * dy / d, y: -h * dx / d };
  ...
  [offset1, offset2].forEach(offset => {
    ...
    if (isPointOnArc(point, arc1) && isPointOnArc(point, arc2)) {
      ...
      results.push(createIntersectionResult(
        point, param1, param2, swapParams, 'exact',
        checkExtensions, originalArc1, originalArc2
      ));
    }
  });
...
function createIntersectionResult(
  ...
  if (checkExtensions && (originalArc1 || originalArc2)) {
    onExtension = isIntersectionOnArcExtension(param1, param2, originalArc1, originalArc2);
  }
 ...
 */
function calculateTangentPoint(c1: Point2D, c2: Point2D, r1: number, d: number, dx: number, dy: number): Point2D {
  const t = r1 / d;
  return {
    x: c1.x + t * dx,
    y: c1.y + t * dy
