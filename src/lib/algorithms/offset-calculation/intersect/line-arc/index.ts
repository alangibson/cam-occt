...
import { calculateArcParameter, isPointOnArc } from '../../../intersection-arc-utils';
...
function processLineArcIntersection(
  ...
  const { discriminant, a, b, sqrtDisc } = calculateLineArcIntersection(line, arc);
  ...
  if (discriminant < EPSILON) {
    ...
    const tangentResults = checkExtensions ?
      processArcIntersectionResults(
        line, arc, [t], swapParams, undefined, checkExtensions, 
        originalLine, originalArc, extensionLength
      ) :
      processArcIntersectionResults(
        line, arc, [t], swapParams, segmentPosition
      );
    ...
    const intersectionResults = checkExtensions ?
      processArcIntersectionResults(
        line, arc, [t1, t2], swapParams, undefined, checkExtensions,
        originalLine, originalArc, extensionLength
      ) :
      processArcIntersectionResults(
        line, arc, [t1, t2], swapParams, segmentPosition
      );
  ...
  tValues.forEach(t => {
    ...
    if (isPointOnArc(point, arc)) {
      ...
      if (checkExtensions && (originalLine || originalArc)) {
        onExtension = isIntersectionOnExtension(t, arcParam, originalLine, originalArc, extensionLength, line, arc, point);
      }
...
export function findLineArcIntersections(
  ...
  const originalResults: IntersectionResult[] = findLineArcIntersectionsCore(line, arc, swapParams);
  ...
  try {
    ...
    const extendedLineResults: IntersectionResult[] = findLineArcIntersectionsCore(extendedLine, arc, swapParams, true, extensionLength, line);
  ...
  try {
    ...
    const extendedArcResults: IntersectionResult[] = findLineArcIntersectionsCore(line, extendedArc, swapParams, true, extensionLength, undefined, arc);
  ...
  try {
    const extendedLine: Line = createExtendedLine(line, extensionLength);
    const extendedArc: Arc = createExtendedArc(arc, extensionLength);
    const bothExtendedResults: IntersectionResult[] = findLineArcIntersectionsCore(extendedLine, extendedArc, swapParams, true, extensionLength, line, arc);
...
function findLineArcIntersectionsCore(
  ...
  return processLineArcIntersection(
    line, arc, swapParams, undefined, checkExtensions,
    originalLine, originalArc, extensionLength
  );
}
...
function isIntersectionOnExtension(
  ...
  if (originalLine && currentLine && currentLine !== originalLine) {
    ...
    const totalExtendedLength: number = 2 * extensionLength + originalLineLength;
    const originalLineStart: number = extensionLength / totalExtendedLength;
    const originalLineEnd: number = (extensionLength + originalLineLength) / totalExtendedLength;
    
    const onLineExtension: boolean = lineParam < (originalLineStart - tolerance) || lineParam > (originalLineEnd + tolerance);
    onExtension = onExtension || onLineExtension;
  }
  
  // Check if on arc extension by checking if intersection point is on original arc
  if (originalArc && currentArc && currentArc !== originalArc && intersectionPoint) {
    const onOriginalArc: boolean = isPointOnArc(intersectionPoint, originalArc);
 ...
 */
function isParameterValidForSegment(t: number, position: SegmentPosition): boolean {
...
export function findLineArcIntersectionsSegmentAware(
  ...
  return processLineArcIntersection(
    line, arc, swapParams, segmentPosition
  );
}
