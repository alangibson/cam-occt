...
export function findShapeIntersections(
  ...
  const intersections: IntersectionResult[] = findIntersectionsByType(shape1, shape2, allowExtensions, extensionLength, intersectionType);
  const clustered: IntersectionResult[] = clusterIntersections(intersections, tolerance);
...
export function findIntersectionsByType(
  ...
  if (firstType === 'line' && secondType === 'line') {
    return findLineLineIntersections(first.geometry as Line, second.geometry as Line, swapShapes, allowExtensions, extensionLength, intersectionType);
  }

  // Line-Arc intersections
  if (firstType === 'arc' && secondType === 'line') {
    return findLineArcIntersections(second.geometry as Line, first.geometry as Arc, true, allowExtensions, extensionLength);
  }

  // Line-Circle intersections
  if (firstType === 'circle' && secondType === 'line') {
    return findLineCircleIntersections(second.geometry as Line, first.geometry as Circle, true, allowExtensions, extensionLength);
  }
  ...
  if (firstType === 'line' && secondType === 'spline') {
    return findSplineLineIntersectionsVerb(second, first, true, allowExtensions, extensionLength);
  }
  ...
  if (firstType === 'arc' && secondType === 'arc') {
    return findArcArcIntersections(first.geometry as Arc, second.geometry as Arc, false, allowExtensions, extensionLength);
  }

  // Arc-Circle intersections
  if (firstType === 'arc' && secondType === 'circle') {
    // Circle as full arc
    const circle: import("$lib/types/geometry").Circle = second.geometry as Circle;
    ...
    return findArcArcIntersections(first.geometry as Arc, circleAsArc, false, allowExtensions, extensionLength);
  }

  // Arc-Polyline intersections
  if (firstType === 'arc' && secondType === 'polyline') {
    return findPolylineArcIntersections(second.geometry as Polyline, first.geometry as Arc, true, allowExtensions, extensionLength);
  }

  // Arc-Spline intersections
  if (firstType === 'arc' && secondType === 'spline') {
    return findSplineArcIntersectionsVerb(second, first, true, allowExtensions, extensionLength);
  }
  ...
  if (firstType === 'circle' && secondType === 'spline') {
    return findSplineCircleIntersectionsVerb(second, first, true, allowExtensions, extensionLength);
  }
  ...
  if (firstType === 'polyline' && secondType === 'polyline') {
    return findPolylinePolylineIntersections(first, second, intersectionType);
  }

  // Polyline-Spline intersections
  if (firstType === 'polyline' && secondType === 'spline') {
    return findSplinePolylineIntersectionsVerb(second, first, true, allowExtensions, extensionLength);
  }
  ...
  if (firstType === 'spline' && secondType === 'spline') {
    return findSplineSplineIntersectionsVerb(first, second, swapShapes, allowExtensions, extensionLength);
  }
...
export function clusterIntersections(
  ...
  const sortedIntersections: IntersectionResult[] = [...intersections].sort((a, b) => {
  ...
  for (let i: number = 0; i < sortedIntersections.length; i++) {
    ...
    for (let j: number = i + 1; j < sortedIntersections.length; j++) {
      ...
      const dist: number = pointDistance(sortedIntersections[i].point, sortedIntersections[j].point);
      if (dist < tolerance) {
  ...
  return clusters.map(cluster => {
    ...
    const bestIntersection: IntersectionResult = cluster.reduce((best, current) =>
      current.confidence > best.confidence ? current : best
    );

    // Determine if all intersections in cluster have same type
    const allSameType: boolean = cluster.every(int => int.type === bestIntersection.type);
