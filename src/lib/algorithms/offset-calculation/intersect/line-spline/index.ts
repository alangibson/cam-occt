import type { Shape, Line, Spline, Point2D } from '../../../../../lib/types/geometry';
...
export function findSplineLineIntersectionsVerb(
  ...
  const splineValidation: { isValid: boolean, error?: string } = validateSplineForIntersection(spline);
  ...
  try {
    ...
    const originalIntersections: CurveCurveIntersection[] = verb.geom.Intersect.curves(splineCurve, lineCurve, INTERSECTION_TOLERANCE);
    ...
    if (allowExtensions) {
      ...
      const extensionMethods: (() => CurveCurveIntersection[])[] = [
        () => {
          ...
          const extendedLineCurve: verb.geom.ICurve = createVerbCurveFromLine(extendedLine);
          return verb.geom.Intersect.curves(splineCurve, extendedLineCurve, INTERSECTION_TOLERANCE);
        },
        () => {
          ...
          return verb.geom.Intersect.curves(extendedSplineCurve, lineCurve, INTERSECTION_TOLERANCE);
        },
        () => {
          // Method 3: Extended line vs extended spline
          const extendedSplineCurve: verb.geom.ICurve = createExtendedSplineVerb(spline, true, true, extensionLength);
          const extendedLineCurve: verb.geom.ICurve = createVerbCurveFromLine(extendedLine);
          return verb.geom.Intersect.curves(extendedSplineCurve, extendedLineCurve, INTERSECTION_TOLERANCE);
        }
      ];
      ...
      for (const method of extensionMethods) {
        ...
        for (let retry: number = 0; retry < 3; retry++) {
          try {
            const intersections: CurveCurveIntersection[] = method();
        ...
        if (selectedResult && selectedResult.length > 0) {
          ...
          return processVerbIntersectionResults(bestResult, swapParams, true);
        }
      }
    }
