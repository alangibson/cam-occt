    ...
    () => {
        ...
        function visualizeIntersection(
            ...
            if (shape1Outset.success && shape1Outset.shapes.length > 0) {
                svg.addShape(
                    shape1Outset.shapes[0],
                    'red',
                    1,
                    `${shape1.type} outset`
                );
                svg.addShapeExtensions(
                    shape1Outset.shapes[0],
                    'red',
                    1,
                    extensionLength
            ...
            const originalIntersections = findShapeIntersections(
                shape1,
                shape2,
                0.1,
                false,
            ...
            if (
                shape1Outset.success &&
                shape1Outset.shapes.length > 0 &&
                shape2Outset.success &&
                shape2Outset.shapes.length > 0
            ) {
                const outsetIntersections = findShapeIntersections(
                    shape1Outset.shapes[0],
                    shape2Outset.shapes[0],
                    0.1,
                    true,
            ...
            if (
                shape1Inset.success &&
                shape1Inset.shapes.length > 0 &&
                shape2Inset.success &&
                shape2Inset.shapes.length > 0
            ) {
            ...
            const hasEllipse =
                shape1.type === 'ellipse' || shape2.type === 'ellipse';
            ...
            const legendItems = [
                ...
                {
                    color: 'red',
                    label: `Outset (+${offsetDistance}) Intersections`,
                },
                { color: 'orange', label: `Outset Gap Intersections` },
                {
                    color: 'blue',
                    label: `Inset (-${offsetDistance}) Intersections`,
                },
            ...
            if (hasEllipse && offsetShapeTypes.has('polyline')) {
                legendItems.push({
                    color: 'gray',
                    label: 'Note: Ellipse offsets are polyline approximations',
                });
            }
            ...
            if (
                hasEllipse &&
                offsetShapeTypes.has('polyline') &&
                totalIntersections > 0
            ) {
                svg.addText(
                    10,
                    350,
                    'Warning: Intersection points may appear off ellipse curves due to polyline approximation',
                    'red',
                    '10px'
                );
            }
            ...
            const fileName =
                combinationName.toLowerCase().replace('-', '-') + '-gap.svg';
        ...
        describe('Line gap tests', () => {
            ...
            it('should test flipped-arc-debug specific Line-Arc case', () => {
                ...
                if (
                    ...
                    arcInsetResult.success &&
                    lineInsetResult.shapes.length > 0 &&
                    arcInsetResult.shapes.length > 0
                ) {
                    const insetIntersections = findShapeIntersections(
