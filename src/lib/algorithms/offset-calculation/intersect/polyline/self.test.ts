...
describe('Polyline Self-Intersection Detection', () => {
  ...
  describe('findPolylineSelfIntersections', () => {
    ...
    it('should return empty array for polylines with less than 3 segments', () => {
      const singleSegmentPolyline: Shape = {
        ...
        geometry: {
          shapes: [
            { id: 'seg1', type: 'line', geometry: { start: { x: 0, y: 0 }, end: { x: 10, y: 0 } } }
          ],
          closed: false
        } as Polyline
      };
      ...
      const twoSegmentPolyline: Shape = {
        ...
        geometry: {
          shapes: [
            { id: 'seg1', type: 'line', geometry: { start: { x: 0, y: 0 }, end: { x: 10, y: 0 } } },
            { id: 'seg2', type: 'line', geometry: { start: { x: 10, y: 0 }, end: { x: 10, y: 10 } } }
          ],
          closed: false
        } as Polyline
      };

      expect(findPolylineSelfIntersections(singleSegmentPolyline)).toEqual([]);
    ...
    it('should detect self-intersection in simple figure-8 polyline', () => {
      ...
      const figure8Polyline: Shape = {
        ...
        geometry: {
          shapes: [
            { id: 'seg1', type: 'line', geometry: { start: { x: 0, y: 0 }, end: { x: 20, y: 20 } } },
            { id: 'seg2', type: 'line', geometry: { start: { x: 20, y: 20 }, end: { x: 0, y: 20 } } },
            { id: 'seg3', type: 'line', geometry: { start: { x: 0, y: 20 }, end: { x: 20, y: 0 } } },
            { id: 'seg4', type: 'line', geometry: { start: { x: 20, y: 0 }, end: { x: 0, y: 0 } } }
          ],
          closed: true
        } as Polyline
      };

      const intersections = findPolylineSelfIntersections(figure8Polyline);
      
      expect(intersections.length).toBeGreaterThan(0);
      
      // Should find intersection at the center at (10, 10)
      const centerIntersection = intersections.find(
        int => Math.abs(int.point.x - 10) < 0.1 && Math.abs(int.point.y - 10) < 0.1
      );
    ...
    it('should detect multiple self-intersections in complex polyline', () => {
      ...
      const complexPolyline: Shape = {
        ...
        geometry: {
          shapes: [
            { id: 'seg1', type: 'line', geometry: { start: { x: 0, y: 0 }, end: { x: 30, y: 0 } } },
            { id: 'seg2', type: 'line', geometry: { start: { x: 30, y: 0 }, end: { x: 30, y: 20 } } },
            { id: 'seg3', type: 'line', geometry: { start: { x: 30, y: 20 }, end: { x: 0, y: 20 } } },
            { id: 'seg4', type: 'line', geometry: { start: { x: 0, y: 20 }, end: { x: 0, y: 10 } } },
            // Add crossing segments
            { id: 'seg5', type: 'line', geometry: { start: { x: 0, y: 10 }, end: { x: 20, y: 10 } } },
            { id: 'seg6', type: 'line', geometry: { start: { x: 20, y: 10 }, end: { x: 10, y: 5 } } }
          ],
          closed: false
        } as Polyline
      };

      const intersections = findPolylineSelfIntersections(complexPolyline);
      ...
      const simplePolyline: Shape = {
        ...
        geometry: {
          shapes: [
            { id: 'seg1', type: 'line', geometry: { start: { x: 0, y: 0 }, end: { x: 10, y: 0 } } },
            { id: 'seg2', type: 'line', geometry: { start: { x: 10, y: 0 }, end: { x: 10, y: 10 } } },
            { id: 'seg3', type: 'line', geometry: { start: { x: 10, y: 10 }, end: { x: 0, y: 10 } } }
          ],
          closed: false
        } as Polyline
      };
    ...
    it('should handle polylines with arc segments', () => {
      const arcPolyline: Shape = {
        ...
        geometry: {
          shapes: [
            { id: 'seg1', type: 'line', geometry: { start: { x: 0, y: 10 }, end: { x: 30, y: 10 } } },
            { 
              id: 'seg2', 
              type: 'arc', 
              geometry: { 
            ...
            { id: 'seg3', type: 'line', geometry: { start: { x: 25, y: 10 }, end: { x: 15, y: 20 } } },
            { id: 'seg4', type: 'line', geometry: { start: { x: 15, y: 20 }, end: { x: 0, y: 10 } } }
          ],
          closed: true
        } as Polyline
      };
    ...
    it('should return proper parameter values for intersections', () => {
      const figure8Polyline: Shape = {
        ...
        geometry: {
          shapes: [
            { id: 'seg1', type: 'line', geometry: { start: { x: 0, y: 0 }, end: { x: 10, y: 10 } } },
            { id: 'seg2', type: 'line', geometry: { start: { x: 10, y: 10 }, end: { x: 20, y: 0 } } },
            { id: 'seg3', type: 'line', geometry: { start: { x: 20, y: 0 }, end: { x: 10, y: -10 } } },
            { id: 'seg4', type: 'line', geometry: { start: { x: 10, y: -10 }, end: { x: 0, y: 0 } } }
          ],
          closed: true
        } as Polyline
      };
    ...
    it('should throw error for non-polyline shapes', () => {
      ...
      expect(() => findPolylineSelfIntersections(lineShape)).toThrow('Shape must be of type polyline');
    });
  });
  ...
  describe('hasPolylineSelfIntersections', () => {
    ...
    it('should return false for polylines with less than 3 segments', () => {
      ...
      expect(hasPolylineSelfIntersections(twoSegmentPolyline)).toBe(false);
    });
  ...
  describe('Edge Cases', () => {
    ...
    it('should handle nearly coincident segments', () => {
      const nearlyCoincidentPolyline: Shape = {
        ...
        geometry: {
          shapes: [
            { id: 'seg1', type: 'line', geometry: { start: { x: 0, y: 0 }, end: { x: 10, y: 0 } } },
            { id: 'seg2', type: 'line', geometry: { start: { x: 10, y: 0 }, end: { x: 10, y: 10 } } },
            { id: 'seg3', type: 'line', geometry: { start: { x: 10, y: 10 }, end: { x: 5, y: 5 } } },
            // Nearly coincident with seg1 but slightly offset
            { id: 'seg4', type: 'line', geometry: { start: { x: 5, y: 5 }, end: { x: 8, y: 0.001 } } }
          ],
          closed: false
        } as Polyline
      };
      ...
      expect(() => findPolylineSelfIntersections(nearlyCoincidentPolyline)).not.toThrow();
    });
