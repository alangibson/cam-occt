import type { Shape, Polyline, Line, Arc, Circle } from '../../../../../lib/types/geometry';
...
export function findPolylineIntersections(
  ...
  if (polylineShape.type !== 'polyline') {
    // Both are polylines
    return findPolylinePolylineIntersections(shape1, shape2, intersectionType);
  }
  ...
  segments.forEach((segment, segmentIndex) => {
    ...
    if (isLine(segment) && otherShape.type === 'line') {
      ...
      segmentIntersections = findIntersectionsByType(segmentShape, otherShape, false, 1000, intersectionType);
    }
