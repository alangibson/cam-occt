...
import type { IntersectionResult, PolylineSelfIntersection } from '../../chain/types';
import { findIntersectionsByType, type IntersectionType, DEFAULT_INTERSECTION_TYPE } from '../index';
 ...
 */
function getSegmentEndpoints(segment: Line | Arc): { start: Point2D; end: Point2D } {
  if (isLine(segment)) {
  ...
  for (let i = 0; i < segments.length; i++) {
    for (let j = i + 2; j < segments.length; j++) {
      ...
      const segmentIntersections = findSegmentSegmentIntersections(
        segment1, i, segments.length,
        segment2, j, segments.length,
        intersectionType
      );

      // Convert segment parameters to polyline parameters
      segmentIntersections.forEach(intersection => {
        const param1 = calculatePolylineParameter(i, intersection.param1, segments.length);
        const param2 = calculatePolylineParameter(j, intersection.param2, segments.length);
  ...
  for (const event of events) {
    if (event.isStart) {
      ...
      for (const activeIndex of activeSegments) {
        if (isAdjacentInPolyline(event.segmentIndex, activeIndex, segments.length)) {
          continue;
        }

        const intersections = findSegmentSegmentIntersections(
          event.segment, event.segmentIndex, segments.length,
          segments[activeIndex], activeIndex, segments.length,
          intersectionType
        );

        intersections.forEach(intersection => {
          const param1 = calculatePolylineParameter(event.segmentIndex, intersection.param1, segments.length);
          const param2 = calculatePolylineParameter(activeIndex, intersection.param2, segments.length);
 ...
 */
function isAdjacentInPolyline(index1: number, index2: number, totalSegments: number): boolean {
  ...
  if ((index1 === 0 && index2 === totalSegments - 1) || 
