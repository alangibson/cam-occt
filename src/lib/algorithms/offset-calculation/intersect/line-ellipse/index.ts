...
function processEllipseIntersectionPoint(
  ...
  const worldPoint = transformToWorldCoordinates(localPoint, center, cosRot, sinRot);
  
  const ellipseParam: number = calculateEllipseParameter(worldPoint, ellipse);
  
  if (isValidEllipseParameter(ellipseParam, ellipse)) {
    const params = createEllipseLineIntersectionParams(t, ellipseParam, swapParams);
    return {
      point: worldPoint,
 ...
 */
export function findEllipseLineIntersections(ellipseShape: Shape, lineShape: Shape, swapParams: boolean): IntersectionResult[] {
  ...
  const { majorAxisLength: a, minorAxisLength: b, majorAxisAngle: rotAngle } = getEllipseParameters(ellipse);
  ...
  if (discriminant < EPSILON) {
    ...
    const result = processEllipseIntersectionPoint(
      t, dx, dy, p1, ellipse, center, cosRot, sinRot, swapParams, 'tangent'
    );
    if (result) {
      results.push(result);
    }
    ...
    [t1, t2].forEach(t => {
      const result = processEllipseIntersectionPoint(
        t, dx, dy, p1, ellipse, center, cosRot, sinRot, swapParams, 'exact'
      );
      if (result) {
        results.push(result);
      }
...
export function findEllipseLineIntersectionsVerb(
  ...
  const intersections: CurveCurveIntersection[] = verb.geom.Intersect.curves(ellipseCurve, lineCurve, INTERSECTION_TOLERANCE);
