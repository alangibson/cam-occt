...
describe('Line-Ellipse Intersections', () => {
    ...
    function validateIntersectionResults(
    ...
    describe('findEllipseLineIntersections (parametric)', () => {
        describe('basic intersection cases', () => {
            it('should find two intersections when line passes through ellipse', () => {
                ...
                const results = findEllipseLineIntersections(
                    ellipse,
                    line,
                    false
                );

                validateIntersectionResults(
                    results,
                    2,
                    'line through ellipse center'
                );
            ...
            it('should find one intersection when line is tangent to ellipse', () => {
                ...
                expect(
                    pointsApproxEqual(results[0].point, { x: 0, y: 1.8 }, 1e-6)
                ).toBe(true);
            });
        ...
        describe('diagonal and angled lines', () => {
            it('should handle diagonal line intersections', () => {
                ...
                results.forEach((result) => {
                    ...
                    expect(
                        Math.abs(result.point.y - result.point.x)
                    ).toBeLessThan(1e-6);

                    // Point should satisfy ellipse equation
                    const ellipseValue =
                        (result.point.x / 4) ** 2 + (result.point.y / 3) ** 2;
            ...
            it('should handle arbitrary angle intersections', () => {
                ...
                results.forEach((result) => {
                    // Point should satisfy ellipse equation
                    const ellipseValue =
                        (result.point.x / 6) ** 2 + (result.point.y / 3) ** 2;
                    expect(Math.abs(ellipseValue - 1)).toBeLessThan(1e-6);
                });
            });
        });
        ...
        describe('rotated ellipses', () => {
            it('should handle vertically oriented ellipse', () => {
                ...
                validateIntersectionResults(
                    results,
                    2,
                    'horizontal line through vertical ellipse'
                );
        ...
        describe('edge cases and numerical precision', () => {
            ...
            it('should handle ellipse at non-origin center', () => {
                ...
                validateIntersectionResults(
                    results,
                    2,
                    'offset ellipse intersection'
                );

                // Check points are symmetric about center
                const center = { x: 10, y: 20 };
                results.forEach((result) => {
                    expect(Math.abs(result.point.y - center.y)).toBeLessThan(
                        1e-6
                    );
                });
            });
            ...
            it('should handle degenerate ellipse (very flat)', () => {
                ...
                expect(
                    Math.abs(results[0].point.x - -10) < 1e-6 ||
                        Math.abs(results[0].point.x - 10) < 1e-6
                ).toBe(true);
            });
        });
        ...
        describe('parameter calculation and validation', () => {
            ...
            it('should handle parameter swapping correctly', () => {
                ...
                const resultsNormal = findEllipseLineIntersections(
                    ellipse,
                    line,
                    false
                );
                const resultsSwapped = findEllipseLineIntersections(
                    ellipse,
                    line,
                    true
                );
                ...
                if (resultsNormal.length > 0) {
                    expect(resultsNormal[0].param1).toBe(
                        resultsSwapped[0].param2
                    );
                    expect(resultsNormal[0].param2).toBe(
                        resultsSwapped[0].param1
                    );
                }
            });
            ...
            it('should calculate accurate intersection points', () => {
                ...
                results.forEach((result) => {
                    ...
                    expect(
                        Math.abs(Math.abs(result.point.y) - expectedY)
                    ).toBeLessThan(1e-6);
                });
            });
        });
    ...
    describe('findEllipseLineIntersectionsVerb (NURBS-based)', () => {
        describe('comparison with parametric method', () => {
            it('should produce similar results to parametric method for simple cases', () => {
                ...
                const parametricResults = findEllipseLineIntersections(
                    ellipse,
                    line,
                    false
                );
                const verbResults = findEllipseLineIntersectionsVerb(
                    ellipse,
                    line,
                    false
                );
                ...
                expect(
                    Math.abs(verbResults.length - parametricResults.length)
                ...
                if (parametricResults.length > 0 && verbResults.length > 0) {
                    ...
                    const parametricPoints = parametricResults.map(
                        (r) => r.point
                    );
            ...
            it('should handle complex intersection cases', () => {
                ...
                const results = findEllipseLineIntersectionsVerb(
                    ellipse,
                    line,
                    false
                );
        ...
        describe('NURBS-specific capabilities', () => {
            ...
            it('should handle parameter swapping correctly', () => {
                ...
                const resultsNormal = findEllipseLineIntersectionsVerb(
                    ellipse,
                    line,
                    false
                );
                const resultsSwapped = findEllipseLineIntersectionsVerb(
                    ellipse,
                    line,
                    true
                );
                ...
                if (resultsNormal.length > 0) {
                    ...
                    expect(
                        pointsApproxEqual(
                            resultsNormal[0].point,
                            resultsSwapped[0].point,
                            1e-6
                        )
                    ).toBe(true);
                }
    ...
    describe('method comparison and consistency', () => {
        it('should produce consistent results between methods for standard cases', () => {
            const testCases = [
                {
                    ellipse: createEllipse(0, 0, 3, 0, 0.6),
                    line: createLine(-4, 0, 4, 0),
                },
                {
                    ellipse: createEllipse(0, 0, 5, 0, 0.8),
                    line: createLine(0, -6, 0, 6),
                },
                {
                    ellipse: createEllipse(0, 0, 2, 0, 0.5),
                    line: createLine(-3, 1, 3, 1),
                },
            ];
            ...
            testCases.forEach((testCase, _index) => {
                ...
                expect(
                    Math.abs(parametricResults.length - verbResults.length)
                ...
                if (parametricResults.length > 0 && verbResults.length > 0) {
                    ...
                    const parametricHasValidResults = parametricResults.every(
                        (result) =>
                            Number.isFinite(result.point.x) &&
                            Number.isFinite(result.point.y)
                    );
                    const verbHasValidResults = verbResults.every(
                        (result) =>
                            Number.isFinite(result.point.x) &&
                            Number.isFinite(result.point.y)
                    );
