...
export function findEllipsePolylineIntersectionsVerb(
  ...
  const segmentResults = processPolylineSegments(polyline, ellipseCurve, swapParams, false);
  results.push(...segmentResults);
  
  // Handle closed polylines using shared utility
  const closingResults = handleClosedPolylineIntersection(polyline, ellipseCurve, swapParams, false);
