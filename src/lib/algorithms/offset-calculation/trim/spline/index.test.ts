  ...
  describe('trimSpline', () => {
    describe('Basic Trimming Operations', () => {
      it('should trim spline keeping start portion using fit points', async () => {
        ...
        vi.mocked(pointDistance).mockImplementation((p1, p2) => {
          if (p1.x === 10 && p1.y === 10 && p2.x === 10 && p2.y === 10) {
        ...
        expect(result.warnings).toContain('Spline trimming uses simplified approximation');
        ...
        expect(trimmedSpline.controlPoints[trimmedSpline.controlPoints.length - 1]).toEqual(trimPoint);
      });
    ...
    describe('Relaxed Tolerance Matching', () => {
      it('should use relaxed control point matching when exact matching fails', async () => {
        ...
        expect(result.success).toBe(true);
        expect(result.warnings).toContain('Spline trim point found via relaxed control point matching');
        expect(result.warnings).toContain('Spline trimming uses simplified approximation');
      ...
      it('should fail when point is beyond relaxed tolerance', async () => {
        ...
        expect(result.errors).toContain('Trim point is not on the spline');
    ...
    describe('Weight and Knot Vector Handling', () => {
      ...
      it('should pad weights with unit weights when needed', async () => {
        ...
        const trimmedSpline = result.shape?.geometry as Spline;
        expect(trimmedSpline.weights.length).toBe(trimmedSpline.controlPoints.length);
      ...
      it('should generate uniform knot vector', async () => {
        ...
        expect(trimmedSpline.knots.length).toBe(
          trimmedSpline.controlPoints.length + trimmedSpline.degree + 1
        );
        
        // Should be uniform increasing sequence
        for (let i = 1; i < trimmedSpline.knots.length; i++) {
          expect(trimmedSpline.knots[i]).toBeGreaterThanOrEqual(trimmedSpline.knots[i - 1]);
        }
      });
    });
    ...
    describe('Error Cases', () => {
      it('should return error for spline with less than 2 control points', async () => {
        ...
        expect(result.success).toBe(false);
        expect(result.errors).toContain('Cannot trim spline with less than 2 control points');
        expect(result.shape).toBeNull();
      });
      ...
      it('should return error for invalid keepSide value', async () => {
        ...
        const result = trimSpline(shape, trimPoint, 'invalid' as KeepSide, 0.1);

        expect(result.success).toBe(false);
        expect(result.errors).toContain('Invalid keepSide value for spline trimming: invalid');
        expect(result.shape).toBeNull();
      });
    ...
    describe('Edge Cases', () => {
      ...
      it('should handle spline with many control points', async () => {
        ...
        const { pointDistance } = await import('..');
        vi.mocked(pointDistance).mockImplementation((p1, p2) => {
          if (Math.abs(p2.x - 50) < 0.1 && Math.abs(p2.y - Math.sin(10 * 0.5) * 10) < 0.1) {
        ...
        expect(trimmedSpline.controlPoints.length).toBeLessThan(manyPoints.length);
      });
      ...
      it('should handle degenerate segment in fit points', async () => {
        ...
        vi.mocked(calculateLineParameter).mockImplementation((point, line) => {
          ...
          if (line.start.x === 0 && line.start.y === 0 && line.end.x === 10 && line.end.y === 10) {
    ...
    describe('Complex Scenarios', () => {
      ...
      it('should preserve original spline properties in trimmed result', async () => {
        ...
        const result = trimSpline(originalShape, trimPoint, 'start', 0.1);
