 ...
 */
export function trimLine(shape: Shape, point: Point2D, keepSide: KeepSide, tolerance: number): TrimResult {
  ...
  const lineVec: Point2D = { x: lineEnd.x - lineStart.x, y: lineEnd.y - lineStart.y };
  const pointVec: Point2D = { x: point.x - lineStart.x, y: point.y - lineStart.y };

  // Calculate perpendicular distance
  const lineLength: number = Math.sqrt(lineVec.x * lineVec.x + lineVec.y * lineVec.y);
  if (lineLength < EPSILON) {
    result.errors.push('Cannot trim degenerate line');
    return result;
  }

  const perpDistance: number = Math.abs(lineVec.x * pointVec.y - lineVec.y * pointVec.x) / lineLength;
  ...
  if (perpDistance > tolerance || param < -paramTolerance || param > 1 + paramTolerance) {
  ...
  switch (keepSide) {
    ...
    case 'after':
      ...
      break;
    default:
      result.errors.push(`Invalid keepSide value for line trimming: ${keepSide}`);
      return result;
  }
