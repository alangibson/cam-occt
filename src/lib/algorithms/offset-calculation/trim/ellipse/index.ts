 ...
 */
export function trimEllipse(shape: Shape, point: Point2D, keepSide: KeepSide, tolerance: number): TrimResult {
  ...
  const a: number = Math.sqrt(majorAxisEndpoint.x ** 2 + majorAxisEndpoint.y ** 2); // Semi-major axis
  ...
  const normalizedTrimAngle: number = trimAngle < 0 ? trimAngle + 2 * Math.PI : trimAngle;
  ...
  if (ellipse.startParam !== undefined && ellipse.endParam !== undefined) {
    ...
    if (startParam <= endParam) {
      angleInRange = normalizedTrimAngle >= startParam - tolerance &&
  ...
  if (ellipse.startParam !== undefined && ellipse.endParam !== undefined) {
    ...
    switch (keepSide) {
      ...
      case 'after':
        ...
        break;
      default:
        result.errors.push(`Invalid keepSide value for ellipse arc trimming: ${keepSide}`);
        return result;
    }
  } else {
    ...
    switch (keepSide) {
      ...
      case 'after':
        ...
        break;
      default:
        result.errors.push(`Invalid keepSide value for ellipse trimming: ${keepSide}`);
        return result;
    }
  }
  ...
  if (angularRange < EPSILON) {
    result.errors.push('Trimmed ellipse arc would be degenerate (zero angular range)');
    return result;
  }
  ...
  if (ellipse.startParam === undefined && ellipse.endParam === undefined) {
    result.warnings.push('Full ellipse converted to ellipse arc for trimming');
  }
