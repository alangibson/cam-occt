...
import type { Shape, Point2D, Polyline, Line, PolylineVertex } from '$lib/types/geometry';
import { generateId } from '$lib/utils/id';
import { type KeepSide, type TrimResult } from '../types';
import { polylineToPoints, createPolylineFromVertices, polylineToVertices } from '$lib/geometry/polyline';
 ...
 */
function findClosestPointOnPolylineSegments(point: Point2D, points: Point2D[]): {
  minDistance: number;
  closestIndex: number;
  closestParam: number;
} {
  ...
  for (let i: number = 0; i < points.length - 1; i++) {
    ...
    const pointOnSegment: Point2D = {
      x: segment.start.x + clampedParam * (segment.end.x - segment.start.x),
      y: segment.start.y + clampedParam * (segment.end.y - segment.start.y)
    };
 ...
 */
export function trimPolyline(shape: Shape, point: Point2D, keepSide: KeepSide, tolerance: number): TrimResult {
  ...
  for (let i: number = 0; i < points.length - 1; i++) {
    ...
    if (param >= -tolerance && param <= 1 + tolerance) {
      ...
      const segmentVec: Point2D = { x: segment.end.x - segment.start.x, y: segment.end.y - segment.start.y };
      const pointVec: Point2D = { x: point.x - segment.start.x, y: point.y - segment.start.y };
      const segmentLength: number = Math.sqrt(segmentVec.x * segmentVec.x + segmentVec.y * segmentVec.y);
  ...
  if (segmentIndex === -1) {
    ...
    const { minDistance, closestIndex, closestParam } = findClosestPointOnPolylineSegments(point, points);
    ...
    if (closestIndex !== -1 && minDistance <= relaxedTolerance) {
      ...
      result.warnings.push('Polyline trim point found via relaxed closest segment matching');
    } else {
  ...
  switch (keepSide) {
    ...
    default:
      result.errors.push(`Invalid keepSide value for polyline trimming: ${keepSide}`);
      return result;
  }
