...
function validateArcAngleRange(arc: Arc, tolerance: number): boolean {
  let angleRange;
  if (arc.clockwise) {
    angleRange = arc.startAngle > arc.endAngle ? 
      arc.startAngle - arc.endAngle : 
      arc.startAngle - arc.endAngle + 2 * Math.PI;
  } else {
    angleRange = arc.endAngle > arc.startAngle ? 
 ...
 */
export function trimArc(shape: Shape, point: Point2D, keepSide: KeepSide, tolerance: number): TrimResult {
  ...
  const pointAngle: number = Math.atan2(point.y - arc.center.y, point.x - arc.center.x);
  ...
  if (!isAngleInArcRange(pointAngle, arc.startAngle, arc.endAngle, arc.clockwise)) {
    ...
    try {
      ...
      const angles = adjustArcAnglesForTrim(extendedArc, pointAngle, keepSide);
      if (!angles) {
        result.errors.push(`Invalid keepSide value for arc trimming: ${keepSide}`);
        return result;
      }
      ...
      if (!validateArcAngleRange(trimmedArc, TOLERANCE)) {
        result.errors.push('Trimmed arc would be degenerate (zero angular range)');
        return result;
      }
      ...
      result.errors.push(`Failed to extend arc to trim point: ${(error as Error).message}`);
      return result;
    }
  }
