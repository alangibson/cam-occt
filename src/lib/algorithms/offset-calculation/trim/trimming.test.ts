...
import type { Shape, Line, Arc, Circle, Point2D, Polyline, Spline, Ellipse } from '../../../types/geometry';
...
describe('trimming', () => {
  ...
  function validateTrimResult(result: TrimResult, shouldSucceed: boolean = true): void {
    if (shouldSucceed) {
  ...
  describe('trimShapeAtPoint', () => {
    describe('line trimming', () => {
      it('should trim line at start (keep end portion)', () => {
        ...
        expect(pointsApproxEqual(trimmedLine.start, trimPoint)).toBe(true);
        expect(pointsApproxEqual(trimmedLine.end, { x: 10, y: 0 })).toBe(true);
      });
      ...
      it('should trim line at end (keep start portion)', () => {
        ...
        validateTrimResult(result, true);
        const trimmedLine = result.shape!.geometry as Line;
        expect(pointsApproxEqual(trimmedLine.start, { x: 0, y: 0 })).toBe(true);
        expect(pointsApproxEqual(trimmedLine.end, trimPoint)).toBe(true);
      });
    ...
    describe('arc trimming', () => {
      it('should trim arc at start (keep end portion)', () => {
        ...
        expect(pointsApproxEqual(trimmedArc.center, { x: 0, y: 0 })).toBe(true);
      });
      ...
      it('should extend arc when trim point is outside angular range', () => {
        ...
        validateTrimResult(result, true);
        expect(result.warnings[0]).toContain('Arc was extended to reach trim point');
    ...
    describe('circle trimming', () => {
      it('should convert circle to arc when trimmed', () => {
        ...
        expect(result.warnings).toContain('Circle converted to arc for trimming');
    ...
    describe('ellipse trimming', () => {
      function createEllipse(cx: number, cy: number, majorAxisX: number, majorAxisY: number, minorToMajorRatio: number, startParam?: number, endParam?: number): Shape {
        return {
          id: generateId(),
          type: 'ellipse',
          geometry: {
      ...
      it('should trim full ellipse at point (convert to ellipse arc)', () => {
        ...
        validateTrimResult(result, true);
        expect(result.warnings).toContain('Full ellipse converted to ellipse arc for trimming');
        ...
        expect(pointsApproxEqual(trimmedEllipse.center, { x: 0, y: 0 })).toBe(true);
        expect(pointsApproxEqual(trimmedEllipse.majorAxisEndpoint, { x: 4, y: 0 })).toBe(true);
      ...
      it('should reject trim point outside ellipse arc bounds', () => {
        ...
        validateTrimResult(result, false);
        expect(result.errors[0]).toContain('not within ellipse arc bounds');
      });

      it('should handle ellipse arc crossing 0° (wraparound case)', () => {
        const ellipse = createEllipse(0, 0, 3, 0, 0.75, 7 * Math.PI / 4, Math.PI / 4); // 315° to 45°
        const a = 3;
        const b = 3 * 0.75; // 2.25
      ...
      it('should reject degenerate ellipse arc', () => {
        const ellipse = createEllipse(0, 0, 4, 0, 0.5, Math.PI / 4, Math.PI / 4 + 1e-12); // Extremely small arc
        const a = 4;
        const b = 4 * 0.5; // 2
      ...
      it('should handle tolerance correctly for on-ellipse detection', () => {
        ...
        const result1 = trimShapeAtPoint(ellipse, trimPoint, 'start', 1e-6);
        validateTrimResult(result1, false);
        
        // Should succeed with loose tolerance
        const result2 = trimShapeAtPoint(ellipse, trimPoint, 'start', 0.02);
      ...
      it('should preserve ellipse properties after trimming', () => {
        ...
        validateTrimResult(result, true);
        const trimmedEllipse = result.shape!.geometry as Ellipse;
        expect(pointsApproxEqual(trimmedEllipse.center, { x: 2, y: 3 })).toBe(true);
        expect(pointsApproxEqual(trimmedEllipse.majorAxisEndpoint, { x: 5, y: 0 })).toBe(true);
      ...
      it('should handle invalid keepSide values', () => {
        ...
        const result = trimShapeAtPoint(ellipse, trimPoint, 'invalid' as KeepSide);
    ...
    describe('unsupported shape types', () => {
      it('should handle polylines successfully', () => {
        ...
        const polyline: Shape = {
          id: generateId(),
          type: 'polyline',
          geometry: createPolylineFromVertices(vertices, false).geometry as Polyline
        };
        
        const result = trimShapeAtPoint(polyline, { x: 5, y: 0 }, 'start');
        
        if (!result.success) {
          console.log('Polyline trim failed with errors:', result.errors);
        }
      ...
      it('should reject trimming at endpoints that would result in degenerate polylines', () => {
        ...
        const result = trimShapeAtPoint(polyline, { x: 0, y: 0 }, 'start');
        
        validateTrimResult(result, false);
        expect(result.errors[0]).toContain('Trimmed polyline would have less than 2 points');
      });
      ...
      it('should handle splines gracefully', () => {
        ...
        const result = trimShapeAtPoint(spline, { x: 5, y: 2.5 }, 'start');
        
        validateTrimResult(result, false);
        expect(result.errors[0]).toContain('Trim point is not on the spline');
      });
    });
  });

  describe('selectTrimPoint', () => {
    function createIntersection(x: number, y: number, confidence: number = 1.0, type: string = 'exact'): IntersectionResult {
      return {
        point: { x, y },
        param1: 0.5,
        param2: 0.5,
  ...
  describe('trimConsecutiveShapes', () => {
    it('should trim both shapes at their intersection', () => {
      ...
      expect(pointsApproxEqual(trimmed2.start, { x: 5, y: 5 })).toBe(true);
    });
    ...
    it('should handle no intersections gracefully', () => {
      ...
      expect(result.shape1Result.errors[0]).toContain('No intersections found');
    });
  ...
  describe('edge cases and error handling', () => {
    it('should handle very small geometric features', () => {
      ...
      const result = trimShapeAtPoint(tinyLine, { x: 5e-9, y: 0 }, 'start');
