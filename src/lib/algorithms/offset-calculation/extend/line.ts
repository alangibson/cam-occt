...
import { pointDistance, calculateLineParameter } from '../shared/trim-extend-utils';
...
export function extendLineToPoint(
  ...
  try {
    ...
    const extendDirection: 'start' | 'end' | null = determineExtensionDirection(line, intersectionPoint, opts);
    if (!extendDirection) {
      return null;
    }

    // Calculate extension parameters
    const extensionInfo: LineExtensionResult = calculateExtension(line, intersectionPoint, extendDirection, unitDir);
    ...
    return createExtendedLineToPoint(line, intersectionPoint, extendDirection, extensionInfo);
...
export function calculateExtension(
  ...
  if (direction === 'start') {
    ...
    if (lineParam >= 0) {
      return { success: false, extensionDistance: 0, direction, error: 'Intersection point is not behind line start' };
    }
    extensionDistance = Math.abs(lineParam) * Math.sqrt(
      (line.end.x - line.start.x) ** 2 + (line.end.y - line.start.y) ** 2
    );
  } else {
    // Extension forwards from end
    if (lineParam <= 1) {
      return { success: false, extensionDistance: 0, direction, error: 'Intersection point is not beyond line end' };
    }
    extensionDistance = (lineParam - 1) * Math.sqrt(
      (line.end.x - line.start.x) ** 2 + (line.end.y - line.start.y) ** 2
    );
  }
...
function createExtendedLineToPoint(
  ...
  if (direction === 'start') {
    ...
    return {
      start: {
        x: originalLine.start.x - unitDir.x * extensionInfo.extensionDistance,
        y: originalLine.start.y - unitDir.y * extensionInfo.extensionDistance
      },
    ...
    return {
      start: originalLine.start,
        ...
        x: originalLine.end.x + unitDir.x * extensionInfo.extensionDistance,
        y: originalLine.end.y + unitDir.y * extensionInfo.extensionDistance
      }
    };
  }
}
