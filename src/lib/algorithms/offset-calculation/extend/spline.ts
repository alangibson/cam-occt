import type { Point2D, Spline, Line, Arc, Circle, Ellipse, Polyline } from '../../../types/geometry';
...
export function calculateSplineStartTangent(spline: Spline): Point2D {
  try {
    ...
    const derivatives: number[][] = verb.eval.Eval.rationalCurveDerivatives(verbCurve.asNurbs(), 0, 1);
    
    // First derivative at parameter 0 gives tangent vector
    const tangent: number[] = derivatives[1];
    const magnitude: number = Math.sqrt(tangent[0] * tangent[0] + tangent[1] * tangent[1]);
...
export function calculateSplineEndTangent(spline: Spline): Point2D {
  try {
    const verbCurve: verb.geom.ICurve = createVerbCurveFromSpline(spline);
    const derivatives: number[][] = verb.eval.Eval.rationalCurveDerivatives(verbCurve.asNurbs(), 1, 1);
 ...
 */
export function calculateSplineExtensionLength(spline: Spline, otherShape: Line | Arc | Circle | Ellipse | Spline | Polyline): number {
  // Calculate rough bounding box diagonal of both shapes
  const splineBounds: { minX: number; maxX: number; minY: number; maxY: number } = calculateSplineBounds(spline);
  const otherBounds: { minX: number; maxX: number; minY: number; maxY: number } = calculateOtherShapeBounds(otherShape);
  
  // Combine bounding boxes
  const combinedBounds: { minX: number; maxX: number; minY: number; maxY: number } = {
 ...
 */
function calculateSplineBounds(spline: Spline): { minX: number; maxX: number; minY: number; maxY: number } {
  let minX: number = Infinity, maxX: number = -Infinity;
 ...
 */
function calculateOtherShapeBounds(shape: Line | Arc | Circle | Ellipse | Spline | Polyline) {
  if (isLine(shape)) {
    // Line
    shape = shape as Line;
    return {
...
export function createExtendedSplineVerb(
  ...
  try {
    const verbCurve: verb.geom.ICurve = createVerbCurveFromSpline(spline);
    const startEval: number[] = verb.eval.Eval.rationalCurvePoint(verbCurve.asNurbs(), 0);
    const endEval: number[] = verb.eval.Eval.rationalCurvePoint(verbCurve.asNurbs(), 1);
  ...
  const originalEnd: number = extendEnd ? spline.controlPoints.length - 1 : spline.controlPoints.length;
  ...
  const extendedKnots: number[] = generateUniformKnots(extendedControlPoints.length, extendedDegree);
  const extendedWeights: number[] = extendedControlPoints.map(() => 1);
  
  // Convert to 3D control points for verb
  const controlPoints3D: [number, number, number][] = extendedControlPoints.map(p => [p.x, p.y, 0] as [number, number, number]);
...
export function analyzeParameterLocation(
  ...
  const totalPoints: number = originalControlPointCount + startExtensionPoints + endExtensionPoints;
  
  // Approximate parameter boundaries (this is a rough estimate)
  const startBoundary: number = startExtensionPoints / totalPoints;
  const endBoundary: number = (startExtensionPoints + originalControlPointCount) / totalPoints;
...
export function extendSplineToPoint(
  ...
  try {
    ...
    const extendDirection: 'start' | 'end' | null = determineSplineExtensionDirection(spline, intersectionPoint, opts);
    if (!extendDirection) {
      return null;
    }

    // Step 3: Calculate extension parameters
    const extensionInfo: SplineExtensionResult = calculateSplineExtension(spline, intersectionPoint, extendDirection, opts);
    ...
    return createExtendedSplineToPoint(spline, intersectionPoint, extendDirection, extensionInfo);
...
export function calculateSplineExtension(
  ...
  const endPoint: Point2D = direction === 'start' ? 
    getSplinePoint(spline, 0) : 
    getSplinePoint(spline, 1);
    
  const extensionDistance: number = pointDistance(endPoint, intersectionPoint);
...
function createExtendedSplineToPoint(
  ...
  if (extensionInfo.method === 'linear') {
    ...
    return createLinearExtendedSplineToPoint(originalSpline, intersectionPoint, direction);
  } else {
...
function createLinearExtendedSplineToPoint(
  ...
  const weights: number[] = originalSpline.weights ? [...originalSpline.weights] : controlPoints.map(() => 1);
...
function calculateTangentExtensionPoint(
  ...
  const tangentLength: number = Math.sqrt(tangentDir.x * tangentDir.x + tangentDir.y * tangentDir.y);
 ...
 */
function generateKnotVector(numControlPoints: number, degree: number): number[] {
  const knots: number[] = [];
