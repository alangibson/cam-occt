...
import { normalizeAngle, isAngleInArcRange } from '../../intersection-arc-utils';
...
export function extendArcToPoint(
  ...
  try {
    ...
    const distanceFromCenter: number = pointDistance(intersectionPoint, arc.center);
    const radiusTolerance: number = Math.max(opts.tolerance!, arc.radius * 1e-6);
    ...
    const extendDirection: 'start' | 'end' | null = determineArcExtensionDirection(arc, intersectionAngle, opts);
    if (!extendDirection) {
      return null;
    }
    ...
    // Create extended arc
    return createExtendedArcToPoint(arc, intersectionAngle, extendDirection);
...
export function determineArcExtensionDirection(
  ...
  const isAngleInArc: boolean = isAngleInArcRange(intersectionAngle, arc.startAngle, arc.endAngle, arc.clockwise);
  
  if (isAngleInArc) {
    // Intersection is within the arc - this shouldn't happen for gap filling
    // Choose the direction that requires less extension
    const startExtension: number = calculateAngularDistance(arc.startAngle, intersectionAngle, !arc.clockwise);
    const endExtension: number = calculateAngularDistance(intersectionAngle, arc.endAngle, arc.clockwise);
  ...
  const distanceFromStart: number = calculateAngularDistanceForExtension(arc, intersectionAngle, 'start');
  const distanceFromEnd: number = calculateAngularDistanceForExtension(arc, intersectionAngle, 'end');
...
function calculateAngularDistanceForExtension(
  ...
  if (direction === 'start') {
    ...
    if (arc.clockwise) {
      ...
      return calculateAngularDistance(arc.startAngle, intersectionAngle, false);
    } else {
      // For counter-clockwise arcs, start extends clockwise  
      return calculateAngularDistance(arc.startAngle, intersectionAngle, true);
    }
  } else {
    // Extending end means going in the same direction as the arc
    if (arc.clockwise) {
      // For clockwise arcs, end extends clockwise
      return calculateAngularDistance(arc.endAngle, intersectionAngle, true);
    } else {
      // For counter-clockwise arcs, end extends counter-clockwise
      return calculateAngularDistance(arc.endAngle, intersectionAngle, false);
    }
  }
}
 ...
 */
export function calculateAngularDistance(fromAngle: number, toAngle: number, clockwise: boolean): number {
