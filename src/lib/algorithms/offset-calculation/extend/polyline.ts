import type { Point2D, Polyline, Shape } from '../../../types/geometry';
import { EPSILON, MAX_EXTENSION } from '../../../constants';
import { pointDistance } from '../trim';
import { polylineToPoints, createPolylineFromVertices } from '$lib/geometry/polyline';
...
export function createExtendedPolyline(
  ...
  const extendedShape: Shape = createPolylineFromVertices(extendedPoints, false);
...
export function extendPolylineToPoint(
  ...
  const opts: Required<PolylineExtensionOptions> = { ...defaultOptions, ...options };
  ...
  try {
    ...
    const extendDirection: 'start' | 'end' | null = determinePolylineExtensionDirection(polyline, intersectionPoint, opts);
    if (!extendDirection) {
      return null;
    }

    // Step 3: Calculate extension parameters
    const extensionInfo: PolylineExtensionResult = calculatePolylineExtension(polyline, intersectionPoint, extendDirection, opts);
    ...
    return createExtendedPolylineToPoint(polyline, intersectionPoint, extendDirection, extensionInfo);
...
export function determinePolylineExtensionDirection(
  ...
  const isBeforeStart: boolean = isPointBeforePolylineStart(polyline, intersectionPoint);
  const isAfterEnd: boolean = isPointAfterPolylineEnd(polyline, intersectionPoint);
 ...
 */
export function isPointBeforePolylineStart(polyline: Polyline, point: Point2D): boolean {
  const polylinePoints: Point2D[] = polylineToPoints(polyline);
  ...
  const projectionParam: number = (toPoint.x * segmentDir.x + toPoint.y * segmentDir.y) /
 ...
 */
export function isPointAfterPolylineEnd(polyline: Polyline, point: Point2D): boolean {
  const polylinePoints: Point2D[] = polylineToPoints(polyline);
  if (polylinePoints.length < 2) return false;
...
export function calculatePolylineExtension(
  ...
  if (direction === 'start') {
    ...
    const firstPoint: Point2D = polylinePoints[0];
    const extensionDistance: number = pointDistance(firstPoint, intersectionPoint);
    
    return { success: true, extensionDistance, direction };
  } else {
    // Extension from end
    const lastPoint: Point2D = polylinePoints[polylinePoints.length - 1];
    const extensionDistance: number = pointDistance(lastPoint, intersectionPoint);
...
function createExtendedPolylineToPoint(
  ...
  if (direction === 'start') {
    ...
    const extendedShape: Shape = createPolylineFromVertices(newPoints, originalPolyline.closed);
    return extendedShape.geometry as Polyline;
  } else {
