...
describe('Shape Extension Visual Tests', { timeout: 60000 }, () => {
  ...
  it('should generate SVG for line extensions', () => {
    // Single line example
    const testLine: Line = { start: { x: 100, y: 150 }, end: { x: 300, y: 150 } };
    ...
    const startExtensionLine: Line = { start: testLine.start, end: extendedLine.start };
    ...
    const endExtensionLine: Line = { start: testLine.end, end: extendedLine.end };
      ...
      try {
        ...
        const fallbackPolyline: Polyline = {
          shapes: spline.controlPoints.slice(0, -1).map((p, i) => ({
            ...
            type: 'line' as const,
            geometry: { start: p, end: spline.controlPoints[i + 1] } as Line
          })),
          closed: spline.closed
        };
        ...
        try {
          ...
          const outsetFallbackPolyline: Polyline = {
            shapes: outsetGeometry.controlPoints.slice(0, -1).map((p, i) => ({
              ...
              type: 'line' as const,
              geometry: { start: p, end: outsetGeometry.controlPoints[i + 1] } as Line
            })),
            closed: outsetGeometry.closed
          };
        ...
        try {
          ...
          const insetFallbackPolyline: Polyline = {
            shapes: insetGeometry.controlPoints.slice(0, -1).map((p, i) => ({
              ...
              type: 'line' as const,
              geometry: { start: p, end: insetGeometry.controlPoints[i + 1] } as Line
            })),
            closed: insetGeometry.closed
          };
  ...
  it('should generate SVG for polyline extensions', () => {
    ...
    testPolylines.forEach((polyline, index) => {
      ...
      if (outsetResult.success && outsetResult.shapes.length > 0) {
        outsetResult.shapes.forEach(shape => {
          ...
          if (!polylineGeometry.closed) {
            svg.addPolylineExtensions(polylineGeometry, 'red', 1, 40);
          }
        });
      }
      ...
      if (insetResult.success && insetResult.shapes.length > 0) {
        insetResult.shapes.forEach(shape => {
          ...
          // Add extensions to offset polyline (only for open polylines)
          if (!polylineGeometry.closed) {
            svg.addPolylineExtensions(polylineGeometry, 'blue', 1, 40);
          }
        });
      }
      ...
      if (!polyline.closed) {
        ...
        if (extendedPoints.length > originalPoints.length) {
          ...
          const startExtLine: Line = { start: extendedPoints[0], end: originalPoints[0] };
          ...
          const extendedEnd = extendedPoints[extendedPoints.length - 1];
          const endExtLine: Line = { start: originalEnd, end: extendedEnd };
      ...
      svg.addText(firstPoint.x, firstPoint.y - 15, labelText, 'black', '10px');
    });
    ...
    writeFileSync(join(outputDir, 'polyline-extensions.svg'), svg.toString());
  });
  ...
  it('should generate SVG for ellipse arc extensions', () => {
    ...
    testEllipseArcs.forEach((ellipseArc, index) => {
      ...
      const centerCircle: Circle = { center: ellipseArc.center, radius: 2 };
      ...
      const majorAxisLine: Line = { start: ellipseArc.center, end: majorEnd };
      ...
      if (arcPoints.length > 0) {
        ...
        const endCircle: Circle = { center: arcPoints[arcPoints.length - 1], radius: 3 };
      ...
      if (arcPoints.length >= 2) {
        ...
        const endTangent = {
          x: arcPoints[arcPoints.length - 1].x - arcPoints[arcPoints.length - 2].x,
          y: arcPoints[arcPoints.length - 1].y - arcPoints[arcPoints.length - 2].y
        };
        ...
        if (startLen > 0 && endLen > 0) {
          ...
          const startExt = {
            x: arcPoints[0].x - (startTangent.x / startLen) * extensionLength,
            y: arcPoints[0].y - (startTangent.y / startLen) * extensionLength
          };
          const endExt = {
            x: arcPoints[arcPoints.length - 1].x + (endTangent.x / endLen) * extensionLength,
            y: arcPoints[arcPoints.length - 1].y + (endTangent.y / endLen) * extensionLength
          };
          
          // Draw tangent extensions
          const startExtLine: Line = { start: startExt, end: arcPoints[0] };
          const startExtShape: Shape = {
          ...
          const endExtLine: Line = { start: arcPoints[arcPoints.length - 1], end: endExt };
          const endExtShape: Shape = {
          ...
          try {
            ...
            const originalEndIdx = extendedPoints.length - originalStartIdx;
            
            if (originalStartIdx > 0) {
              const startExtPoints = extendedPoints.slice(0, originalStartIdx);
              const startExtPolyline: Polyline = {
                shapes: startExtPoints.slice(0, -1).map((p, i) => ({
                  id: `ellipse-ext-start-seg-${index}-${i}`,
                  type: 'line' as const,
                  geometry: { start: p, end: startExtPoints[i + 1] } as Line
                })),
                closed: false
              };
            ...
            if (originalEndIdx < extendedPoints.length) {
              const endExtPoints = extendedPoints.slice(originalEndIdx);
              const endExtPolyline: Polyline = {
                shapes: endExtPoints.slice(0, -1).map((p, i) => ({
                  id: `ellipse-ext-end-seg-${index}-${i}`,
                  type: 'line' as const,
                  geometry: { start: p, end: endExtPoints[i + 1] } as Line
                })),
                closed: false
              };
        ...
        if (offsetShape.type === 'spline') {
          ...
          try {
            const verbCurve = verb.geom.NurbsCurve.byKnotsControlPointsWeights(
              ...
              splineGeometry.controlPoints.map(p => [p.x, p.y, 0]),
            ...
            const offsetPolyline: Polyline = {
              shapes: offsetPoints.slice(0, -1).map((p, i) => ({
                ...
                type: 'line' as const,
                geometry: { start: p, end: offsetPoints[i + 1] } as Line
              })),
      ...
      const paramRange = ((ellipseArc.endParam! - ellipseArc.startParam!) * 180 / Math.PI).toFixed(0);
      svg.addText(ellipseArc.center.x, ellipseArc.center.y + 80, 
        `Arc ${index + 1}: ${paramRange}°`, 'black', '10px');
    });
    ...
    writeFileSync(join(outputDir, 'ellipse-arc-extensions.svg'), svg.toString());
  });
});