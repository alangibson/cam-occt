 ...
 */
export function validateSplineForIntersection(spline: Spline): { isValid: boolean; error?: string } {
  if (!spline.controlPoints || !Array.isArray(spline.controlPoints) || spline.controlPoints.length < 2) {
    return { isValid: false, error: 'Invalid or insufficient control points' };
  }
  
  const degree = spline.degree || 3;
  if (degree < 1 || degree >= spline.controlPoints.length) {
    return { isValid: false, error: 'Invalid degree for control point count' };
  }
  ...
  if (spline.knots.length !== expectedKnotCount) {
    return { isValid: false, error: `Invalid knot vector length: expected ${expectedKnotCount}, got ${spline.knots.length}` };
  }
  
  // Check for degenerate control points (all the same)
  const firstPoint = spline.controlPoints[0];
  const allSame = spline.controlPoints.every(p => 
    Math.abs(p.x - firstPoint.x) < INTERSECTION_TOLERANCE && 
    Math.abs(p.y - firstPoint.y) < INTERSECTION_TOLERANCE
  );
  
  if (allSame) {
    return { isValid: false, error: 'Degenerate spline: all control points are identical' };
  }
...
export function processSplineIntersection(
  ...
  try {
    ...
    const originalIntersections = verb.geom.Intersect.curves(curve1, curve2, INTERSECTION_TOLERANCE);
    
    if (originalIntersections && originalIntersections.length > 0) {
      // Found intersections with original shapes - return them
      return processVerbIntersectionResults(originalIntersections, swapParams, false);
    }
    
    // If no intersections found and extensions are allowed, try with extended splines
    if (allowExtensions) {
      return processSplineIntersectionWithExtensions(spline1, spline2, swapParams, extensionLength);
    }
...
function processSplineIntersectionWithExtensions(
  ...
  try {
    const extendedCurve1 = createExtendedSplineVerb(spline1, true, true, extensionLength);
    const extendedSpline1Intersections = verb.geom.Intersect.curves(extendedCurve1, originalCurve2, INTERSECTION_TOLERANCE);
    
    if (extendedSpline1Intersections && extendedSpline1Intersections.length > 0) {
      const results = processVerbIntersectionResults(extendedSpline1Intersections, swapParams, true);
  ...
  try {
    const extendedCurve2 = createExtendedSplineVerb(spline2, true, true, extensionLength);
    const extendedSpline2Intersections = verb.geom.Intersect.curves(originalCurve1, extendedCurve2, INTERSECTION_TOLERANCE);
    
    if (extendedSpline2Intersections && extendedSpline2Intersections.length > 0) {
      const results = processVerbIntersectionResults(extendedSpline2Intersections, swapParams, true);
      allExtensionResults.push(...results);
    }
  } catch {
  ...
  try {
    const extendedCurve1 = createExtendedSplineVerb(spline1, true, true, extensionLength);
    const extendedCurve2 = createExtendedSplineVerb(spline2, true, true, extensionLength);
    const extendedBothIntersections = verb.geom.Intersect.curves(extendedCurve1, extendedCurve2, INTERSECTION_TOLERANCE);
    
    if (extendedBothIntersections && extendedBothIntersections.length > 0) {
      const results = processVerbIntersectionResults(extendedBothIntersections, swapParams, true);
      allExtensionResults.push(...results);
    }
  } catch {
...
export function processSplineWithCurveIntersection(
  ...
  try {
    ...
    const originalIntersections = verb.geom.Intersect.curves(splineCurve, otherCurve, INTERSECTION_TOLERANCE);
    ...
    if (allowExtensions) {
      try {
        ...
        const extendedIntersections = verb.geom.Intersect.curves(extendedSplineCurve, otherCurve, INTERSECTION_TOLERANCE);
        
        if (extendedIntersections && extendedIntersections.length > 0) {
          return processVerbIntersectionResults(extendedIntersections, swapParams, true);
        }
      } catch {
...
export function processSplineIntersectionWithRetry(
  ...
  for (let retry = 0; retry < retryCount; retry++) {
    try {
      const intersections = verb.geom.Intersect.curves(splineCurve, otherCurve, INTERSECTION_TOLERANCE);
  ...
  if (bestResult) {
    return processVerbIntersectionResults(bestResult, swapParams, isExtended);
  }
