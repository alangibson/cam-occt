 ...
 */
export function calculatePointToLineDistance(point: Point2D, line: Line): number {
  ...
  const numerator = Math.abs((line.end.y - line.start.y) * point.x - 
 ...
 */
export function snapParameterToEndpoints(param: number, tolerance: number = EPSILON): number {
 ...
 */
export function validateTrimExtendParameters(params: TrimExtendParams): TrimExtendValidationResult {
  ...
  // Validate point
  if (!params.point || typeof params.point.x !== 'number' || typeof params.point.y !== 'number') {
    result.isValid = false;
    result.errors.push('Invalid point: must have numeric x and y coordinates');
  ...
  if (params.tolerance > 1.0) {
    result.warnings.push('Large tolerance value may lead to unexpected results');
  }

  // Validate max extension if provided
  if (params.maxExtension !== undefined) {
    if (typeof params.maxExtension !== 'number' || params.maxExtension <= 0) {
      result.isValid = false;
      result.errors.push('Invalid maxExtension: must be a positive number');
    }

    if (params.maxExtension > 1000) {
      result.warnings.push('Large maxExtension value may lead to unexpected results');
    }
  }
