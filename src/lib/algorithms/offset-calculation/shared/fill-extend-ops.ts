 ...
 */
export function validateEllipseOperation(
    ...
    try {
        ...
        if (radiusX <= 0 || radiusY <= 0) {
            return {
                isValid: false,
                error: 'Ellipse must have positive radii',
            };
        }
        ...
        if (params.maxExtension <= 0) {
            return {
                isValid: false,
                error: 'Maximum extension must be positive',
            };
        }
...
export function validateEllipseIntersectionPoint(
    ...
    try {
        ...
        const ellipseValue =
            (localPoint.x * localPoint.x) / (radiusX * radiusX) +
            (localPoint.y * localPoint.y) / (radiusY * radiusY);

        // Distance from ellipse (approximate)
        const distance =
            Math.abs(ellipseValue - 1) * Math.min(radiusX, radiusY);
...
export function processEllipseOperation(
    ...
    try {
        ...
        const pointValidation = validateEllipseIntersectionPoint(
            intersectionPoint,
            ellipse,
            params.tolerance
        );
            ...
            return {
                ...
                errors: [
                    pointValidation.error ||
                        'Intersection point validation failed',
                ],
            };
        }

        // Step 3: Calculate parametric angle of intersection point
        const intersectionAngle = calculateEllipseAngle(
            intersectionPoint,
            ellipse
        );

        // Step 4: Determine extension strategy
        const extensionInfo = determineEllipseExtension(
            ellipse,
            intersectionAngle,
            params
        );
        ...
        const resultGeometry = createEllipticalArcFromEllipse(
            ellipse,
            intersectionAngle,
            extensionInfo
        );
        ...
        return {
            ...
            warnings: [],
            errors: [
                `Ellipse ${operation} failed: ${error instanceof Error ? (error as Error).message : String(error)}`,
            ],
        };
    }
}
 ...
 */
export function calculateEllipseAngle(
    point: Point2D,
    ellipse: Ellipse | SimpleEllipse
): number {
    const { radiusX, radiusY } = extractEllipseProperties(ellipse);
...
export function determineEllipseExtension(
    ...
    try {
        ...
        if (params.extendDirection === 'start') {
            ...
            direction = 'end';
            originalStartAngle =
                intersectionAngle - Math.min(maxAngularExtension, Math.PI / 2);
...
export function createEllipticalArcFromEllipse(
    ...
    const { center, radiusX, radiusY, rotation } =
        extractEllipseProperties(ellipse);
 ...
 */
export function transformToEllipseLocal(
    point: Point2D,
    ellipse: Ellipse | SimpleEllipse
): Point2D {
 ...
 * @returns Validation result with success/failure information
 */
export function validateCircleOperation(
    circle: Circle,
    params: OperationParams
): ValidationResult {
    try {
        ...
        if (!circle.center) {
            return {
                isValid: false,
                error: 'Circle must have a valid center point',
            };
        }
...
export function processCircleOperation(
    ...
    try {
        ...
        // Step 2: Validate intersection point
        const pointValidation = validateCircleIntersectionPoint(
            intersectionPoint,
            circle,
            params.tolerance
        );
        ...
        const intersectionAngle = calculateCircleAngle(
            intersectionPoint,
            circle
        );

        // Step 4: Determine extension strategy
        const extensionInfo = determineCircleExtension(
            circle,
            intersectionAngle,
            params
        );
        ...
        const resultGeometry = createArcFromCircle(
            circle,
            intersectionAngle,
            extensionInfo
        );
        ...
        return {
            ...
            warnings: [],
            errors: [
                `Circle ${operation} failed: ${error instanceof Error ? (error as Error).message : String(error)}`,
            ],
        };
    }
}
...
export function determineCircleExtension(
    ...
    try {
        ...
        const maxAngularExtension = Math.min(
            params.maxExtension / circle.radius,
            2 * Math.PI - EPSILON
        );
        ...
        if (params.extendDirection === 'start') {
            ...
            direction = 'end';
            originalStartAngle =
                intersectionAngle - Math.min(maxAngularExtension, Math.PI);
...
export function createArcFromCircle(
    ...
    const { angularExtension, direction, originalStartAngle }: ExtensionResult =
        extensionInfo;
...
export function processFillExtendResult(
    ...
    const extension = {
        ...
        originalShape: shape,
        extensionStart: getShapePoint(
            shape,
            result.extension.originalStartAngle
        ),
    ...
    if (shape.type === 'circle') {
        ...
        const ellArc: EllipticalArcGeometry =
            result.resultGeometry as EllipticalArcGeometry;
        geometry = {
            center: ellArc.center,
            majorAxisEndpoint: {
                x:
                    ellArc.center.x +
                    ellArc.radiusX * Math.cos(ellArc.rotation || 0),
                y:
                    ellArc.center.y +
                    ellArc.radiusX * Math.sin(ellArc.rotation || 0),
            },
