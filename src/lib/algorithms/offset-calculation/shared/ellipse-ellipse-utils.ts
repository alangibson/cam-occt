...
export function calculateEllipseEllipseIntersection(
  ...
  const intersections = verb.geom.Intersect.curves(curve1, curve2, INTERSECTION_TOLERANCE);
...
export function findEllipseEllipseIntersectionsVerb(
  ...
  const validation = validateEllipseIntersectionParameters(ellipse1, ellipse2);
  if (!validation.isValid) {
    console.warn(`Ellipse intersection validation failed: ${validation.error}`);
    return [];
  }
