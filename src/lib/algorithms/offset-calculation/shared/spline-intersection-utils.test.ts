...
describe('Spline Intersection Utilities', () => {
  ...
  describe('validateSplineForIntersection', () => {
    ...
    it('should reject spline with wrong knot vector length', () => {
      ...
      expect(result.isValid).toBe(false);
      expect(result.error).toBe('Invalid knot vector length: expected 8, got 2');
    });
    ...
    it('should reject degenerate spline with identical control points', () => {
      ...
      expect(result.isValid).toBe(false);
      expect(result.error).toBe('Degenerate spline: all control points are identical');
    });
  ...
  describe('processSplineIntersection', () => {
    it('should find intersections between original splines', async () => {
      ...
      expect(processVerbIntersectionResults).toHaveBeenCalledWith(mockIntersections, false, false);
    });
    ...
    it('should try extensions when no original intersections found', async () => {
      ...
      vi.mocked(processVerbIntersectionResults).mockReturnValue(mockExtendedResults);

      const results = processSplineIntersection(spline1, spline2, false, true, 1000);
    ...
    it('should handle parameter swapping', async () => {
      ...
      expect(processVerbIntersectionResults).toHaveBeenCalledWith(mockIntersections, true, false);
    });
  });
  ...
  describe('processSplineWithCurveIntersection', () => {
    it('should find intersections with other curve', async () => {
      ...
      const results = processSplineWithCurveIntersection(spline, mockOtherCurve);
    ...
    it('should try extensions when original intersection fails', async () => {
      ...
      const results = processSplineWithCurveIntersection(spline, mockOtherCurve, false, true, 1000);

      expect(results).toEqual(mockExtendedResults);
      expect(createExtendedSplineVerb).toHaveBeenCalledWith(spline, true, true, 1000);
    });
    ...
    it('should return empty when extensions fail', async () => {
      ...
      const results = processSplineWithCurveIntersection(spline, mockOtherCurve, false, true);
  ...
  describe('processSplineIntersectionWithRetry', () => {
    it('should return consistent results from multiple retries', async () => {
      ...
      const results = processSplineIntersectionWithRetry(mockSplineCurve, mockOtherCurve, false, false, 3);
    ...
    it('should handle inconsistent results from retries', async () => {
      ...
      const results = processSplineIntersectionWithRetry(mockSplineCurve, mockOtherCurve, false, false);
    ...
    it('should use provided retry count', async () => {
      ...
      processSplineIntersectionWithRetry(mockSplineCurve, mockOtherCurve, false, false, 5);
