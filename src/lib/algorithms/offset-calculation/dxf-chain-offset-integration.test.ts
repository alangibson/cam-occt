...
async function processDxfFile(filename: string, outputDir: string) {
  ...
  shapes = shapes.map(shape => scaleShape(shape, physicalScale, { x: 0, y: 0 }));
  ...
  for (const chain of detectedChains) {
    ...
    const offsetResult: ChainOffsetResult = offsetChain(normalizedChain, offsetDistance, {
      tolerance,
      maxExtension,
      snapThreshold
    });
    
    console.log(`Chain ${chainIndex}: offset result - innerChain: ${offsetResult.innerChain ? 'exists with ' + offsetResult.innerChain.shapes.length + ' shapes' : 'null'}, outerChain: ${offsetResult.outerChain ? 'exists with ' + offsetResult.outerChain.shapes.length + ' shapes' : 'null'}`);

    // Draw inner offsets to inner SVG
    if (offsetResult.innerChain) {
      console.log(`Adding ${offsetResult.innerChain.shapes.length} inner offset shapes to SVG`);
      ...
      if (offsetResult.innerChain.intersectionPoints) {
        offsetResult.innerChain.intersectionPoints.forEach((intersection, idx) => {
          const intersectionCircle: Circle = { center: intersection.point, radius: 5 };
    ...
    if (offsetResult.outerChain) {
      ...
      if (offsetResult.outerChain.intersectionPoints) {
        offsetResult.outerChain.intersectionPoints.forEach((intersection, idx) => {
          const intersectionCircle: Circle = { center: intersection.point, radius: 5 };
          const intersectionShape: Shape = {
  ...
  writeFileSync(join(outputDir, `${baseName}-inner-offsets.svg`), innerSvgContent);
  writeFileSync(join(outputDir, `${baseName}-outer-offsets.svg`), outerSvgContent);
...
describe('DXF Chain Offset Integration Test', () => {
  ...
  it('should load ADLER.dxf, convert to chains, apply offsets, and render to SVG', async () => {
    const { shapes, detectedChains } = await processDxfFile('ADLER.dxf', outputDir);
  ...
  it('should load probleme.dxf, convert to chains, apply offsets, and render to SVG', async () => {
    const { shapes, detectedChains } = await processDxfFile('probleme.dxf', outputDir);
  ...
  it('should load 2013-11-08_test.dxf, convert to chains, apply offsets, and render to SVG', async () => {
    const { shapes, detectedChains } = await processDxfFile('2013-11-08_test.dxf', outputDir);
  ...
  it('should load Tractor Seat Mount - Left.dxf, convert to chains, apply offsets, and render to SVG', async () => {
    const { shapes, detectedChains } = await processDxfFile('Tractor Seat Mount - Left.dxf', outputDir);
    ...
    const { shapes, detectedChains } = await processDxfFile('polylines_with_bulge.dxf', outputDir);
  ...
  it('should load Polylinie.dxf, convert to chains, apply offsets, and render to SVG', async () => {
    const { shapes, detectedChains } = await processDxfFile('Polylinie.dxf', outputDir);
  ...
  it('should load polyline-pill.dxf, convert to chains, apply offsets, and render to SVG', async () => {
    const { shapes, detectedChains } = await processDxfFile('polyline-pill.dxf', outputDir);
  ...
  it('should load chain-pill.dxf, convert to chains, apply offsets, and render to SVG', async () => {
    const { shapes, detectedChains } = await processDxfFile('chain-pill.dxf', outputDir);
