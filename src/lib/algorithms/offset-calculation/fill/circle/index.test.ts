...
describe('fillCircleToIntersection', () => {
  ...
  describe('basic circle to arc conversion', () => {
    it('should convert circle to arc when extended to intersection point', () => {
      ...
      const result = fillCircleToIntersection(circle, intersectionPoint, defaultOptions);
      ...
      expect(pointsApproxEqual(resultArc.center, { x: 0, y: 0 })).toBe(true);
    ...
    it('should preserve circle center and radius', () => {
      ...
      expect(pointsApproxEqual(resultArc.center, { x: 10, y: 20 })).toBe(true);
    });
  });
  ...
  describe('extension direction and angular range', () => {
    it('should create arc with appropriate angular range', () => {
      ...
      let angularRange = Math.abs(resultArc.endAngle - resultArc.startAngle);
  ...
  describe('geometric validation', () => {
    it('should create arc that includes the intersection point', () => {
      ...
      const intersectionPoint = { x: 8 * Math.cos(Math.PI / 3), y: 8 * Math.sin(Math.PI / 3) }; // 60°
    ...
    it('should maintain circle properties in resulting arc', () => {
      ...
      const circle = createCircle(originalCenter.x, originalCenter.y, originalRadius);
      const intersectionPoint = { x: originalCenter.x + originalRadius, y: originalCenter.y };
      ...
      expect(pointsApproxEqual(resultArc.center, originalCenter)).toBe(true);
  ...
  describe('error conditions', () => {
    it('should reject non-circle shapes', () => {
      ...
      const result = fillCircleToIntersection(line, { x: 15, y: 0 }, defaultOptions);
  ...
  describe('special geometric cases', () => {
    ...
    it('should handle circles with large coordinates', () => {
      ...
      validateFillResult(result, true);
      const resultArc = result.extendedShape!.geometry as Arc;
      expect(pointsApproxEqual(resultArc.center, { x: 1000, y: 2000 })).toBe(true);
