...
export function fillEllipseToIntersection(
  ...
  const result = processEllipseOperation(ellipse, intersectionPoint, 'fill', params);
  
  // Process result using shared function
  return processFillExtendResult(result, shape, intersectionPoint, 'fill', 
    (s: Shape, angle: number) => getEllipsePoint(s.geometry as Ellipse, angle));
}
