...
export function fillLineToIntersection(
  ...
  try {
    ...
    const distanceToStart: number = pointDistance(intersectionPoint, line.start);
    const distanceToEnd: number = pointDistance(intersectionPoint, line.end);
    ...
    const extendedLine: Line | null = extendLineToPoint(line, intersectionPoint, {
    ...
    if (!extendedLine) {
      return createFailureResult('Failed to extend line to intersection point');
    }

    // Calculate extension info for the shape extension data
    const extendDirection: 'start' | 'end' | null = determineExtensionDirection(line, intersectionPoint, {
      maxExtension: options.maxExtension,
      tolerance: options.tolerance,
      direction: options.extendDirection === 'start' ? 'start' : 
                 options.extendDirection === 'end' ? 'end' : 'auto'
    ...
    if (!extendDirection) {
      return createFailureResult('Could not determine extension direction');
    }
    ...
    if (!extensionInfo.success) {
      return createFailureResult(extensionInfo.error || 'Extension calculation failed');
    }
    ...
    const extension: ShapeExtension = {
      ...
      originalShape: shape,
      extensionStart: extendDirection === 'start' ? extendedLine.start : line.end,
      extensionEnd: extendDirection === 'start' ? line.start : extendedLine.end
    };
    ...
    return createFailureResult(`Line extension failed: ${error instanceof Error ? (error as Error).message : String(error)}`);
  }
}
