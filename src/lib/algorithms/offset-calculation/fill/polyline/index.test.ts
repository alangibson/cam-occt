...
describe('fillPolylineToIntersection', () => {
  ...
  function createPolyline(vertices: { x: number; y: number; bulge?: number }[], closed: boolean = false): Shape {
    const polylineVertices = vertices.map(v => ({ x: v.x, y: v.y, bulge: v.bulge || 0 }));
    return {
      id: generateId(),
      type: 'polyline',
      geometry: createPolylineFromVertices(polylineVertices, closed).geometry as Polyline
    };
  }
  ...
  describe('basic polyline extension', () => {
    it('should extend polyline at end to intersection point', () => {
      ...
      const result = fillPolylineToIntersection(polyline, intersectionPoint, {
        ...defaultOptions,
        extendDirection: 'end'
      });
  ...
  describe('diagonal and angled polylines', () => {
    it('should extend diagonal polyline correctly', () => {
      ...
      expect(pointsApproxEqual(points[points.length - 1], intersectionPoint)).toBe(true);
    });
  ...
  describe('edge cases and error handling', () => {
    it('should reject non-polyline shapes', () => {
      ...
      const result = fillPolylineToIntersection(line, { x: 15, y: 0 }, defaultOptions);
  ...
  describe('extension information validation', () => {
    it('should provide correct extension metadata', () => {
      ...
      expect(pointsApproxEqual(result.extension!.extensionStart, { x: 10, y: 0 })).toBe(true);
  ...
  describe('geometric validation', () => {
    it('should maintain polyline continuity after extension', () => {
      ...
      expect(pointsApproxEqual(extendedPoint, intersectionPoint)).toBe(true);
      ...
      expect(Math.abs(originalDirection - extensionDirection)).toBeLessThan(1e-6);
    });
    ...
    it('should preserve original polyline properties', () => {
      ...
      for (let i = 0; i < originalVertices.length; i++) {
        expect(pointsApproxEqual(points[i], originalVertices[i])).toBe(true);
      }
  ...
  describe('numerical precision and tolerance', () => {
    ...
    it('should handle tolerance correctly for alignment detection', () => {
      ...
      const result = fillPolylineToIntersection(polyline, slightlyOffPoint, {
        maxExtension: 100,
        tolerance: 1e-8,
        extendDirection: 'end'
      });
  ...
  describe('error conditions', () => {
    ...
    it('should handle single-point polyline', () => {
      ...
      const result = fillPolylineToIntersection(polyline, intersectionPoint, defaultOptions);
