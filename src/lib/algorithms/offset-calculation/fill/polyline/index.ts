...
export function fillPolylineToIntersection(
  ...
  try {
    ...
    const extendedPolyline: Polyline | null = extendPolylineToPoint(polyline, intersectionPoint, {
    ...
    if (!extendedPolyline) {
      return createFailureResult('Failed to extend polyline to intersection point');
    }

    // Calculate extension info for the shape extension data
    const extendDirection: 'start' | 'end' | null = determinePolylineExtensionDirection(polyline, intersectionPoint, {
      maxExtension: options.maxExtension,
      tolerance: options.tolerance,
      direction: options.extendDirection === 'start' ? 'start' : 
                 options.extendDirection === 'end' ? 'end' : 'auto'
    ...
    if (!extendDirection) {
      return createFailureResult('Could not determine polyline extension direction');
    }

    const extensionInfo: PolylineExtensionResult = calculatePolylineExtension(polyline, intersectionPoint, extendDirection, {
      maxExtension: options.maxExtension,
      tolerance: options.tolerance,
      direction: options.extendDirection === 'start' ? 'start' : 
                 options.extendDirection === 'end' ? 'end' : 'auto'
    ...
    if (!extensionInfo.success) {
      return createFailureResult(extensionInfo.error || 'Polyline extension calculation failed');
    }
    ...
    const extension: ShapeExtension = {
      ...
      originalShape: shape,
      extensionStart: (() => {
        const points: import('$lib/types/geometry').Point2D[] = polylineToPoints(polyline);
        return extendDirection === 'start' ? points[0] : points[points.length - 1];
      })(),
    ...
    return createFailureResult(`Polyline extension failed: ${error instanceof Error ? (error as Error).message : String(error)}`);
  }
}
