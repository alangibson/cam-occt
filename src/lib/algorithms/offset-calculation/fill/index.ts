...
import type { FillOptions, FillResult, GapContext, FillStrategy, ShapeExtension } from './types';
...
export function fillGapBetweenShapes(
  ...
  try {
    ...
    const validation: { valid: boolean; error?: string } = validateGapContext(context, options);
    if (!validation.valid) {
      results.shape1Result = createFailureResult(`Validation failed: ${validation.error}`);
      results.shape2Result = createFailureResult(`Validation failed: ${validation.error}`);
      return results;
    }
    ...
    const intersection: Point2D | null = findExtendedIntersection(context, options);
    if (!intersection) {
      results.shape1Result = createFailureResult('No intersection found with extended shapes');
      results.shape2Result = createFailureResult('No intersection found with extended shapes');
      return results;
    }
    ...
    switch (strategy) {
      case 'extend-first':
        results.shape1Result = extendShapeToPoint(context.shape1, intersection, options);
        results.shape2Result = createSuccessResult(context.shape2, null, intersection);
        break;
        
      case 'extend-second':
        results.shape1Result = createSuccessResult(context.shape1, null, intersection);
        results.shape2Result = extendShapeToPoint(context.shape2, intersection, options);
        break;
      ...
      default:
        results.shape1Result = createFailureResult(`Unsupported fill strategy: ${strategy}`);
        results.shape2Result = createFailureResult(`Unsupported fill strategy: ${strategy}`);
    }

  } catch (error) {
    const errorMsg: string = error instanceof Error ? (error as Error).message : String(error);
    results.shape1Result = createFailureResult(`Fill operation failed: ${errorMsg}`);
    results.shape2Result = createFailureResult(`Fill operation failed: ${errorMsg}`);
  }
...
function extendShapeToPoint(
  ...
  switch (shape.type) {
    ...
    case 'polyline':
      return fillPolylineToIntersection(shape, intersectionPoint, options);
    case 'spline':
    ...
    default:
      return createFailureResult(`Unsupported shape type for extension: ${shape.type}`);
  }
}
...
function findExtendedIntersection(
  ...
  try {
    ...
    if (options.preferredIntersection) {
      ...
      let bestDistance: number = pointDistance(intersections[0].point, options.preferredIntersection);
      
      for (let i: number = 1; i < intersections.length; i++) {
        const distance: number = pointDistance(intersections[i].point, options.preferredIntersection);
    ...
    const highConfidenceIntersections: IntersectionResult[] = intersections.filter(i => i.confidence > 0.8);
 ...
 */
function determineFillStrategy(context: GapContext, options: FillOptions): FillStrategy {
