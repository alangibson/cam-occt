...
export function fillSplineToIntersection(
    ...
    try {
        ...
        const extendedSpline: Spline | null = extendSplineToPoint(
            spline,
            intersectionPoint,
            {
        ...
        const extendDirection: 'start' | 'end' | null =
            determineSplineExtensionDirection(spline, intersectionPoint, {
                maxExtension: options.maxExtension,
                tolerance: options.tolerance,
                direction:
                    options.extendDirection === 'start'
        ...
        if (!extensionInfo.success) {
            return createFailureResult(
                extensionInfo.error || 'Spline extension calculation failed'
            );
        }
        ...
        return createFailureResult(
            `Spline extension failed: ${error instanceof Error ? (error as Error).message : String(error)}`
        );
    }
}
