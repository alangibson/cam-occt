...
describe('Spline Fill Operations', () => {
  ...
  describe('fillSplineToIntersection', () => {
    it('should successfully extend spline to intersection point', async () => {
      ...
      const { extendSplineToPoint, determineSplineExtensionDirection, calculateSplineExtension, getSplinePoint } = await import('../../extend/spline');
      ...
      const result: FillResult = fillSplineToIntersection(shape, intersectionPoint, options);
      ...
      expect(result.warnings).toContain('Spline extended using linear approximation from end tangent');
    ...
    it('should successfully extend spline with parametric method giving higher confidence', async () => {
      ...
      vi.mocked(extendSplineToPoint).mockReturnValue(extendedSpline);
      vi.mocked(determineSplineExtensionDirection).mockReturnValue('start');
      vi.mocked(calculateSplineExtension).mockReturnValue({
        success: true,
    ...
    it('should return failure when shape is not a spline', () => {
      ...
      const result: FillResult = fillSplineToIntersection(lineShape, intersectionPoint, options);
    ...
    it('should return failure when spline extension fails', async () => {
      ...
      expect(result.success).toBe(false);
      expect(result.extendedShape).toBeNull();
      expect(result.errors).toContain('Failed to extend spline to intersection point');
      expect(result.confidence).toBe(0.0);
    });
    ...
    it('should return failure when direction determination fails', async () => {
      ...
      const { extendSplineToPoint, determineSplineExtensionDirection } = await import('../../extend/spline');
      ...
      expect(result.success).toBe(false);
      expect(result.extendedShape).toBeNull();
      expect(result.errors).toContain('Could not determine spline extension direction');
      expect(result.confidence).toBe(0.0);
    });
    ...
    it('should return failure when extension calculation fails', async () => {
      ...
      const { extendSplineToPoint, determineSplineExtensionDirection, calculateSplineExtension } = await import('../../extend/spline');
    ...
    it('should handle exceptions gracefully', async () => {
      ...
      expect(result.success).toBe(false);
      expect(result.extendedShape).toBeNull();
      expect(result.errors).toContain('Spline extension failed: Unexpected error during extension');
      expect(result.confidence).toBe(0.0);
    });
    ...
    it('should handle non-Error exceptions', async () => {
      ...
      expect(result.success).toBe(false);
      expect(result.extendedShape).toBeNull();
      expect(result.errors).toContain('Spline extension failed: String error');
      expect(result.confidence).toBe(0.0);
    });
