...
export function generateChainOffsetSVG(
  ...
  const bounds: { minX: number, minY: number, maxX: number, maxY: number } = calculateBounds(chain, offsets);
  ...
  // Write to file
  const outputPath: string = join(process.cwd(), 'tests', 'output', 'visual', 'chain', `${filename}.svg`);
 ...
 */
function aggregateShapeBounds(shapes: Shape[]): { minX: number, minY: number, maxX: number, maxY: number } {
 ...
 * Calculate bounds of all shapes
 */
function calculateBounds(chain: Chain, offsets: OffsetChain[]): { minX: number, minY: number, maxX: number, maxY: number } {
  let minX: number = Infinity, minY: number = Infinity;
  let maxX: number = -Infinity, maxY: number = -Infinity;
 ...
 */
function calculateScale(bounds: { minX: number, minY: number, maxX: number, maxY: number }, options: Required<VisualizationOptions>): number {
 ...
 * Generate grid lines
 */
function generateGrid(bounds: { minX: number, minY: number, maxX: number, maxY: number }, spacing: number): string {
  let grid: string = '\n    <!-- Grid -->\n    <g stroke="#ddd" stroke-width="0.5">';
  
  // Vertical lines
  for (let x: number = Math.floor(bounds.minX / spacing) * spacing; x <= bounds.maxX; x += spacing) {
    grid += `\n      <line x1="${x}" y1="${bounds.minY}" x2="${x}" y2="${bounds.maxY}" />`;
  }
  
  // Horizontal lines
  for (let y: number = Math.floor(bounds.minY / spacing) * spacing; y <= bounds.maxY; y += spacing) {
...
function shapeToSVG(shape: Shape): string {
  switch (shape.type) {
    ...
    case 'polyline': {
      ...
      const points: string = polylineToPoints(polyline).map((p: { x: number; y: number }) => `${p.x},${p.y}`).join(' ');
      return `<polyline points="${points}" />`;
    }
      
    case 'spline': {
      const spline: import("$lib/types/geometry").Spline = shape.geometry as Spline;
      const tessellationResult: { points: { x: number; y: number }[] } = tessellateSpline(spline, { method: 'verb-nurbs', numSamples: 50 });
      const points: string = tessellationResult.points.map(p => `${p.x},${p.y}`).join(' ');
      return `<polyline points="${points}" />`;
    }
      
    case 'ellipse': {
      const ellipse: import("$lib/types/geometry").Ellipse = shape.geometry as Ellipse;
      const points: { x: number; y: number }[] = tessellateEllipse(ellipse, { numPoints: 32 });
