...
describe('Star Offset Parallelism Diagnostic', () => {
  ...
  it('should diagnose star offset parallelism issues', () => {
    ...
    if (outsetResult.success && outsetResult.shapes.length > 0) {
      ...
      for (let i: number = 0; i < originalPoints.length - 1; i++) {
        ...
        const perpDist1 = calculatePerpendicularDistance(offsetStart, originalStart, originalEnd);
        const perpDist2 = calculatePerpendicularDistance(offsetEnd, originalStart, originalEnd);
        
        // Calculate angles to check parallelism
        const originalAngle = Math.atan2(originalEnd.y - originalStart.y, originalEnd.x - originalStart.x);
        const offsetAngle = Math.atan2(offsetEnd.y - offsetStart.y, offsetEnd.x - offsetStart.x);
        let angleDiff = Math.abs(originalAngle - offsetAngle);
        // Normalize to [0, π] range for parallelism check
        if (angleDiff > Math.PI) angleDiff = 2 * Math.PI - angleDiff;
        const isParallel = angleDiff < 0.05 || Math.abs(angleDiff - Math.PI) < 0.05; // Parallel or anti-parallel
        
        // Average perpendicular distance
        const avgPerpDistance = (perpDist1 + perpDist2) / 2;
        
        measurements.push({
          originalSegment: { start: originalStart, end: originalEnd, index: i },
        ...
        svg.addShape(measurementShape, isParallel ? 'green' : 'orange', 0.5);
        
        // Add distance label
        svg.addText(midOffset.x + 2, midOffset.y - 2, 
        ...
        if (!isParallel) {
          svg.addText(midOffset.x + 2, midOffset.y + 8, 
            `${angleDiff.toFixed(1)}°`, 
            'red', 
            '8px'
          );
    ...
    // Save diagnostic SVG
    writeFileSync(join(outputDir, 'star-parallelism-diagnostic.svg'), svg.toString());
    
    // Generate diagnostic report
    const nonParallelCount = measurements.filter(m => !m.isParallel).length;
    const distanceVariation = calculateDistanceVariation(measurements.map(m => m.perpDistance));
    const avgDistance = measurements.reduce((sum, m) => sum + m.perpDistance, 0) / measurements.length;
    
    console.log('=== Star Offset Parallelism Diagnostic ===');
    console.log(`Total segments: ${measurements.length}`);
    console.log(`Non-parallel segments: ${nonParallelCount} (${(nonParallelCount / measurements.length * 100).toFixed(1)}%)`);
    console.log(`Average perpendicular distance: ${avgDistance.toFixed(3)}`);
    console.log(`Distance variation (std dev): ${distanceVariation.toFixed(3)}`);
   ...
   */
  function calculatePerpendicularDistance(point: Point2D, lineStart: Point2D, lineEnd: Point2D): number {
