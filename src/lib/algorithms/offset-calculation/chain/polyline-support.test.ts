...
describe('offsetChain Polyline Support', () => {
  ...
  it('should respect polyline closed flag over geometric analysis', () => {
    ...
    const offsetDistance = 10;
    const result = offsetChain(geometricallyClosedPolyline, offsetDistance, {
      tolerance: 0.05,
      maxExtension: 20,
      snapThreshold: 0.1
    });
  ...
  it('should work identically to chain input for equivalent geometry', () => {
    ...
    if (chainResult.innerChain && polylineResult.innerChain) {
      expect(chainResult.innerChain.shapes.length).toBe(polylineResult.innerChain.shapes.length);
      expect(chainResult.innerChain.closed).toBe(polylineResult.innerChain.closed);
    }

    if (chainResult.outerChain && polylineResult.outerChain) {
      expect(chainResult.outerChain.shapes.length).toBe(polylineResult.outerChain.shapes.length);
      expect(chainResult.outerChain.closed).toBe(polylineResult.outerChain.closed);
    }
  });
  ...
  describe('Side Classification Fix', () => {
    it('should correctly classify inner and outer offsets for polylines_with_bulge.dxf', async () => {
      ...
      const dxfContent = readFileSync('tests/dxf/polylines_with_bulge.dxf', 'utf-8');
      ...
      for (const chain of detectedChains) {
        ...
        const offsetResult = offsetChain(normalizedChain, offsetDistance, {
          tolerance,
      ...
      for (let i: number = 0; i < results.length; i++) {
        ...
        const hasInner = result.innerChain !== undefined && result.innerChain.shapes.length > 0;
        const hasOuter = result.outerChain !== undefined && result.outerChain.shapes.length > 0;
        
        expect(hasInner || hasOuter, `Chain ${i + 1} should have at least one offset`).toBe(true);
        
        // For closed chains with bulges, we expect both inner and outer offsets
        if (hasInner && hasOuter) {
          expect(result.innerChain!.shapes.length, `Chain ${i + 1} inner offset should have shapes`).toBeGreaterThan(0);
          expect(result.outerChain!.shapes.length, `Chain ${i + 1} outer offset should have shapes`).toBeGreaterThan(0);
          expect(result.innerChain!.side, `Chain ${i + 1} inner chain should be classified as inner`).toBe('inner');
          expect(result.outerChain!.side, `Chain ${i + 1} outer chain should be classified as outer`).toBe('outer');
        }
      }
      
      // Overall validation: we should have inner offsets for all chains that support them
      const totalInnerChains = results.filter(r => r.innerChain !== undefined).length;
      const totalOuterChains = results.filter(r => r.outerChain !== undefined).length;
      
      expect(totalInnerChains, 'Should have inner offset chains').toBeGreaterThan(0);
      expect(totalOuterChains, 'Should have outer offset chains').toBeGreaterThan(0);
    });
    ...
    it('should never classify both offsets of a closed shape as the same side', async () => {
      ...
      for (const chain of detectedChains) {
        ...
        const innerShapeCount = offsetResult.innerChain?.shapes.length || 0;
        const outerShapeCount = offsetResult.outerChain?.shapes.length || 0;
        ...
        if (totalShapes > 1) {
          ...
          expect(innerShapeCount, 'Inner chain should have shapes when total > 1').toBeGreaterThan(0);
          expect(outerShapeCount, 'Outer chain should have shapes when total > 1').toBeGreaterThan(0);
        }
      }
    });
  });
