...
describe('offset chain connectivity validation', () => {
  ...
  function createArc(cx: number, cy: number, radius: number, startAngle: number, endAngle: number): Shape {
    return {
      id: generateId(),
      type: 'arc',
      geometry: {
  ...
  function getShapeEndpoint(shape: Shape, isStart: boolean): { x: number, y: number } {
    switch (shape.type) {
      case 'line': {
  ...
  function validateOffsetChainConnectivity(offsetChain: OffsetChain, tolerance: number = 0.1): {
    isConnected: boolean;
    gaps: Array<{
      shapeIndex1: number;
      shapeIndex2: number;
    ...
    for (let i: number = 0; i < offsetChain.shapes.length; i++) {
      ...
      const nextIndex = offsetChain.closed ? (i + 1) % offsetChain.shapes.length : i + 1;
  ...
  it('should detect connectivity gaps in L-shaped offset chains', () => {
    ...
    const validation = validateOffsetChainConnectivity(brokenRightOffset, 0.1);
  ...
  it('should validate properly connected L-shaped offset chains', () => {
    ...
    const validation = validateOffsetChainConnectivity(correctRightOffset, 0.1);
  ...
  it('should detect connectivity gaps in rounded rectangle chains', () => {
    ...
    const validation = validateOffsetChainConnectivity(brokenInnerOffset, 0.1);
    
    expect(validation.isConnected).toBe(false);
    expect(validation.gaps.length).toBeGreaterThan(0);
    
    // Should detect the gap between arc end (90, 20) and line start (95, 20)
    const arcToLineGap = validation.gaps.find(gap => 
      gap.shapeIndex1 === 1 && gap.shapeIndex2 === 2
    );
