...
describe('Spline-Line Offset Intersection Instability', () => {
    ...
    it('should reproduce the exact offset intersection instability', () => {
        ...
        const splineOffsetResults: {
            insetSuccess: boolean;
            outsetSuccess: boolean;
            insetShapes: number;
            outsetShapes: number;
        }[] = [];
        const lineOffsetResults: {
            insetSuccess: boolean;
            outsetSuccess: boolean;
            insetShapes: number;
            outsetShapes: number;
        ...
        for (let run = 0; run < 10; run++) {
            ...
            if (
                splineInset.success &&
                lineInset.success &&
                splineInset.shapes.length > 0 &&
                lineInset.shapes.length > 0
            ) {
            ...
            if (
                splineOutset.success &&
                lineOutset.success &&
                splineOutset.shapes.length > 0 &&
                lineOutset.shapes.length > 0
            ) {
                const intersections = findShapeIntersections(
            ...
            console.log(
                `Run ${run + 1}: ${totalIntersections} total intersections`,
                runResults
            );
        }
        ...
        const splineOffsetsConsistent = splineOffsetResults.every(
            (result) => JSON.stringify(result) === firstSplineResult
        );

        const firstLineResult = JSON.stringify(lineOffsetResults[0]);
        const lineOffsetsConsistent = lineOffsetResults.every(
            (result) => JSON.stringify(result) === firstLineResult
        );
        ...
        const pairKeys = [
            'splineInset_lineInset',
            'splineOutset_lineOutset',
            'splineInset_lineOutset',
            'splineOutset_lineInset',
        ];
        ...
        for (const key of pairKeys) {
            ...
            console.log(
                `${key}: ${isConsistent ? 'CONSISTENT' : 'INCONSISTENT'} - counts: [${counts.join(', ')}]`
            );
        }

        // Check if intersection counts are consistent
        const firstCount = intersectionCounts[0];
        const intersectionCountsConsistent = intersectionCounts.every(
            (count) => count === firstCount
        );

        console.log('Intersection counts:', intersectionCounts);
        console.log(
            'Intersection counts consistent:',
            intersectionCountsConsistent
        );

        // Report findings
        if (!splineOffsetsConsistent) {
            console.log(
                'INSTABILITY FOUND: Spline offset generation is non-deterministic'
            );
        }
        if (!lineOffsetsConsistent) {
            console.log(
                'INSTABILITY FOUND: Line offset generation is non-deterministic'
            );
        }
        if (
            splineOffsetsConsistent &&
            lineOffsetsConsistent &&
            !intersectionCountsConsistent
        ) {
            console.log(
                'INSTABILITY FOUND: Intersection detection between offset shapes is non-deterministic'
            );
        }
    ...
    it('should debug the specific problematic intersection pair', () => {
        ...
        console.log(
            'Offset spline geometry:',
            JSON.stringify(offsetSpline.geometry, null, 2)
        );
        console.log(
            'Offset line geometry:',
            JSON.stringify(offsetLine.geometry, null, 2)
        );
        ...
        for (let run = 0; run < 20; run++) {
            ...
            console.log(
                `Run ${run + 1}: ${intersections.length} intersections`
            );
    ...
    it(
        'should test side detection consistency for offset shapes',
        { timeout: 30000 },
        () => {
            const chain = {
                id: 'test-chain',
            ...
            for (let run = 0; run < 5; run++) {
                ...
                if (splineInset.success && splineInset.shapes.length > 0) {
                    const sideResult = detectChainSide(
                        splineInset.shapes[0],
                        -offsetDistance,
                        chain,
                        0.1,
            ...
            const sideDetectionConsistent = sideDetectionResults.every(
                (result) => {
                    return (
                        result.splineInsetSide ===
                            firstResult.splineInsetSide &&
                        result.splineOutsetSide ===
                            firstResult.splineOutsetSide &&
            ...
            if (!sideDetectionConsistent) {
                console.log(
                    'INSTABILITY FOUND: Side detection for offset shapes is non-deterministic'
                );
            }
    ...
    it('should test individual intersection detection stability', () => {
        ...
        console.log(
            'Testing intersection detection between fixed offset shapes...'
        );
        ...
        const intersectionConsistent = intersectionResults.every(
            (count) => count === firstCount
        );

        console.log(
            'Intersection detection consistent:',
            intersectionConsistent
        );

        if (!intersectionConsistent) {
            console.log(
                'INSTABILITY FOUND: Intersection detection between fixed offset shapes is non-deterministic'
            );
