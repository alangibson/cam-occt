...
import { validateSplineGeometry, type SplineValidationResult } from "$lib/utils/spline-validation-utils";
import type { Line, Arc, Circle, Polyline, Spline, Ellipse } from "$lib/types/geometry";
...
import { DEFAULT_CHAIN_OFFSET_PARAMETERS, type ChainOffsetParameters, type ChainOffsetResult, type IntersectionResult, type OffsetChain, type Shape, type TrimPoint, type GapFillingResult } from "./types";
...
function processAndValidateOffsetShape(
  ...
  if (offsetShape.type === 'spline') {
    const validation: SplineValidationResult = validateSplineGeometry(offsetShape.geometry as Spline);
...
export function offsetChain(
  ...
  try {
    ...
    const allOffsets: Array<{shape: Shape, originalIndex: number, offset: number}> = generateAllOffsets(chain, distance);
    ...
    if ('closed' in chain && typeof (chain as Polyline).closed === 'boolean') {
    ...
    const groupedOffsets: {inner: Shape[], outer: Shape[], left: Shape[], right: Shape[]} = groupOffsetsBySide(allOffsets, chain, params.tolerance, isClosed);
    ...
    if (isClosed) {
      ...
      if (groupedOffsets.inner.length > 0) {
        result.innerChain = processOffsetGroup(groupedOffsets.inner, chain, 'inner', params, isClosed);
      }
      if (groupedOffsets.outer.length > 0) {
        result.outerChain = processOffsetGroup(groupedOffsets.outer, chain, 'outer', params, isClosed);
      }
    } else {
      // For open chains: left and right
      if (groupedOffsets.left.length > 0) {
        result.innerChain = processOffsetGroup(groupedOffsets.left, chain, 'left', params, isClosed);
      }
      if (groupedOffsets.right.length > 0) {
        result.outerChain = processOffsetGroup(groupedOffsets.right, chain, 'right', params, isClosed);
      }
    }
    ...
    const totalGapsFilled: number = (result.innerChain?.gapFills?.length || 0) + 
                           (result.outerChain?.gapFills?.length || 0);
    const totalIntersections: number = (result.innerChain?.intersectionPoints?.length || 0) + 
 ...
 */
export function generateAllOffsets(chain: Chain, distance: number): Array<{shape: Shape, originalIndex: number, offset: number}> {
  const allOffsets: Array<{shape: Shape, originalIndex: number, offset: number}> = [];
  ...
  for (let i: number = 0; i < chain.shapes.length; i++) {
    ...
    if (shape.type === 'arc' && (shape.geometry as Arc).clockwise === true) {
    ...
    const positiveResult: { success: boolean; shapes: Shape[] } = offsetShape(shape, Math.abs(distance), positiveDirection);
    const negativeResult: { success: boolean; shapes: Shape[] } = offsetShape(shape, Math.abs(distance), negativeDirection);
    ...
    if (positiveResult.success && positiveResult.shapes.length > 0) {
      const offsetData = processAndValidateOffsetShape(
        positiveResult.shapes[0], i, Math.abs(distance)
      );
    ...
    if (negativeResult.success && negativeResult.shapes.length > 0) {
      const offsetData = processAndValidateOffsetShape(
        negativeResult.shapes[0], i, -Math.abs(distance)
      );
      allOffsets.push(offsetData);
    }
  }
...
export function groupOffsetsBySide(
  ...
  const groups: {inner: Shape[], outer: Shape[], left: Shape[], right: Shape[]} = {
    inner: [] as Shape[],
    outer: [] as Shape[],
    left: [] as Shape[],
    right: [] as Shape[]
  ...
  const offsetsByShape: Map<number, Array<{shape: Shape, originalIndex: number, offset: number}>> = new Map<number, Array<{shape: Shape, originalIndex: number, offset: number}>>();
  ...
  for (const [, shapeOffsets] of offsetsByShape) {
    ...
    if (shapeOffsets.length >= 2) {
      ...
      const positiveOffset: {shape: Shape, originalIndex: number, offset: number} | undefined = shapeOffsets.find(o => o.offset > 0);
      const negativeOffset: {shape: Shape, originalIndex: number, offset: number} | undefined = shapeOffsets.find(o => o.offset < 0);
      
      if (positiveOffset && negativeOffset) {
        // Classify based on spatial detection, but ensure they go to different sides
        const positiveSide: { side: 'inner' | 'outer' | 'left' | 'right'; confidence: number } = detectChainSide(positiveOffset.shape, positiveOffset.offset, chain, tolerance, isClosed);
        const negativeSide: { side: 'inner' | 'outer' | 'left' | 'right'; confidence: number } = detectChainSide(negativeOffset.shape, negativeOffset.offset, chain, tolerance, isClosed);
        ...
        if (isClosed && positiveSide.side === negativeSide.side) {
          if (positiveSide.confidence >= negativeSide.confidence) {
            ...
            const flippedNegativeSide: 'inner' | 'outer' = positiveSide.side === 'inner' ? 'outer' : 'inner';
            addToGroup(groups, positiveOffset.shape, positiveSide.side);
            addToGroup(groups, negativeOffset.shape, flippedNegativeSide);
          } else {
            // Keep negative side, flip positive to opposite
            const flippedPositiveSide: 'inner' | 'outer' = negativeSide.side === 'inner' ? 'outer' : 'inner';
            addToGroup(groups, positiveOffset.shape, flippedPositiveSide);
      ...
      const offsetData: {shape: Shape, originalIndex: number, offset: number} = shapeOffsets[0];
      const sideResult: { side: 'inner' | 'outer' | 'left' | 'right'; confidence: number } = detectChainSide(offsetData.shape, offsetData.offset, chain, tolerance, isClosed);
...
function addToGroup(groups: {inner: Shape[], outer: Shape[], left: Shape[], right: Shape[]}, shape: Shape, side: 'inner' | 'outer' | 'left' | 'right') {
  switch (side) {
    case 'inner':
...
function collectIntersections(
  ...
  allIntersections: Array<{index1: number, index2: number, intersections: IntersectionResult[]}>,
  intersectionPoints: IntersectionResult[]
} {
  const allIntersections: Array<{index1: number, index2: number, intersections: IntersectionResult[]}> = [];
  ...
  for (let i: number = 0; i < shapes.length - 1; i++) {
    ...
    const intersections: IntersectionResult[] = findShapeIntersections(shape1, shape2, params.tolerance, true, params.maxExtension, params.intersectionType || 'infinite');
  ...
  if (isClosed && shapes.length > 2) {
    ...
    const intersections: IntersectionResult[] = findShapeIntersections(lastShape, firstShape, params.tolerance, true, params.maxExtension, params.intersectionType || 'infinite');
  ...
  if (params.polylineIntersections) {
    for (let i: number = 0; i < shapes.length; i++) {
      ...
      if (shape.type === 'polyline') {
        const selfIntersections: IntersectionResult[] = findPolylineSelfIntersections(shape, params.intersectionType);
...
function applyTrimming(
  ...
  for (const { index1, index2, intersections } of allIntersections) {
    if (intersections.length > 0) {
      ...
      const trimResult: { shape1Result: TrimResult; shape2Result: TrimResult } = trimConsecutiveShapes(shape1, shape2, intersections, params.tolerance);

      if (trimResult.shape1Result.success && trimResult.shape2Result.success) {
        // Calculate trim amounts before replacing shapes
        const trim1Amount: number = calculateTrimAmount(shape1, trimResult.shape1Result.shape!);
        const trim2Amount: number = calculateTrimAmount(shape2, trimResult.shape2Result.shape!);
        ...
        trimPoints.push({
          ...
          trim1Amount,
          trim2Amount,
          cornerType: intersection.type === 'tangent' ? 'tangent' : 'sharp'
        });
      } else {
        // Trimming failed
        continuous = false;
...
function applyGapFilling(
  ...
  for (let i: number = 0; i < shapes.length - 1; i++) {
    ...
    const gapInfo: { hasGap: boolean; gapSize: number; endpoint1: { x: number; y: number }; startpoint2: { x: number; y: number } } = detectGap(shape1, shape2, params.tolerance);
    if (gapInfo.hasGap && gapInfo.gapSize <= params.maxExtension) {
      ...
      const fillResult: { shape1Result: FillResult; shape2Result: FillResult } = fillGapBetweenShapes(gapContext, {
        maxExtension: params.maxExtension,
        tolerance: params.tolerance,
        extendDirection: 'auto'
      });
      
      if (fillResult.shape1Result.success && fillResult.shape2Result.success) {
        ...
        gapFills.push({
          ...
          gapLocation: {
            ...
            shape2Index: i + 1,
            point: {
              x: (gapInfo.endpoint1.x + gapInfo.startpoint2.x) / 2,
              y: (gapInfo.endpoint1.y + gapInfo.startpoint2.y) / 2
            }
          }
        });
      } else {
  ...
  if (isClosed && shapes.length > 2) {
    ...
    const gapInfo: { hasGap: boolean; gapSize: number; endpoint1: { x: number; y: number }; startpoint2: { x: number; y: number } } = detectGap(lastShape, firstShape, params.tolerance);
    if (gapInfo.hasGap && gapInfo.gapSize <= params.maxExtension) {
      const gapContext: GapContext = {
        shape1: lastShape,
        shape2: firstShape,
      ...
      if (fillResult.shape1Result.success && fillResult.shape2Result.success) {
        processedShapes[shapes.length - 1] = fillResult.shape1Result.extendedShape!;
        ...
        gapFills.push({
          ...
          modifiedShapes: [
            { original: lastShape, modified: processedShapes[shapes.length - 1] },
...
function detectGap(
  ...
  const startpoint2: { x: number; y: number } = getShapeEndpoint(shape2, 'start');
 ...
 */
function getShapeEndpoint(shape: Shape, endpoint: 'start' | 'end'): { x: number; y: number } {
  switch (shape.type) {
    case 'line':
    ...
    case 'arc':
      ...
      const angle: number = endpoint === 'start' ? arc.startAngle : arc.endAngle;
      return {
    ...
    case 'polyline':
      ...
      const points: Array<{ x: number; y: number }> = polylineToPoints(polyline);
    ...
    case 'spline':
      ...
      const controlPoints: Array<{ x: number; y: number }> = spline.controlPoints || [];
      if (controlPoints.length === 0) return { x: 0, y: 0 };
      return endpoint === 'start' ? controlPoints[0] : controlPoints[controlPoints.length - 1];
    case 'ellipse':
 ...
 */
function calculateTrimAmount(originalShape: Shape, trimmedShape: Shape): number {
  try {
    switch (originalShape.type) {
      ...
      case 'arc': {
        ...
        const originalAngleRange: number = Math.abs(original.endAngle - original.startAngle);
        const trimmedAngleRange: number = Math.abs(trimmed.endAngle - trimmed.startAngle);
        
        const arcLengthDiff: number = Math.abs(originalAngleRange - trimmedAngleRange) * original.radius;
      ...
      case 'polyline': {
        ...
        const originalPoints: Array<{ x: number; y: number }> = polylineToPoints(original);
        const trimmedPoints: Array<{ x: number; y: number }> = polylineToPoints(trimmed);
        const pointCountDiff: number = Math.abs(originalPoints.length - trimmedPoints.length);
...
function processOffsetGroup(
  ...
  try {
    ...
    const { allIntersections, intersectionPoints }: { allIntersections: Array<{index1: number, index2: number, intersections: IntersectionResult[]}>; intersectionPoints: IntersectionResult[] } = collectIntersections(shapes, originalChain, params, isClosed);
    
    // Step 2: Apply trimming to create sharp corners
    const trimmingResult: { processedShapes: Shape[]; trimPoints: TrimPoint[]; continuous: boolean } = applyTrimming(processedShapes, allIntersections, params);
    ...
    const gapFillingResult: { processedShapes: Shape[]; gapFills: GapFillingResult[]; continuous: boolean } = applyGapFilling(processedShapes, params, isClosed);
