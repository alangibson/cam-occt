...
import type { Shape, Line, Arc, Circle, Point2D, Polyline, Ellipse, Spline } from '../../../../lib/types/geometry';
...
describe('intersections', () => {
  ...
  function pointsApproxEqual(p1: Point2D, p2: Point2D, tolerance = EPSILON): boolean {
  ...
  function checkIntersection(
    ...
    expect(pointsApproxEqual(result.point, expectedPoint, tolerance)).toBe(true);
  ...
  describe('findLineLineIntersections', () => {
    ...
    it('should find intersection at line endpoints', () => {
      ...
      const line2: Line = { start: { x: 10, y: 0 }, end: { x: 10, y: 10 } };
    ...
    it('should handle collinear overlapping lines', () => {
      ...
      const overlapPoints = result.map(r => r.point.x).sort((a, b) => a - b);
    ...
    it('should handle collinear non-overlapping lines', () => {
      ...
      const line2: Line = { start: { x: 10, y: 0 }, end: { x: 15, y: 0 } };
    ...
    it('should handle near-parallel lines', () => {
      const line1: Line = { start: { x: 0, y: 0 }, end: { x: 10, y: 0 } };
      const line2: Line = { start: { x: 0, y: 0.0001 }, end: { x: 10, y: 0.0002 } };
      ...
      if (result.length > 0) {
        ...
        expect(pointsApproxEqual(result[0].point, { x: -10, y: 0 }, 1e-3)).toBe(true);
      }
    });
  });
  ...
  describe('clusterIntersections', () => {
    it('should cluster nearby intersection points', () => {
      ...
      expect(pointsApproxEqual(result[0].point, { x: 1.00005, y: 0.99995 }, 1e-4)).toBe(true);
      expect(pointsApproxEqual(result[1].point, { x: 5.0, y: 5.0 }, EPSILON)).toBe(true);
    });
  ...
  describe('new intersection types', () => {
    ...
    function createPolyline(points: Array<{x: number, y: number, bulge?: number}>, closed: boolean = false): Shape {
      const vertices = points.map(p => ({ x: p.x, y: p.y, bulge: p.bulge ?? 0 }));
      return createPolylineFromVertices(vertices, closed, { id: generateId() });
    }

    // Helper function to create ellipse
    function createEllipse(cx: number, cy: number, majorX: number, majorY: number, ratio: number): Shape {
      return {
        id: generateId(),
        type: 'ellipse',
        geometry: {
    ...
    function createSpline(controlPoints: Array<{x: number, y: number}>, degree: number = 3, closed: boolean = false): Shape {
      return {
        id: generateId(),
        type: 'spline',
        geometry: {
    ...
    it('should find polyline-line intersections', () => {
      ...
      expect(y === 0 || y === 10 || Math.abs(y) < 0.1 || Math.abs(y - 10) < 0.1).toBe(true);
    });
