...
describe('overlapping lines validation', () => {
  ...
  function validateNoOverlappingLines(offsetChain: OffsetChain, tolerance: number = 0.1): {
    hasOverlaps: boolean;
    overlaps: Array<{
      shapeIndex1: number;
      shapeIndex2: number;
    ...
    const lines = offsetChain.shapes.filter(shape => shape.type === 'line');
    ...
    for (let i: number = 0; i < lines.length; i++) {
      for (let j: number = i + 1; j < lines.length; j++) {
        ...
        if (Math.abs(line1.start.x - line2.start.x) < tolerance &&
        ...
        else if (Math.abs(line1.start.x - line1.end.x) < tolerance && // line1 is vertical
                 Math.abs(line2.start.x - line2.end.x) < tolerance && // line2 is vertical
                 Math.abs(line1.start.x - line2.start.x) < tolerance) { // same x coordinate
        ...
        else if (Math.abs(line1.start.y - line1.end.y) < tolerance && // line1 is horizontal
                 Math.abs(line2.start.y - line2.end.y) < tolerance && // line2 is horizontal
                 Math.abs(line1.start.y - line2.start.y) < tolerance) { // same y coordinate
  ...
  it('should detect when offset lines coincide with original chain lines', () => {
    ...
    const correctValidation = validateNoOverlappingLines(correctLeftOffset, 0.1);
