...
describe('intersections-spline-verb', () => {
  ...
  function expectPointNear(actual: Point2D, expected: Point2D, tolerance = 1e-6) {
  ...
  describe('findSplineLineIntersectionsVerb', () => {
    it('should find intersection between straight spline and horizontal line', () => {
      ...
      const spline: import("$lib/types/geometry").Spline = createTestSpline([
        { x: 0, y: 0 },
        { x: 2, y: 2 }
      ], 1, [0, 0, 1, 1]);
    ...
    it('should handle parameter swapping', () => {
      ...
      if (normalResult.length > 0 && swappedResult.length > 0) {
        expect(normalResult[0].param1).toBeCloseTo(swappedResult[0].param2, 6);
        expect(normalResult[0].param2).toBeCloseTo(swappedResult[0].param1, 6);
      }
    });
  });
  ...
  describe('findSplinePolylineIntersectionsVerb', () => {
    ...
    it('should handle closed polyline with closing segment', () => {
      ...
      if (result.length >= 2) {
        ...
        const sortedResults = result.sort((a, b) => a.point.x - b.point.x);
  ...
  describe('edge cases and robustness', () => {
    it('should handle splines with different weight configurations', () => {
      ...
      const splineWithWeights = createTestSpline(controlPoints, 2, undefined, [1, 2, 1]);
    ...
    it('should handle various spline degrees', () => {
      ...
      const line: Line = { start: { x: 0, y: 0.5 }, end: { x: 3, y: 0.5 } };

      // Test different degrees
      [1, 2, 3].forEach(degree => {
        const spline: import("$lib/types/geometry").Spline = createTestSpline(baseControlPoints, degree);
