...
export function detectChainSide(
  ...
  const opts: Required<SideDetectionOptions> = { ...DEFAULT_SIDE_DETECTION_OPTIONS, ...options };
...
function detectClosedChainSide(
  ...
  const confidence: number = Math.abs(insideCount - options.sampleCount / 2) / (options.sampleCount / 2);
...
function detectOpenChainSide(
  ...
  for (let i: number = 0; i < options.sampleCount; i++) {
    ...
    const closestChainInfo: { point: Point2D; shapeIndex: number; t: number; distance: number; } | null = findClosestPointOnChain(offsetPoint, chain);
    
    if (closestChainInfo) {
      // Get the chain direction at the closest point
      const chainDirection: Point2D = getChainDirectionAt(chain, closestChainInfo.shapeIndex, closestChainInfo.t);
      ...
      const crossProd: number = crossProduct2D(chainDirection, toOffsetVector);
      crossProducts.push(crossProd);
    }
  }
  
  // Determine consensus from all sample points
  const leftCount: number = crossProducts.filter(cp => cp > tolerance).length;
  const rightCount: number = crossProducts.filter(cp => cp < -tolerance).length;
  const confidence: number = Math.abs(leftCount - rightCount) / options.sampleCount;
 ...
 */
function findClosestPointOnChain(point: Point2D, chain: Chain): {
  point: Point2D;
  shapeIndex: number;
  t: number;
  distance: number;
} | null {
  let closestInfo: { point: Point2D; shapeIndex: number; t: number; distance: number } | null = null;
  let minDistance: number = Infinity;
  
  for (let shapeIndex: number = 0; shapeIndex < chain.shapes.length; shapeIndex++) {
 ...
 */
function getChainDirectionAt(chain: Chain, shapeIndex: number, t: number): Point2D {
  const shape: Shape = chain.shapes[shapeIndex];
