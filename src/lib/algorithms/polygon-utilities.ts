...
import { calculateSignedArea, getWindingDirection, type WindingDirection } from '../utils/geometry-utils';
import { calculatePerimeter } from '../utils/math-utils';
import { calculateDistanceBetweenPoints, isPointInPolygon } from '../utils/polygon-geometry-shared';
 ...
 */
export function analyzePolygon(points: Point2D[], config: PolygonAnalysisConfig): PolygonAnalysisResult {
  if (points.length < 3) {
  ...
  const distanceToFirst = calculateDistanceBetweenPoints(firstPoint, lastPoint);
  const isClosed = distanceToFirst < config.tolerance ||
    (config.treatNearlyClosedAsClosed === true && distanceToFirst < config.tolerance * 10);
  ...
  let minX: number = points[0].x, maxX: number = points[0].x;
  let minY: number = points[0].y, maxY: number = points[0].y;
...
export function classifyPolygonsAsShellsAndHoles(
  ...
  const analyses: Array<{ points: Point2D[]; analysis: PolygonAnalysisResult }> = polygons.map(points => ({
    points,
    analysis: analyzePolygon(points, config)
  }));

  // Separate by winding direction
  const clockwisePolygons: Array<{ points: Point2D[]; analysis: PolygonAnalysisResult }> = analyses.filter(p => p.analysis.winding === 'CW');
  const counterClockwisePolygons: Array<{ points: Point2D[]; analysis: PolygonAnalysisResult }> = analyses.filter(p => p.analysis.winding === 'CCW');
  ...
  for (const shell of clockwisePolygons) {
    ...
    for (const hole of counterClockwisePolygons) {
      if (isPolygonInsidePolygon(hole.points, shell.points, config.tolerance)) {
...
export function simplifyPolygon(
  ...
  for (let i: number = 0; i < points.length; i++) {
    ...
    const prev: Point2D = simplified[simplified.length - 1] || points[points.length - 1];
    ...
    if (simplified.length > 0 && i < points.length - 1) {
      if (arePointsCollinear(prev, current, next, config.collinearityTolerance)) {
        continue;
      }
    }
...
export function calculateConvexHull(points: Point2D[]): Point2D[] {
  ...
  for (const point of sorted) {
    ...
    while (hull.length > 1) {
      ...
      const cross: number = (p2.x - p1.x) * (point.y - p1.y) - (p2.y - p1.y) * (point.x - p1.x);
