 ...
 */
export function isAngleInArcRange(angle: number, startAngle: number, endAngle: number, clockwise: boolean): boolean {
  ...
  if (clockwise) {
    if (normStart > normEnd) {
      ...
      return normAngle >= normEnd - EPSILON && normAngle <= normStart + EPSILON;
    } else {
      ...
      return normAngle >= normStart - EPSILON || normAngle <= normEnd + EPSILON;
    }
  } else {
    if (normStart <= normEnd) {
      return normAngle >= normStart - EPSILON && normAngle <= normEnd + EPSILON;
...
export function isPointOnArc(point: Point2D, arc: Arc): boolean {
  ...
  const angle: number = Math.atan2(point.y - center.y, point.x - center.x);
  const inRange: boolean = isAngleInArcRange(angle, startAngle, endAngle, clockwise);
 ...
 */
export function processArcIntersectionResults(results: IntersectionResult[]): IntersectionResult[] {
