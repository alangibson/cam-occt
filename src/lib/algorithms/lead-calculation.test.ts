...
describe('calculateLeads', () => {
  ...
  describe('arc leads', () => {
    it('should calculate arc lead-in for horizontal line', () => {
      ...
      const lastPoint = result.leadIn?.points[result.leadIn.points.length - 1];
  ...
  describe('part context (holes vs shells)', () => {
    it('should place lead inside for holes', () => {
      ...
      const part: DetectedPart = {
        id: 'part1',
        shell: { id: 'shell1', chain: shellChain, type: 'shell', boundingBox: { minX: 0, maxX: 10, minY: 0, maxY: 10 }, holes: [] },
