...
describe('Layer Squashing Algorithm', () => {
  it('should load Tractor Seat Mount - Left.dxf, squash layers, and detect 13 chains', async () => {
    ...
    const squashedDrawing = squashLayers(originalDrawing, { tolerance: 0.1 });
    ...
    console.log(`Shape count reduction: ${originalShapeCount} -> ${finalShapeCount} (eliminated ${originalShapeCount - finalShapeCount} duplicates)`);
    ...
    const chains = detectShapeChains(squashedDrawing.shapes, { tolerance: 0.1 });
    
    console.log(`Detected ${chains.length} chains after layer squashing`);
    console.log('Chain details:', chains.map(chain => ({
      id: chain.id,
  ...
  it('should eliminate duplicate shapes when squashing layers', async () => {
    ...
    console.log(`Eliminated duplicates: ${originalStats.totalShapes - squashedShapeCount}`);
  ...
  it('should preserve layer info when requested', async () => {
    ...
    const originalDrawing = await parseDXF(dxfContent, { squashLayers: false });
    const squashedDrawing = squashLayers(originalDrawing, { preserveLayerInfo: true });
    
    // If file has no separate layers, layer preservation should still work gracefully
    if (originalDrawing.layers && Object.keys(originalDrawing.layers).length > 0) {
    ...
    const shapesWithLayerInfo = squashedDrawing.shapes.filter(shape => 
      shape.metadata?.originalLayer || shape.layer
    );
