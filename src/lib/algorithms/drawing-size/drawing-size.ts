...
export function calculateDrawingSize(drawing: Drawing | null): DrawingSize | null {
  ...
  const isValidBounds: boolean = bounds && 
  ...
  console.log('DXF bounds are invalid, falling back to custom bounding box calculation');
  
  try {
    const calculatedBounds: BoundingBox = getBoundingBoxForShapes(drawing.shapes);
    return {
    ...
    const errorMessage: string = error instanceof Error ? (error as Error).message : String(error);
    throw new Error(`Failed to calculate bounding box from shapes: ${errorMessage}`);
  }
}
