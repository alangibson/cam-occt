...
describe('Lead Solid Area Avoidance', () => {
  ...
  function isPointInSolidArea(point: Point2D, part: DetectedPart): boolean {
    ...
    if (!isPointInRectangle(point, 
      shell.boundingBox.minX, shell.boundingBox.minY,
      shell.boundingBox.maxX, shell.boundingBox.maxY)) {
      return false;
    }
    ...
    for (const hole of part.holes) {
      if (isPointInRectangle(point,
        hole.boundingBox.minX, hole.boundingBox.minY,
        hole.boundingBox.maxX, hole.boundingBox.maxY)) {
        return false;
      }
    }
  ...
  describe('solid area avoidance tests', () => {
    ...
    it('should handle complex part with multiple well-spaced holes', () => {
      ...
      const part: DetectedPart = {
        ...
        holes: [
          ...
          { 
            ...
            chain: hole2Chain, 
            type: 'hole', 
            boundingBox: { minX: 120, maxX: 160, minY: 30, maxY: 70 }, 
            holes: [] 
          }
        ]
      };
      ...
      for (const point of points1) {
        ...
        if (Math.abs(point.x - connectionPoint1.x) < 0.001 && 
      ...
      for (const point of points2) {
        // Skip connection point
        if (Math.abs(point.x - connectionPoint2.x) < 0.001 && 
