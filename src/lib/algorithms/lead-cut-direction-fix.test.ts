...
describe('Lead Cut Direction Fix', () => {
  ...
  it('should show improvement for ADLER Part 5 with correct cut direction', async () => {
    ...
    if (noCutDirResult.leadIn) {
      for (let i: number = 0; i < noCutDirResult.leadIn.points.length - 1; i++) {
    ...
    if (clockwiseResult.leadIn) {
      for (let i: number = 0; i < clockwiseResult.leadIn.points.length - 1; i++) {
    ...
    if (counterclockwiseResult.leadIn) {
      for (let i: number = 0; i < counterclockwiseResult.leadIn.points.length - 1; i++) {
