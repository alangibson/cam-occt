...
export function calculateShapeBounds(shape: Shape): ShapeBounds {
  try {
    ...
    console.warn(`Failed to calculate bounds for shape ${shape.id}:`, error);
 ...
 */
export function isShapeInViewport(shapeBounds: ShapeBounds, viewport: ViewportBounds, margin: number = 0): boolean {
  return !(
