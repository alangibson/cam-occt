...
describe('viewport-culling', () => {
  describe('calculateShapeBounds', () => {
    it('should convert bounding box to shape bounds format', async () => {
      const { getBoundingBoxForShape } = await import('../geometry/bounding-box');
    ...
    it('should handle bounding box calculation errors', async () => {
      ...
      expect(consoleSpy).toHaveBeenCalledWith(
        expect.stringContaining('Failed to calculate bounds for shape invalid-shape'),
        expect.any(Error)
      );
  ...
  describe('isShapeInViewport', () => {
    ...
    it('should handle margin correctly for inclusion', () => {
      ...
      expect(isShapeInViewport(shapeBounds, tightViewport, 0)).toBe(false);
      
      // With sufficient margin - should be inside
      expect(isShapeInViewport(shapeBounds, tightViewport, 10)).toBe(true);
    });
  ...
  describe('cullShapesToViewport', () => {
    ...
    it('should return all shapes when all are visible', async () => {
      ...
      vi.mocked(getBoundingBoxForShape)
        .mockReturnValueOnce({ min: { x: 10, y: 10 }, max: { x: 20, y: 20 } })
        .mockReturnValueOnce({ min: { x: 30, y: 30 }, max: { x: 40, y: 40 } })
        .mockReturnValueOnce({ min: { x: 50, y: 50 }, max: { x: 60, y: 60 } });
    ...
    it('should cull shapes outside viewport', async () => {
      ...
      vi.mocked(getBoundingBoxForShape)
        .mockReturnValueOnce({ min: { x: 10, y: 10 }, max: { x: 20, y: 20 } }) // Inside
        .mockReturnValueOnce({ min: { x: 200, y: 200 }, max: { x: 210, y: 210 } }) // Outside
        .mockReturnValueOnce({ min: { x: 80, y: 80 }, max: { x: 90, y: 90 } }); // Inside
    ...
    it('should respect margin parameter', async () => {
      ...
      vi.mocked(getBoundingBoxForShape)
        .mockReturnValue({ min: { x: 120, y: 120 }, max: { x: 130, y: 130 } });
    ...
    it('should handle shapes with bounding box errors gracefully', async () => {
      ...
      vi.mocked(getBoundingBoxForShape)
        .mockImplementationOnce(() => { throw new Error('Bounding box error'); })
        .mockReturnValueOnce({ min: { x: 10, y: 10 }, max: { x: 20, y: 20 } });
    ...
    it('should maintain shape order in visible shapes', async () => {
      ...
      expect(result.visibleShapes.map(s => s.id)).toEqual(['first', 'second', 'third']);
    });
  });
});