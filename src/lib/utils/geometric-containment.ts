...
import { calculatePolygonArea, isPointInPolygon as isPointInPolygonShared } from './polygon-geometry-shared';
 ...
 */
function tessellateChain(chain: Chain, params: PartDetectionParameters = DEFAULT_PART_DETECTION_PARAMETERS): Point2D[] {
 ...
 */
export function calculateChainArea(chain: Chain, tolerance: number = 0.01, params: PartDetectionParameters = DEFAULT_PART_DETECTION_PARAMETERS): number {
...
export function isShapeContainedInShape(
  ...
  try {
    ...
    const innerDistance = Math.sqrt(
      Math.pow(innerPoints[0].x - innerPoints[innerPoints.length - 1].x, 2) +
      Math.pow(innerPoints[0].y - innerPoints[innerPoints.length - 1].y, 2)
    );
...
export function detectPolygonContainment(
  ...
  const polygonsWithArea = polygons.map((polygon, index) => ({
    polygon,
    index,
 ...
 */
export function calculateNestingLevel(chainId: string, containmentMap: Map<string, string>): number {
  let level = 0;
 ...
 */
export function identifyShells(chains: Chain[], containmentMap: Map<string, string>, tolerance: number): Chain[] {
  const shells: Chain[] = [];
