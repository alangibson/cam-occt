...
function getEllipsePointAtParameter(ellipse: Ellipse, parameter: number): Point2D {
  ...
  const majorAxisLength = Math.sqrt(majorAxis.x * majorAxis.x + majorAxis.y * majorAxis.y);
...
describe('Ellipse Point Calculations', () => {
  it('should calculate correct points for a horizontal ellipse', () => {
    ...
    const point3PI2 = getEllipsePointAtParameter(ellipse, 3 * Math.PI / 2);
  ...
  it('should handle ellipse arcs with start and end parameters', () => {
    ...
    const startPoint = getEllipsePointAtParameter(ellipse, ellipse.startParam!);
