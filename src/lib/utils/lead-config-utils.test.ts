...
describe('createLeadInConfig', () => {
  ...
  it('should handle different lead types', () => {
    const lineConfig = createLeadInConfig(createTestPath({ leadInType: LeadType.LINE }));
    expect(lineConfig.type).toBe(LeadType.LINE);
    
    const arcConfig = createLeadInConfig(createTestPath({ leadInType: LeadType.ARC }));
    expect(arcConfig.type).toBe(LeadType.ARC);
    
    const noneConfig = createLeadInConfig(createTestPath({ leadInType: LeadType.NONE }));
    expect(noneConfig.type).toBe(LeadType.NONE);
  });

  it('should handle zero and negative lengths', () => {
    const zeroConfig = createLeadInConfig(createTestPath({ leadInLength: 0 }));
    expect(zeroConfig.length).toBe(0);
    
    const negativeConfig = createLeadInConfig(createTestPath({ leadInLength: -5 }));
    expect(negativeConfig.length).toBe(-5);
  });

  it('should handle extreme angle values', () => {
    const config360 = createLeadInConfig(createTestPath({ leadInAngle: 360 }));
    expect(config360.angle).toBe(360);
    
    const configNegative = createLeadInConfig(createTestPath({ leadInAngle: -90 }));
    expect(configNegative.angle).toBe(-90);
    
    const configLarge = createLeadInConfig(createTestPath({ leadInAngle: 720 }));
...
describe('createLeadOutConfig', () => {
  ...
  it('should handle different lead types', () => {
    const lineConfig = createLeadOutConfig(createTestPath({ leadOutType: LeadType.LINE }));
    expect(lineConfig.type).toBe(LeadType.LINE);
    
    const arcConfig = createLeadOutConfig(createTestPath({ leadOutType: LeadType.ARC }));
    expect(arcConfig.type).toBe(LeadType.ARC);
    
    const noneConfig = createLeadOutConfig(createTestPath({ leadOutType: LeadType.NONE }));
    expect(noneConfig.type).toBe(LeadType.NONE);
  });

  it('should handle zero and negative lengths', () => {
    const zeroConfig = createLeadOutConfig(createTestPath({ leadOutLength: 0 }));
    expect(zeroConfig.length).toBe(0);
    
    const negativeConfig = createLeadOutConfig(createTestPath({ leadOutLength: -3.5 }));
    expect(negativeConfig.length).toBe(-3.5);
  });

  it('should handle extreme angle values', () => {
    const config360 = createLeadOutConfig(createTestPath({ leadOutAngle: 360 }));
    expect(config360.angle).toBe(360);
    
    const configNegative = createLeadOutConfig(createTestPath({ leadOutAngle: -45 }));
    expect(configNegative.angle).toBe(-45);
    
    const configLarge = createLeadOutConfig(createTestPath({ leadOutAngle: 540 }));
