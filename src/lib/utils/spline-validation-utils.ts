...
export function validateSplineGeometry(spline: Spline): SplineValidationResult {
  ...
  if (!spline.knots || spline.knots.length !== expectedKnots) {
    ...
    const validKnots: number[] = generateValidKnotVector(controlPoints.length, degree);
...
export function repairSplineKnotVector(spline: Spline): Spline {
  const degree: number = spline.degree || 3;
  const validKnots: number[] = generateValidKnotVector(spline.controlPoints.length, degree);
 ...
 */
export function validateSplineKnots(knots: number[], degree: number, numControlPoints: number): boolean {
