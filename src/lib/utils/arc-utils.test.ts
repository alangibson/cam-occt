  ...
  describe('convertBulgeToArc', () => {
    ...
    it('should calculate correct radius for simple case', () => {
      ...
      const startDist = Math.sqrt(
        (start.x - result!.center.x) ** 2 + (start.y - result!.center.y) ** 2
      );
      const endDist = Math.sqrt(
        (end.x - result!.center.x) ** 2 + (end.y - result!.center.y) ** 2
      );
    ...
    it('should handle error conditions gracefully', () => {
      ...
      if (result === null) {
        ...
        expect(Number.isNaN(result.radius) || Number.isNaN(result.center.x) || Number.isNaN(result.center.y)).toBe(true);
      }
