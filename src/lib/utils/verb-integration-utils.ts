import type { Point2D, Line, Arc, Spline, Ellipse, Circle } from '../types/geometry';
...
export function createVerbCurveFromSpline(spline: Spline): verb.geom.ICurve {
  const controlPoints3D = spline.controlPoints.map(p => [p.x, p.y, 0] as [number, number, number]);
  ...
  if (!knots || knots.length === 0 || knots.length !== expectedKnotLength) {
    ...
    knots = generateUniformKnotVector(controlPoints3D.length, spline.degree);
  }
...
export function processVerbIntersectionResults(
  ...
  for (const intersection of intersections) {
    ...
    } else if ('point0' in intersection && Array.isArray((intersection as unknown as AlternativeIntersectionWithPoints).point0)) {
      // TODO this is only used by tests. remove it

      // Alternative structure: { point0: [x, y, z], point1: [x, y, z], u0: param, u1: param }
      const altIntersection = intersection as unknown as AlternativeIntersectionWithPoints;
      point = {
    ...
    } else if ('point' in intersection && Array.isArray((intersection as unknown as AlternativeIntersectionWithPoint).point)) {
      // TODO this is only used by tests. remove it
      
      // Another structure: { point: [x, y, z], u: number, v: number }
      const altIntersection = intersection as unknown as AlternativeIntersectionWithPoint;
      point = {
        x: altIntersection.point[0],
        y: altIntersection.point[1]
      };
      param1 = swapParams ? (altIntersection.v ?? altIntersection.u1 ?? 0) : (altIntersection.u ?? altIntersection.u0 ?? 0);
      param2 = swapParams ? (altIntersection.u ?? altIntersection.u0 ?? 0) : (altIntersection.v ?? altIntersection.u1 ?? 0);
    } else if (Array.isArray(intersection) && intersection.length >= 2) {
      // TODO this is only used by tests. remove it
      
      // Array structure: [[x, y, z], u, v] or similar array format
      const arrIntersection = intersection as unknown as ArrayIntersection;
      point = {
