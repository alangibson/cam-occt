...
describe('findPartContainingChain', () => {
  ...
  function createMockPart(shellChainId: string, holeChainIds: string[] = []): DetectedPart {
  ...
  describe('hole chain matching', () => {
    it('should find part by hole chain ID', () => {
      ...
      expect(result?.id).toBe('part-shell-2');
      expect(result?.shell.chain.id).toBe('shell-2');
      expect(result?.holes.some(h => h.chain.id === 'hole-3')).toBe(true);
    });
  ...
  describe('edge cases', () => {
    ...
    it('should handle special characters in chain IDs', () => {
      ...
      expect(findPartContainingChain('shell-1_test', parts)?.id).toBe('part-shell-1_test');
      expect(findPartContainingChain('hole-1@special', parts)?.id).toBe('part-shell-1_test');
      expect(findPartContainingChain('shell-2-dash', parts)?.id).toBe('part-shell-2-dash');
      expect(findPartContainingChain('hole.dot', parts)?.id).toBe('part-shell-2-dash');
    });
  ...
  describe('real-world scenarios', () => {
    it('should handle typical ADLER.dxf-like scenario', () => {
      ...
      expect(findPartContainingChain('chain-5', parts)?.id).toBe('part-chain-5');
      expect(findPartContainingChain('chain-1', parts)?.id).toBe('part-chain-1');
      
      // Should find hole chain
      expect(findPartContainingChain('chain-10', parts)?.id).toBe('part-chain-5');
    ...
    it('should handle complex part with multiple holes', () => {
      ...
      expect(findPartContainingChain('main-shell', parts)?.id).toBe('part-main-shell');
      
      // Should find any of the holes
      expect(findPartContainingChain('hole-1', parts)?.id).toBe('part-main-shell');
      expect(findPartContainingChain('hole-6', parts)?.id).toBe('part-main-shell');
      expect(findPartContainingChain('hole-12', parts)?.id).toBe('part-main-shell');
    });
  });
});