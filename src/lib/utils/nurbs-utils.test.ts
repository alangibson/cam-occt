...
describe('nurbs-utils', () => {
  ...
  describe('validateKnotVector', () => {
    ...
    it('should reject incorrect length', () => {
      ...
      expect(result.isValid).toBe(false);
      expect(result.error).toBe('Knot vector length 4 does not match expected length 7');
    });
    ...
    it('should reject insufficient first knot multiplicity', () => {
      ...
      expect(result.isValid).toBe(false);
      expect(result.error).toBe('First knot multiplicity 2 is less than degree + 1 (3)');
    });
    ...
    it('should reject insufficient last knot multiplicity', () => {
      ...
      expect(result.isValid).toBe(false);
      expect(result.error).toBe('Last knot multiplicity 2 is less than degree + 1 (3)');
    });
  ...
  describe('integration tests', () => {
    it('should validate generated knot vectors', () => {
      ...
      for (const testCase of testCases) {
        const knots = generateUniformKnotVector(testCase.controlPoints, testCase.degree);
        const validation = validateKnotVector(knots, testCase.controlPoints, testCase.degree);
    ...
    it('should repair invalid vectors to valid ones', () => {
      ...
      for (const testCase of testCases) {
        const repaired = repairKnotVector(testCase.invalid, testCase.controlPoints, testCase.degree);
        const validation = validateKnotVector(repaired, testCase.controlPoints, testCase.degree);
