...
function createRectangle(x: number, y: number, width: number, height: number): Shape[] {
  return [
    { 
      id: '1', 
      type: 'line', 
...
function createEllipse(id: string, center: { x: number; y: number }, majorAxisEndpoint: { x: number; y: number }, minorToMajorRatio: number, startParam?: number, endParam?: number): Shape {
  return {
    id,
    type: 'ellipse',
    geometry: {
...
function createSpline(id: string, controlPoints: { x: number; y: number }[], degree: number = 3): Shape {
  return {
    id,
    type: 'spline',
    geometry: {
...
describe('isChainGeometricallyContained', () => {
  it('should detect small rectangle contained in large rectangle', () => {
    ...
    expect(isChainGeometricallyContained(innerChain, outerChain)).toBe(true);
  });
  ...
  it('should handle arcs correctly', () => {
    const outerCircle = createCircle('outer', 0, 0, 10);
    const innerArc = createArc('inner', { x: 0, y: 0 }, 5, 0, Math.PI, false);
    const outerChain = createTestChain('outer', [outerCircle]);
  ...
  it('should handle clockwise arcs', () => {
    const outerCircle = createCircle('outer', 0, 0, 10);
    const innerArc = createArc('inner', { x: 0, y: 0 }, 5, Math.PI, 0, true);
    const outerChain = createTestChain('outer', [outerCircle]);
    const innerChain = createTestChain('inner', [innerArc]);
  ...
  it('should handle ellipses correctly', () => {
    const outerEllipse = createEllipse('outer', { x: 0, y: 0 }, { x: 20, y: 0 }, 0.5);
    const innerEllipse = createEllipse('inner', { x: 0, y: 0 }, { x: 10, y: 0 }, 0.5);
  ...
  it('should handle ellipse arcs', () => {
    const outerEllipse = createEllipse('outer', { x: 0, y: 0 }, { x: 20, y: 0 }, 0.5);
    const innerEllipseArc = createEllipse('inner', { x: 0, y: 0 }, { x: 10, y: 0 }, 0.5, 0, Math.PI);
    const outerChain = createTestChain('outer', [outerEllipse]);
  ...
  it('should throw error for chains that cannot form polygons', () => {
    ...
    const validChain = createTestChain('valid', createRectangle(0, 0, 10, 10));
  ...
  it('should handle mixed shape types in same chain', () => {
    ...
    expect(isChainGeometricallyContained(mixedChain, outerChain)).toBe(true);
  });
});
