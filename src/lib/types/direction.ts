...
export class DirectionUtils {
   ...
   */
  static cutDirectionToRotation(cutDir: CutDirection): RotationDirection | null {
    switch (cutDir) {
  ...
  static convertAngle(
    ...
    return unit === AngleUnit.DEGREES ? this.radiansToDegrees(radians) : radians;
  }

  /**
   * Get unit vector for a cardinal direction
   */
  static getCardinalVector(direction: CardinalDirection): { x: number; y: number } {
    switch (direction) {
