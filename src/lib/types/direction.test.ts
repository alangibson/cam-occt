...
describe('DirectionUtils', () => {
  describe('cutDirectionToRotation', () => {
    it('should convert CLOCKWISE to CLOCKWISE', () => {
      expect(DirectionUtils.cutDirectionToRotation(CutDirection.CLOCKWISE))
        .toBe(RotationDirection.CLOCKWISE);
    });

    it('should convert COUNTERCLOCKWISE to COUNTERCLOCKWISE', () => {
      expect(DirectionUtils.cutDirectionToRotation(CutDirection.COUNTERCLOCKWISE))
        .toBe(RotationDirection.COUNTERCLOCKWISE);
    });

    it('should convert NONE to null', () => {
      expect(DirectionUtils.cutDirectionToRotation(CutDirection.NONE)).toBe(null);
    });

    it('should handle invalid input gracefully', () => {
      expect(DirectionUtils.cutDirectionToRotation('invalid' as CutDirection)).toBe(null);
    });
  });

  describe('oppositeCutDirection', () => {
    it('should return COUNTERCLOCKWISE for CLOCKWISE', () => {
      expect(DirectionUtils.oppositeCutDirection(CutDirection.CLOCKWISE))
        .toBe(CutDirection.COUNTERCLOCKWISE);
    });

    it('should return CLOCKWISE for COUNTERCLOCKWISE', () => {
      expect(DirectionUtils.oppositeCutDirection(CutDirection.COUNTERCLOCKWISE))
        .toBe(CutDirection.CLOCKWISE);
    });

    it('should return NONE for NONE', () => {
      expect(DirectionUtils.oppositeCutDirection(CutDirection.NONE))
        .toBe(CutDirection.NONE);
    });

    it('should handle invalid input gracefully', () => {
      expect(DirectionUtils.oppositeCutDirection('invalid' as CutDirection))
        .toBe(CutDirection.NONE);
    });
  });
  ...
  describe('angle conversion', () => {
    describe('degreesToRadians', () => {
      ...
      it('should convert 90 degrees to π/2 radians', () => {
        expect(DirectionUtils.degreesToRadians(90)).toBeCloseTo(Math.PI / 2);
      });

      it('should convert 180 degrees to π radians', () => {
        expect(DirectionUtils.degreesToRadians(180)).toBeCloseTo(Math.PI);
      });

      it('should convert 360 degrees to 2π radians', () => {
        expect(DirectionUtils.degreesToRadians(360)).toBeCloseTo(2 * Math.PI);
      });
    ...
    describe('radiansToDegrees', () => {
      ...
      it('should convert π/2 radians to 90 degrees', () => {
        expect(DirectionUtils.radiansToDegrees(Math.PI / 2)).toBeCloseTo(90);
      });

      it('should convert π radians to 180 degrees', () => {
        expect(DirectionUtils.radiansToDegrees(Math.PI)).toBeCloseTo(180);
      });

      it('should convert 2π radians to 360 degrees', () => {
        expect(DirectionUtils.radiansToDegrees(2 * Math.PI)).toBeCloseTo(360);
      });
  ...
  describe('angle normalization', () => {
    describe('normalizeRadians', () => {
      ...
      it('should wrap angles greater than π', () => {
        expect(DirectionUtils.normalizeRadians(3 * Math.PI)).toBeCloseTo(Math.PI);
        expect(DirectionUtils.normalizeRadians(5 * Math.PI)).toBeCloseTo(Math.PI);
      });
  ...
  describe('convertAngle', () => {
    it('should return same angle for same coordinate system', () => {
      expect(DirectionUtils.convertAngle(45, AngleUnit.DEGREES, CoordinateSystem.WORLD, CoordinateSystem.WORLD))
        .toBe(45);
      expect(DirectionUtils.convertAngle(Math.PI/4, AngleUnit.RADIANS, CoordinateSystem.SCREEN, CoordinateSystem.SCREEN))
        .toBe(Math.PI/4);
    });

    it('should convert degrees between world and screen coordinates', () => {
      // World to screen: flip Y (negate angle)
      expect(DirectionUtils.convertAngle(90, AngleUnit.DEGREES, CoordinateSystem.WORLD, CoordinateSystem.SCREEN))
        .toBe(-90);
      
      // Screen to world: flip Y (negate angle)
      expect(DirectionUtils.convertAngle(-90, AngleUnit.DEGREES, CoordinateSystem.SCREEN, CoordinateSystem.WORLD))
        .toBe(90);
    });

    it('should convert radians between world and screen coordinates', () => {
      expect(DirectionUtils.convertAngle(Math.PI/2, AngleUnit.RADIANS, CoordinateSystem.WORLD, CoordinateSystem.SCREEN))
        .toBeCloseTo(-Math.PI/2);
      
      expect(DirectionUtils.convertAngle(-Math.PI/2, AngleUnit.RADIANS, CoordinateSystem.SCREEN, CoordinateSystem.WORLD))
        .toBeCloseTo(Math.PI/2);
    });

    it('should handle unit circle coordinate system', () => {
      // Unit circle should behave like world coordinates
      expect(DirectionUtils.convertAngle(45, AngleUnit.DEGREES, CoordinateSystem.UNIT_CIRCLE, CoordinateSystem.WORLD))
        .toBe(45);
      
      expect(DirectionUtils.convertAngle(45, AngleUnit.DEGREES, CoordinateSystem.UNIT_CIRCLE, CoordinateSystem.SCREEN))
        .toBe(-45);
    });
  });
  ...
  describe('getCardinalVector', () => {
    it('should return correct unit vectors for cardinal directions', () => {
      expect(DirectionUtils.getCardinalVector(CardinalDirection.EAST)).toEqual({ x: 1, y: 0 });
      expect(DirectionUtils.getCardinalVector(CardinalDirection.NORTH)).toEqual({ x: 0, y: 1 });
      expect(DirectionUtils.getCardinalVector(CardinalDirection.WEST)).toEqual({ x: -1, y: 0 });
      expect(DirectionUtils.getCardinalVector(CardinalDirection.SOUTH)).toEqual({ x: 0, y: -1 });
    });

    it('should return default EAST vector for invalid input', () => {
      expect(DirectionUtils.getCardinalVector('invalid' as CardinalDirection))
        .toEqual({ x: 1, y: 0 });
    });
  });
  ...
  describe('getCardinalAngle', () => {
    it('should return correct angles for cardinal directions', () => {
      expect(DirectionUtils.getCardinalAngle(CardinalDirection.EAST)).toBe(0);
      expect(DirectionUtils.getCardinalAngle(CardinalDirection.NORTH)).toBe(Math.PI / 2);
      expect(DirectionUtils.getCardinalAngle(CardinalDirection.WEST)).toBe(Math.PI);
      expect(DirectionUtils.getCardinalAngle(CardinalDirection.SOUTH)).toBe(3 * Math.PI / 2);
    });

    it('should return default 0 for invalid input', () => {
      expect(DirectionUtils.getCardinalAngle('invalid' as CardinalDirection)).toBe(0);
    });
  });
  ...
  describe('validation methods', () => {
    describe('isValidCutDirection', () => {
      it('should validate correct cut directions', () => {
        expect(DirectionUtils.isValidCutDirection('clockwise')).toBe(true);
        expect(DirectionUtils.isValidCutDirection('counterclockwise')).toBe(true);
        expect(DirectionUtils.isValidCutDirection('none')).toBe(true);
      });

      it('should reject invalid cut directions', () => {
        expect(DirectionUtils.isValidCutDirection('invalid')).toBe(false);
        expect(DirectionUtils.isValidCutDirection('')).toBe(false);
        expect(DirectionUtils.isValidCutDirection('CLOCKWISE')).toBe(false);
      });
    });
  ...
  describe('describeCutDirection', () => {
    it('should provide human-readable descriptions', () => {
      expect(DirectionUtils.describeCutDirection(CutDirection.CLOCKWISE))
        .toBe('Clockwise (CW)');
      expect(DirectionUtils.describeCutDirection(CutDirection.COUNTERCLOCKWISE))
        .toBe('Counterclockwise (CCW)');
      expect(DirectionUtils.describeCutDirection(CutDirection.NONE))
        .toBe('No specific direction');
    });

    it('should handle invalid input gracefully', () => {
      expect(DirectionUtils.describeCutDirection('invalid' as CutDirection))
...
describe('Edge Cases and Mathematical Properties', () => {
  describe('angle conversion precision', () => {
    ...
    it('should handle very small angles', () => {
      expect(DirectionUtils.degreesToRadians(0.001)).toBeCloseTo(0.001 * Math.PI / 180);
      expect(DirectionUtils.radiansToDegrees(0.001)).toBeCloseTo(0.001 * 180 / Math.PI);
    });
  });
  ...
  describe('normalization edge cases', () => {
    ...
    it('should handle multiple wraps', () => {
      expect(DirectionUtils.normalizeRadians(7 * Math.PI)).toBeCloseTo(Math.PI);
  ...
  describe('coordinate system conversion edge cases', () => {
    it('should handle zero angle conversions', () => {
      expect(DirectionUtils.convertAngle(0, AngleUnit.DEGREES, CoordinateSystem.WORLD, CoordinateSystem.SCREEN))
        .toBeCloseTo(0);
      expect(DirectionUtils.convertAngle(0, AngleUnit.RADIANS, CoordinateSystem.SCREEN, CoordinateSystem.WORLD))
        .toBeCloseTo(0);
    });

    it('should handle π angle conversions', () => {
      expect(DirectionUtils.convertAngle(Math.PI, AngleUnit.RADIANS, CoordinateSystem.WORLD, CoordinateSystem.SCREEN))
        .toBeCloseTo(-Math.PI);
    });
    ...
    it('should maintain precision in coordinate system round trips', () => {
      ...
      for (const radians of testAngles) {
        const screenAngle = DirectionUtils.convertAngle(radians, AngleUnit.RADIANS, CoordinateSystem.WORLD, CoordinateSystem.SCREEN);
        const backToWorld = DirectionUtils.convertAngle(screenAngle, AngleUnit.RADIANS, CoordinateSystem.SCREEN, CoordinateSystem.WORLD);
