...
import type { Point2D, Arc, Line, Circle, Spline, Ellipse, Polyline } from '../types';
...
import { calculateSplineEndTangent, calculateSplineStartTangent } from '$lib/algorithms/offset-calculation/extend/spline';
...
export class SVGBuilder {
  ...
  constructor(width?: number, height?: number, offsetX?: number, offsetY?: number, flipY: boolean = true) {
    this.flipY = flipY;
  ...
  private updateBoundsWithPoint(point: Point2D, strokeRadius: number = 0) {
    ...
    if (isNaN(point.x) || isNaN(point.y)) {
      console.error(`updateBoundsWithPoint received NaN: x=${point.x}, y=${point.y}`);
  ...
  private updateBoundsWithRect(x: number, y: number, width: number, height: number) {
    if (!this.autoSize) return;
  ...
  private addLine(line: Line, color: string, width: number, dashArray?: string) {
  ...
  private addCircle(circle: Circle, color: string, strokeWidth: number, fill: string, dashArray?: string) {
    // Update bounds for auto-sizing - circle bounds include radius + stroke width
    this.updateBoundsWithPoint(circle.center, circle.radius + strokeWidth / 2);
  ...
  private addArc(arc: Arc, color: string, strokeWidth: number, dashArray?: string) {
    ...
    const arcShape: Shape = { id: `arc-${Date.now()}`, type: 'arc', geometry: arc };
  ...
  private addPolyline(polyline: Polyline, color: string, strokeWidth: number, dashArray?: string) {
  ...
  private addPolylinePoints(points: Point2D[], closed: boolean, color: string, strokeWidth: number, fill: string, dashArray?: string) {
    ...
    if (hasNaN) {
      console.error(`addPolyline received points with NaN values. First point: x=${points[0]?.x}, y=${points[0]?.y}`);
    }

    // Update bounds for auto-sizing with stroke width
    points.forEach(point => this.updateBoundsWithPoint(point, strokeWidth / 2));
  ...
  addText(x: number, y: number, text: string, color: string, fontSize: string) {
  ...
  private renderLegend(): string[] {
    ...
    const maxLabelLength: number = Math.max(...items.map(item => item.label.length));
    ...
    items.forEach((item, index) => {
      ...
      if (item.color === 'yellow' || item.color === 'green' || item.color === 'orange' || item.color === 'cyan') {
  ...
  addShape(shape: Shape, color: string, strokeWidth: number, label?: string, dashArray?: string) {
    switch (shape.type) {
      case 'line':
        ...
        break;
      case 'arc':
        this.addArc(shape.geometry as Arc, color, strokeWidth, dashArray);
        break;
      case 'circle':
        const circleGeom: Circle = shape.geometry as Circle;
        this.addCircle(circleGeom, color, strokeWidth, 'none', dashArray);
        break;
      case 'polyline':
        this.addPolyline(shape.geometry as Polyline, color, strokeWidth, dashArray);
        break;
      case 'spline':
        this.addSpline(shape.geometry as Spline, color, strokeWidth, dashArray);
        break;
      case 'ellipse':
        this.addEllipse(shape.geometry as Ellipse, color, strokeWidth, dashArray);
        break;
    }
  }

  private addSpline(spline: Spline, color: string, strokeWidth: number, dashArray?: string) {
    ...
    const hasNaNControlPoints: boolean = spline.controlPoints.some(p => isNaN(p.x) || isNaN(p.y));
    ...
    const weights: number[] = (spline.weights && spline.weights.length > 0)
      ? spline.weights
    ...
    if (spline.degree === 3 && spline.controlPoints.length === 4) {
      if (spline.knots.length === 8 &&
        spline.knots[0] === 0 && spline.knots[7] === 1 &&
    ...
    try {
      ...
      try {
        ...
        throw new Error(`Both verb NURBS construction methods failed. Original: ${error}, Fallback: ${fallbackError}`);
      }
    }
    ...
    const points: Point2D[] = tessellateVerbCurve(verbCurve, visualTolerance);

    // Check for NaN in tessellated points
    const hasNaNTessellation: boolean = points.some(p => isNaN(p.x) || isNaN(p.y));
    if (hasNaNTessellation) {
      throw new Error(`Spline tessellation produced NaN. Degree: ${spline.degree}, Knots: ${spline.knots.length}, Control points: ${spline.controlPoints.length}`);
    }

    this.addPolylinePoints(points, spline.closed, color, strokeWidth, 'none', dashArray);
  }

  private addEllipse(ellipse: Ellipse, color: string, strokeWidth: number, dashArray?: string) {
    ...
    this.addPolylinePoints(points, !isArc, color, strokeWidth, 'none', dashArray);
    
    // Add green point at ellipse arc start and red point at ellipse arc end (like addArc does)
    if (isArc) {
      const ellipseShape: Shape = { id: `ellipse-${Date.now()}`, type: 'ellipse', geometry: ellipse };
   ...
   */
  addLineExtensions(line: Line, color: string, strokeWidth: number, extensionLength: number = 100) {
    // Calculate line direction
    const lineDir: { x: number; y: number } = {
      x: line.end.x - line.start.x,
      y: line.end.y - line.start.y
    };
    const lineLength: number = Math.sqrt(lineDir.x * lineDir.x + lineDir.y * lineDir.y);
    ...
    this.addLine({ start: startExtension, end: line.start }, color, strokeWidth, "5,5");
    ...
    this.addLine({ start: line.end, end: endExtension }, color, strokeWidth, "5,5");
  }

  /**
   * Draw extensions for a spline shape as dotted tangent lines
   */
  addSplineExtensions(spline: Spline, color: string, strokeWidth: number, extensionLength: number = 100) {
    try {

      // Get start and end points of spline
      const startPoint: Point2D = spline.controlPoints[0];
      const endPoint: Point2D = spline.controlPoints[spline.controlPoints.length - 1];
      ...
      // Normalize tangents
      const startTangentLength: number = Math.sqrt(startTangent.x * startTangent.x + startTangent.y * startTangent.y);
      const endTangentLength: number = Math.sqrt(endTangent.x * endTangent.x + endTangent.y * endTangent.y);
      ...
      this.addLine({ start: startExtension, end: startPoint }, color, strokeWidth, "5,5");
      ...
      this.addLine({ start: endPoint, end: endExtension }, color, strokeWidth, "5,5");
      ...
      if (spline.controlPoints.length > 1) {
        ...
        const startLength: number = Math.sqrt(startTangent.x * startTangent.x + startTangent.y * startTangent.y);
      ...
      if (spline.controlPoints.length > 1) {
        ...
        const endTangent: Point2D = {
          x: spline.controlPoints[lastIdx].x - spline.controlPoints[lastIdx - 1].x,
          y: spline.controlPoints[lastIdx].y - spline.controlPoints[lastIdx - 1].y
        };
        const endLength: number = Math.sqrt(endTangent.x * endTangent.x + endTangent.y * endTangent.y);
   ...
   */
  addArcExtensions(arc: Arc, color: string, strokeWidth: number, extensionLength: number = 100) {
    ...
    const extensionAngle: number = Math.min(extensionLength / arc.radius, 4 * Math.PI);
   ...
   */
  addArcWithDash(arc: Arc, color: string, strokeWidth: number, dashArray?: string) {
    const startPoint = calculateArcPoint(arc.center, arc.radius, arc.startAngle);
    ...
    const largeArcFlag: number = Math.abs(arc.endAngle - arc.startAngle) > Math.PI ? 1 : 0;
    ...
    const dashStyle: string = dashArray ? ` stroke-dasharray="${dashArray}"` : '';
   ...
   */
  addPolylineExtensions(polyline: Polyline, color: string, strokeWidth: number, extensionLength: number = 100) {
    ...
    const firstSegmentLength: number = Math.sqrt(firstSegmentDir.x * firstSegmentDir.x + firstSegmentDir.y * firstSegmentDir.y);
    ...
    if (firstSegmentLength > 0) {
      ...
      this.addLine({ start: startExtension, end: firstPoint }, color, strokeWidth, "5,5");
    }
    ...
    const lastSegmentLength: number = Math.sqrt(lastSegmentDir.x * lastSegmentDir.x + lastSegmentDir.y * lastSegmentDir.y);
    ...
    if (lastSegmentLength > 0) {
      ...
      this.addLine({ start: lastPoint, end: endExtension }, color, strokeWidth, "5,5");
    }
  }
   ...
   */
  addShapeExtensions(shape: Shape, color: string, strokeWidth: number, extensionLength: number = 100) {
    switch (shape.type) {
      case 'line':
        this.addLineExtensions(shape.geometry as Line, color, strokeWidth, extensionLength);
        break;
      case 'arc':
        this.addArcExtensions(shape.geometry as Arc, color, strokeWidth, extensionLength);
        break;
      case 'spline':
        this.addSplineExtensions(shape.geometry as Spline, color, strokeWidth, extensionLength);
        break;
      case 'polyline':
        this.addPolylineExtensions(shape.geometry as Polyline, color, strokeWidth, extensionLength);
        break;
  ...
  private transformElementToSVG(element: string): string {
    ...
    switch (type) {
      case 'LINE': {
        const [startX, startY]: number[] = parts[1].split(',').map(Number);
      ...
      case 'CIRCLE': {
        const [centerX, centerY]: number[] = parts[1].split(',').map(Number);
      ...
      case 'ARC': {
        ...
        const arcStart = calculateArcPoint({ x: centerX, y: centerY }, radius, startAngle);
        const arcEnd = calculateArcPoint({ x: centerX, y: centerY }, radius, endAngle);
        ...
        if (clockwise) {
          ...
          if (normalizedStart >= normalizedEnd) {
            ...
            sweepAngle = (2 * Math.PI - normalizedEnd) + normalizedStart;
          }
        } else {
        ...
        const largeArcFlag: number = sweepAngle > (Math.PI + tolerance) ? 1 : 0;
      ...
      case 'POLYLINE': {
        ...
        const pointsStr: string = points.map(p => `${this.transformX(p.x)},${this.transformY(p.y)}`).join(' ');
  ...
  toString(): string {
    ...
    if (this.legendItems && this.legendItems.length > 0) {
      const maxLabelLength: number = Math.max(...this.legendItems.map(item => item.label.length));
    ...
    if (this.autoSize) {
      if (!this.bounds || this.bounds.minX === Infinity || this.bounds.maxX === -Infinity ||
        this.bounds.minY === Infinity || this.bounds.maxY === -Infinity) {
        ...
        const boundsHeight: number = this.bounds.maxY - this.bounds.minY;
        ...
        if (legendWidth > 0) {
          this.width = Math.max(this.width, legendWidth + 3 * this.padding);
          this.height = Math.max(this.height, legendHeight + 2 * this.padding);
        }
    ...
    const transformedElements: string[] = this.elements.map(element => this.transformElementToSVG(element));

    // Step 4: Render legend elements now that dimensions are finalized  
    const legendElements: string[] = this.renderLegend();
    const allElements: string[] = [...transformedElements, ...legendElements];
