...
describe('Chain Store - Clockwise Property Setting', () => {
  ...
  it('should set clockwise property to null for open chain', () => {
    ...
    const openLine: Shape[] = [
      { id: 'line1', type: 'line', geometry: { start: { x: 0, y: 0 }, end: { x: 10, y: 0 } } }
    ];
  ...
  it('should handle multiple chains with different winding directions', () => {
    ...
    const counterclockwiseSquare: Shape[] = [
      { id: 'line5', type: 'line', geometry: { start: { x: 20, y: 0 }, end: { x: 30, y: 0 } } },  // right
      { id: 'line6', type: 'line', geometry: { start: { x: 30, y: 0 }, end: { x: 30, y: 10 } } }, // up
      { id: 'line7', type: 'line', geometry: { start: { x: 30, y: 10 }, end: { x: 20, y: 10 } } }, // left
      { id: 'line8', type: 'line', geometry: { start: { x: 20, y: 10 }, end: { x: 20, y: 0 } } }   // down
    ];

    const openLine: Shape[] = [
      { id: 'line9', type: 'line', geometry: { start: { x: 0, y: 20 }, end: { x: 10, y: 20 } } }
    ];
    ...
    const clockwiseChain = state.chains.find(c => c.id === 'clockwise-square');
    const counterclockwiseChain = state.chains.find(c => c.id === 'counterclockwise-square');
