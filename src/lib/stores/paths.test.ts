...
import { pathStore, selectPath, highlightPath, clearPathHighlight, type Path, type PathsState } from './paths';
...
vi.mocked(workflowStore).invalidateDownstreamStages = mockWorkflowStore.invalidateDownstreamStages;
...
describe('pathStore', () => {
  ...
  describe('addPath', () => {
    ...
    it('should complete program stage when paths exist', () => {
      ...
      expect(mockWorkflowStore.completeStage).toHaveBeenCalledWith('program');
      vi.useRealTimers();
    });
  });
  ...
  describe('updatePath', () => {
    it('should update path properties', () => {
      ...
      pathStore.updatePath('mock-path-uuid-123', { name: 'Updated Path' });
  ...
  describe('updatePathLeadGeometry', () => {
    it('should update lead-in geometry', () => {
      ...
      const leadInPoints: Point2D[] = [{ x: 0, y: 0 }, { x: 5, y: 0 }];
      pathStore.updatePathLeadGeometry('mock-path-uuid-123', {
        leadIn: { points: leadInPoints, type: LeadType.LINE }
      });

      const state = get(pathStore);
      expect(state.paths[0].calculatedLeadIn?.points).toEqual(leadInPoints);
    ...
    it('should update lead-out geometry', () => {
      ...
      const state = get(pathStore);
      expect(state.paths[0].calculatedLeadOut?.points).toEqual(leadOutPoints);
    ...
    it('should update validation results', () => {
      ...
      pathStore.updatePathLeadGeometry('mock-path-uuid-123', { validation });

      const state = get(pathStore);
      expect(state.paths[0].leadValidation?.isValid).toBe(false);
      expect(state.paths[0].leadValidation?.warnings).toEqual(['Lead too short']);
  ...
  describe('clearPathLeadGeometry', () => {
    it('should clear calculated lead geometry', () => {
      ...
      pathStore.updatePathLeadGeometry('mock-path-uuid-123', {
        ...
        validation: { isValid: true, warnings: [], errors: [], severity: 'info' }
      });
  ...
  describe('updatePathOffsetGeometry', () => {
    it('should update offset geometry', () => {
      ...
      const originalShapes: Shape[] = [
        { id: 'line-1', type: 'line', geometry: { start: { x: 0, y: 0 }, end: { x: 10, y: 0 } } }
      ];
      const offsetShapes: Shape[] = [
        { id: 'line-1-offset', type: 'line', geometry: { start: { x: 0, y: 1 }, end: { x: 10, y: 1 } } }
      ];
      ...
      const state = get(pathStore);
      expect(state.paths[0].calculatedOffset?.offsetShapes).toEqual(offsetShapes);
      expect(state.paths[0].calculatedOffset?.originalShapes).toEqual(originalShapes);
