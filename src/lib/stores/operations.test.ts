...
vi.mock('./lead-warnings', () => ({
  leadWarningsStore: {
    clearWarningsForOperation: vi.fn()
  }
}));

vi.mock('./offset-warnings', () => ({
  offsetWarningsStore: {
    clearWarningsForOperation: vi.fn(),
...
describe('operationsStore', () => {
  ...
  describe('updateOperation', () => {
    it('should update operation properties', () => {
      ...
      operationsStore.updateOperation('mock-uuid-123', { name: 'Updated Operation' });
    ...
    it('should clear warnings and regenerate paths when updated', () => {
      ...
      operationsStore.updateOperation('mock-uuid-123', { name: 'Updated' });

      expect(mockLeadWarningsStore.clearWarningsForOperation).toHaveBeenCalledWith('mock-uuid-123');
      expect(mockOffsetWarningsStore.clearWarningsForOperation).toHaveBeenCalledWith('mock-uuid-123');
    });

    it('should not update non-existent operation', () => {
      operationsStore.updateOperation('non-existent', { name: 'Updated' });
  ...
  describe('deleteOperation', () => {
    it('should remove operation and clean up related data', () => {
      ...
      const operations = get(operationsStore);
      expect(operations).toHaveLength(0);
      expect(mockPathStore.deletePathsByOperation).toHaveBeenCalledWith('mock-uuid-123');
      expect(mockLeadWarningsStore.clearWarningsForOperation).toHaveBeenCalledWith('mock-uuid-123');
      expect(mockOffsetWarningsStore.clearWarningsForOperation).toHaveBeenCalledWith('mock-uuid-123');
    });
  });
  ...
  describe('applyAllOperations', () => {
    ...
    it('should skip disabled operations', () => {
      const op1 = createTestOperation();
      const op2 = { ...createTestOperation(), name: 'Disabled', enabled: false };
  ...
  describe('getAssignedTargets', () => {
    ...
    it('should exclude specified operation', () => {
      ...
      const assigned = operationsStore.getAssignedTargets('mock-uuid-123');
