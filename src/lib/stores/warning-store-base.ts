...
export function getWarningsByOperationId<T extends Warning>(store: WarningStore<T>, operationId: string): T[] {
  return store.getWarningsForOperation(operationId);
}

export function getWarningsByChainId<T extends Warning>(store: WarningStore<T>, chainId: string): T[] {
  let warnings: T[] = [];
