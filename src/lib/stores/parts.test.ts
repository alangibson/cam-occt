...
describe('Parts Store', () => {
  ...
  describe('Helper Functions', () => {
    const parts = [
      createTestPart('part-1', 'shell-chain-1', ['hole-chain-1', 'hole-chain-2']),
    ...
    describe('getPartChainIds', () => {
      it('should return all chain IDs for a part (shell + holes)', () => {
        ...
        expect(chainIds).toEqual(['shell-chain-1', 'hole-chain-1', 'hole-chain-2']);
      });

      it('should return shell chain ID only for part without holes', () => {
        const partWithoutHoles = [createTestPart('part-no-holes', 'shell-only')];
        const chainIds = getPartChainIds('part-no-holes', partWithoutHoles);
  ...
  describe('Integration with Complex Hierarchies', () => {
    it('should handle nested holes correctly', () => {
        ...
        shell: {
          ...
          holes: [{
            ...
            holes: [{
              ...
              type: 'hole',
              boundingBox: { minX: 10, maxX: 20, minY: 10, maxY: 20 },
              holes: []
            }]
          }]
        },
      ...
      expect(chainIds).toEqual(['shell-chain', 'hole-chain', 'nested-chain']);
