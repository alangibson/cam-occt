...
describe('Prepare Stage Store', () => {
  ...
  it('should update specific algorithm parameter', () => {
    prepareStageStore.updateAlgorithmParam('chainDetection', { tolerance: 0.2 });
    
    const state = get(prepareStageStore);
    expect(state.algorithmParams.chainDetection.tolerance).toBe(0.2);
    expect(state.algorithmParams.chainNormalization).toEqual(DEFAULT_ALGORITHM_PARAMETERS.chainNormalization);
  });
  ...
  it('should get chain normalization result by ID', () => {
    ...
    const result3 = prepareStageStore.getChainNormalizationResult('nonexistent');
  ...
  it('should reset to defaults', () => {
    ...
    prepareStageStore.updateAlgorithmParam('chainDetection', { tolerance: 0.5 });
