...
describe('overlayStore', () => {
  ...
  describe('clearStageOverlay', () => {
    it('should clear all overlay data for stage', () => {
      const points: ShapePoint[] = [{ x: 0, y: 0, type: 'origin', shapeId: 'shape-1' }];
      const endpoints: ChainEndpoint[] = [{ x: 0, y: 0, type: 'start', chainId: 'chain-1' }];
      const tessPoints: TessellationPoint[] = [{ x: 5, y: 5, shapeId: 'shape-1', chainId: 'chain-1' }];
  ...
  describe('clearAllOverlays', () => {
    it('should clear all overlay data for all stages', () => {
      ...
      Object.keys(state.overlays).forEach(stage => {
        expect(state.overlays[stage as WorkflowStage].shapePoints).toHaveLength(0);
        expect(state.overlays[stage as WorkflowStage].chainEndpoints).toHaveLength(0);
        expect(state.overlays[stage as WorkflowStage].tessellationPoints).toHaveLength(0);
      });
    });
  });
});
...
describe('generateShapePoints', () => {
  const createLineShape = (id: string, x1: number, y1: number, x2: number, y2: number): Shape => ({
    id,
    type: 'line',
    geometry: {
      start: { x: x1, y: y1 },
  ...
  const createCircleShape = (id: string, cx: number, cy: number, r: number): Shape => ({
    id,
    type: 'circle',
    geometry: {
      center: { x: cx, y: cy },
  ...
  const createArcShape = (id: string, cx: number, cy: number, r: number, startAngle: number, endAngle: number): Shape => ({
    id,
    type: 'arc',
    geometry: {
      center: { x: cx, y: cy },
  ...
  it('should generate points for selected line shapes', () => {
    ...
    expect(points).toContainEqual({ x: 0, y: 0, type: 'start', shapeId: 'line-1' });
    expect(points).toContainEqual({ x: 10, y: 10, type: 'end', shapeId: 'line-1' });
  });

  it('should generate points for selected circle shapes', () => {
    const shapes: Shape[] = [
      createCircleShape('circle-1', 5, 5, 10)
    ];
    ...
    expect(points).toContainEqual({ x: 5, y: 5, type: 'origin', shapeId: 'circle-1' });
  });
  ...
  it('should generate points for selected arc shapes', () => {
    ...
    expect(points).toHaveLength(3); // origin (center), start, and end points
    expect(points).toContainEqual({ x: 0, y: 0, type: 'origin', shapeId: 'arc-1' });
  });
...
describe('generateChainEndpoints', () => {
  ...
  it('should generate endpoints for open chain', () => {
    ...
    expect(endpoints).toContainEqual({ x: 0, y: 0, type: 'start', chainId: 'chain-1' });
    expect(endpoints).toContainEqual({ x: 20, y: 0, type: 'end', chainId: 'chain-1' });
  });
  ...
  it('should generate single endpoint for closed chain', () => {
    ...
    expect(endpoints[0]).toEqual({ x: 0, y: 0, type: 'start', chainId: 'chain-1' });
  });

  it('should skip empty chains', () => {
    const chains: Chain[] = [
      createTestChain('empty-chain', []),
      createTestChain('chain-1', [createLineShape('line-1', 0, 0, 10, 0)])
    ];
  ...
  it('should handle chains with single shape', () => {
    ...
    expect(endpoints).toHaveLength(2);
    expect(endpoints).toContainEqual({ x: 0, y: 0, type: 'start', chainId: 'chain-1' });
    expect(endpoints).toContainEqual({ x: 10, y: 10, type: 'end', chainId: 'chain-1' });
  });
  ...
  it('should handle multiple chains', () => {
    ...
    expect(endpoints.filter(e => e.chainId === 'chain-1')).toHaveLength(2);
    expect(endpoints.filter(e => e.chainId === 'chain-2')).toHaveLength(2);
  });
});