...
describe('CuttingParameters Component - Function Coverage', () => {
    ...
    describe('parameter binding and updates', () => {
        it('should render with default parameters', () => {
            ...
            const feedRateInput = container.querySelector(
                '#feedRate'
            ) as HTMLInputElement;
            expect(feedRateInput.value).toBe('1000');

            const pierceHeightInput = container.querySelector(
                '#pierceHeight'
            ) as HTMLInputElement;
            expect(pierceHeightInput.value).toBe('3.8');

            const pierceDelayInput = container.querySelector(
                '#pierceDelay'
            ) as HTMLInputElement;
        ...
        it('should show correct units in labels', () => {
            ...
            const feedRateLabel = container.querySelector(
                'label[for="feedRate"]'
            );
            expect(feedRateLabel?.textContent).toContain('inch/min');

            const pierceHeightLabel = container.querySelector(
                'label[for="pierceHeight"]'
            );
        ...
        it('should bind all parameter fields', async () => {
            ...
            const cutHeightInput = container.querySelector(
                '#cutHeight'
            ) as HTMLInputElement;
            expect(cutHeightInput.value).toBe('1.5');

            const kerfInput = container.querySelector(
                '#kerf'
            ) as HTMLInputElement;
    ...
    describe('reactive unit display', () => {
        it('should update unit display when units prop changes', () => {
            ...
            const feedRateLabelInch = newContainer.querySelector(
                'label[for="feedRate"]'
            );
