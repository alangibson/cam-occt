...
describe('ResizableColumn Component', () => {
  ...
  it('should use default width when no localStorage value exists', () => {
    ...
    const column = container.querySelector('.resizable-column') as HTMLElement;
  ...
  it('should save width to localStorage when resizing ends', async () => {
    ...
    expect(localStorageMock.setItem).toHaveBeenCalledWith('test-column', expect.any(String));
  });
  ...
  it('should support keyboard resize with arrow keys', async () => {
    ...
    const { container } = render(ResizableColumn, { 
      props: { storageKey: 'keyboard-test', width: 280, position: 'left' }
    });
    ...
    expect(localStorageMock.setItem).toHaveBeenCalledWith('keyboard-test', '290');
  });
  ...
  it('should handle keyboard resize for right position', async () => {
    ...
    const { container } = render(ResizableColumn, { 
      props: { storageKey: 'keyboard-right-test', width: 280, position: 'right' }
    });
  ...
  it('should apply correct CSS classes for position', () => {
    ...
    expect(rightHandle?.classList.contains('resize-handle-right')).toBe(true);
  });
