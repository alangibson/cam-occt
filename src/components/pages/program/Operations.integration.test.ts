...
describe('Operations Highlighting Integration', () => {
    ...
    it('should handle multiple rapid hover changes correctly', () => {
        ...
        expect(selectionStore.getState().parts.highlighted).toBe(partId1);

        selectionStore.selectChain(chainId1);
        expect(selectionStore.getState().chains.selected.has(chainId1)).toBe(
            true
        );
        expect(selectionStore.getState().chains.selected.size).toBe(1);

        selectionStore.highlightPart(partId2);
        expect(selectionStore.getState().parts.highlighted).toBe(partId2);

        selectionStore.selectChain(chainId2);
        expect(selectionStore.getState().chains.selected.has(chainId2)).toBe(
            true
        );
        expect(selectionStore.getState().chains.selected.size).toBe(1);
