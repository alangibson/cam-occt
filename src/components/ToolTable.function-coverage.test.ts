...
describe('ToolTable Component - Function Coverage', () => {
  ...
  describe('addNewTool function', () => {
    ...
    it('should handle max tool number calculation with gaps', async () => {
      ...
      const existingTools = JSON.stringify([{
        toolNumber: 5, toolName: 'Tool 5', feedRate: 100, rapidRate: 3000,
        pierceHeight: 3.8, pierceDelay: 0.5, arcVoltage: 120, kerfWidth: 1.5,
        thcEnable: true, gasPressure: 4.5, pauseAtEnd: 0, puddleJumpHeight: 50,
        puddleJumpDelay: 0, plungeRate: 500, id: 'tool-5'
      }, {
        toolNumber: 10, toolName: 'Tool 10', feedRate: 100, rapidRate: 3000,
        pierceHeight: 3.8, pierceDelay: 0.5, arcVoltage: 120, kerfWidth: 1.5,
        thcEnable: true, gasPressure: 4.5, pauseAtEnd: 0, puddleJumpHeight: 50,
        puddleJumpDelay: 0, plungeRate: 500, id: 'tool-10'
      }]);
  ...
  describe('localStorage integration', () => {
    it('should load tools from localStorage on mount', () => {
      ...
      localStorageMock.getItem.mockReturnValue(JSON.stringify(savedTools));
    ...
    it('should handle invalid localStorage data gracefully', () => {
      ...
      expect(consoleSpy).toHaveBeenCalledWith('Failed to load tools from localStorage:', expect.any(Error));
  ...
  describe('tool operations', () => {
    beforeEach(() => {
      ...
      const existingTools = JSON.stringify([{
        toolNumber: 1, toolName: 'Test Tool', feedRate: 100, rapidRate: 3000,
        pierceHeight: 3.8, pierceDelay: 0.5, arcVoltage: 120, kerfWidth: 1.5,
        thcEnable: true, gasPressure: 4.5, pauseAtEnd: 0, puddleJumpHeight: 50,
        puddleJumpDelay: 0, plungeRate: 500, id: 'test-tool-1'
      }]);
      localStorageMock.getItem.mockReturnValue(existingTools);
    });
    ...
    it('should delete tool', async () => {
      ...
      const deleteButton = container.querySelector('[title="Delete tool"]');
    ...
    it('should duplicate tool with incremented number', async () => {
      ...
      const duplicateButton = container.querySelector('[title="Duplicate tool"]');
    ...
    it('should update tool properties through input fields', async () => {
      ...
      const toolNameInput = container.querySelector('input[type="text"]') as HTMLInputElement;
      if (toolNameInput) {
        // Use change event instead of input since component uses onchange
        await fireEvent.change(toolNameInput, { target: { value: 'Updated Tool Name' } });
  ...
  describe('drag and drop functionality', () => {
    beforeEach(() => {
      ...
      const existingTools = JSON.stringify([{
        toolNumber: 1, toolName: 'Tool 1', feedRate: 100, rapidRate: 3000,
        pierceHeight: 3.8, pierceDelay: 0.5, arcVoltage: 120, kerfWidth: 1.5,
        thcEnable: true, gasPressure: 4.5, pauseAtEnd: 0, puddleJumpHeight: 50,
        puddleJumpDelay: 0, plungeRate: 500, id: 'tool-1'
      }, {
        toolNumber: 2, toolName: 'Tool 2', feedRate: 200, rapidRate: 4000,
        pierceHeight: 4.0, pierceDelay: 0.6, arcVoltage: 130, kerfWidth: 2.0,
        thcEnable: false, gasPressure: 5.0, pauseAtEnd: 1, puddleJumpHeight: 60,
        puddleJumpDelay: 0.5, plungeRate: 600, id: 'tool-2'
      }]);
      localStorageMock.getItem.mockReturnValue(existingTools);
    });
