<script lang="ts">
  ...
  function drawPathLeads(path: Path, operation: Operation) {
    ...
    // Draw lead-in
    if (showLeadIn && leadResult.leadIn && leadResult.leadIn.points.length > 1) {
      const opacity = getLeadOpacity(path, 'leadIn');
      drawLeadGeometry(leadResult.leadIn.points, leadInColor, opacity);
    }

    // Draw lead-out
    if (showLeadOut && leadResult.leadOut && leadResult.leadOut.points.length > 1) {
   ...
   */
  function drawLeadGeometry(points: Point2D[], color: string, opacity: number = 1.0) {
   ...
   */
  function getLeadOpacity(path: Path, leadType: 'leadIn' | 'leadOut'): number {
    if (!isSimulating) {
      return leadType === 'leadIn' ? leadInOpacity : leadOutOpacity;
    }

    // During simulation, reduce opacity of static leads
    const baseOpacity = leadType === 'leadIn' ? leadInOpacity : leadOutOpacity;
   ...
   */
  export function setVisualizationMode(mode: 'normal' | 'simulation' | 'hidden') {
    switch (mode) {
      case 'normal':
        showLeadIn = true;
        showLeadOut = true;
