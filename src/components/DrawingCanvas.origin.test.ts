...
describe('Drawing Canvas Origin Positioning', () => {
  ...
  it('should maintain origin position when canvas size changes', () => {
    ...
    const originalTransformer = new CoordinateTransformer(originalCanvas, 1, offset);
    ...
    const resizedTransformer = new CoordinateTransformer(resizedCanvas, 1, offset);
    const resizedOrigin = resizedTransformer.getScreenOrigin();
    
    // Origin percentage should be the same
    expect(originalOrigin.x / originalCanvas.width).toBe(resizedOrigin.x / resizedCanvas.width);
    expect(originalOrigin.y / originalCanvas.height).toBe(resizedOrigin.y / resizedCanvas.height);
